//<---------------------------------------------------------------------->															
Report 50051 "Loans Guaranteed Report"
{
    ApplicationArea = All;
        ...
        dataitem(Customer; Customer)
        {
            ...
            dataitem("Loans Guarantee Details"; "Loans Guarantee Details")
            {
                ...
                begin
                    ...
                    approvedAmount := Loansr."Approved Amount";
                    FNo := FNo + 1;
                    VarCurrLiability := 0;
                    //END;															
                    ...
                    if ObjLoans.Get("Loans Guarantee Details"."Loan No") then begin
                        if (ObjLoans."Approved Amount" = 0) and (ObjLoans."Loan Disbursement Date" < 20240331D) then begin
                            ObjLoans."Approved Amount" := ObjLoans."Requested Amount";
                            ObjLoans.Modify;
                        end else if (ObjLoans."Approved Amount" = 0) and (ObjLoans."Loan Disbursement Date" > 20240331D) then begin
                            ObjLoans.CalcFields("Disbursed Amount");
                            ObjLoans."Approved Amount" := ObjLoans."Disbursed Amount";
                            ObjLoans.Modify;
                        end;
                        ...
                        if ObjLoans."Outstanding Balance" > 0 then begin
                            if "Loans Guarantee Details"."Total Loans Guaranteed" <> 0 then begin
                                // if obj
                                VarCurrLiability := ROUND((ObjLoans."Outstanding Balance" / ObjLoans."Approved Amount") * "Loans Guarantee Details"."Amont Guaranteed", 0.5, '=');
                    ...
                    ObjLoanGuar.RESET;
                    ObjLoanGuar.SETRANGE(ObjLoanGuar."Loan No", "Loans Guarantee Details"."Loan No");

                    ObjLoanGuar.SETRANGE(ObjLoanGuar."Loanees  No", "Loans Guarantee Details"."Member No");
                    IF ObjLoanGuar.FINDSET THEN BEGIN

                        ObjLoanGuar.CALCFIELDS(ObjLoanGuar."Outstanding Balance");

                        // VarCurrLiability := (ObjLoanGuar."Outstanding Balance" / Loansr."Approved Amount") * ObjLoanGuar."Amont Guaranteed";
                        //MODIFY;
