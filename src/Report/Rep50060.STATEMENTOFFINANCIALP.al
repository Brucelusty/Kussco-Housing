...
Report 50060 "STATEMENT OF FINANCIAL P"
{
    ApplicationArea = All;

    ExcelLayout = 'Layouts/STATEMENTOFFINANCIALP2.xlsx';
    DefaultLayout = Excel;
    ...
    dataset
    {
        dataitem("Company Information"; "Company Information")
        {
            ...
            column(shortTermDeposits; shortTermDeposits)
            {
            }
            column(WithdrawableFOSASavings; WithdrawableFOSASavings)
            {
            }
            ...
            begin
                ...
                //shortTermDeposits
                GLAccount.Reset;
                GLAccount.SetFilter(GLAccount.StatementOfFP, '%1', GLAccount.Statementoffp::WithdrawableFOSASavings);
                GLAccount.SetFilter(GLAccount."Date Filter", DateFilter);
                GLAccount.SetAutocalcFields("Net Change");
                if GLAccount.FindSet then begin
                    repeat
                        WithdrawableFOSASavings += GLAccount."Net Change" * -1;
                    until GLAccount.Next = 0;
                    MESSAGE('%1', WithdrawableFOSASavings);
                end;

                //shortTermDeposits
                GLAccount.Reset;
                GLAccount.SetFilter(GLAccount.StatementOfFP, '%1', GLAccount.Statementoffp::ShortTermDeposits);
                GLAccount.SetFilter(GLAccount."Date Filter", DateFilter);
                GLAccount.SetAutocalcFields("Net Change");
                if GLAccount.FindSet then begin
                    repeat
                        shortTermDeposits += GLAccount."Net Change" * -1;
                    until GLAccount.Next = 0;
                    MESSAGE('%1', shortTermDeposits);
                end;
                ...
                TotalLiabilitiesNew := Nonwithdrawabledeposits + AccountsPayableOtherLiabilities + WithdrawableFOSASavings + shortTermDeposits;
    ...
    var
        ...
        TotalAssets: Decimal;
        WithdrawableFOSASavings: Decimal;
