report 50037 "Teller-Treasury Report"
{
    ApplicationArea = All;
    DefaultLayout = RDLC;
    RDLCLayout = 'Layouts/TellerTreasuryReport.rdlc';
    ...
    dataset
    {
        dataitem("Treasury Transactions"; "Treasury Transactions")
        {
            ...
            column(Destination_Account_Balance;"Destination Account Balance")
            {}
            ...
            column(CashDeposit;CashDeposit)
            {}
            column(CashWithdrawal;CashWithdrawal)
            {}
            column(chequeTransactions;chequeTransaction)
            {}
            ...
            begin
                ...
                reportTime:= CreateDateTime(Today, Time);
                startDay:= CreateDateTime(Today, 0T);
                // Error('%1', startDay);

                CashDeposit:= 0;
                bankLedger.Reset();
                bankLedger.SetRange("Posting Date", "Treasury Transactions"."Transaction Date");
                bankLedger.SetRange("Bank Account No.", "Treasury Transactions"."From Account");
                // bankLedger.SetFilter(SystemCreatedAt, '%1..%2', startDay, reportTime);
                if bankLedger.FindSet() then begin
                    repeat
                        CashDeposit:= CashDeposit + bankLedger."Debit Amount";
                    until bankLedger.Next() = 0;
                    TreasuryTrans.Reset();
                    TreasuryTrans.SetRange("Transaction Date", "Treasury Transactions"."Transaction Date");
                    TreasuryTrans.SetRange("Transaction Type", "Transaction Type"::"Issue To Teller");
                    if TreasuryTrans.FindSet() then begin
                        repeat
                            CashDeposit:= CashDeposit - TreasuryTrans.Amount;
                        until TreasuryTrans.Next()= 0;
                    end;
                end;
                ...
                if bankLedger.FindSet() then begin
                    repeat
                        CashWithdrawal:= CashWithdrawal + bankLedger."Credit Amount";
                    until bankLedger.Next() = 0;
                    TreasuryTrans.Reset();
                    TreasuryTrans.SetRange("Transaction Date", "Treasury Transactions"."Transaction Date");
                    TreasuryTrans.SetRange("Transaction Type", "Transaction Type"::"Return To Treasury");
                    if TreasuryTrans.FindSet() then begin
                        repeat
                            CashWithdrawal:= CashWithdrawal - TreasuryTrans.Amount;
                        until "Treasury Transactions".Next()= 0;
                    end;
                end;
                
                chequeTransaction:= 0;
                trans.Reset();
                trans.SetRange("Transaction Date", "Treasury Transactions"."Transaction Date");
                trans.SetRange(Cashier, "Treasury Transactions"."From Account User");
                trans.SetRange("Transaction Type", 'ORDCHECK');
                // trans.SetFilter(SystemCreatedAt, '%1..%2', startDay, reportTime);
                if trans.FindSet() then begin
                    repeat
                       chequeTransaction:=chequeTransaction + trans.Amount;
                    until trans.Next() = 0;
                end;
