report 50092 "Posted Bank Reconciliation Ki"
{
    ApplicationArea = All;
    // version Payment ProcessesV1.0(Surestep Systems)

    DefaultLayout = RDLC;
    RDLCLayout = 'Layouts/PostedKIBankrec.rdlc';
    Caption = 'KI Bank Reconciliation Posted';
    UsageCategory = ReportsAndAnalysis;

    dataset
    {
        dataitem("Bank Acc. Reconciliation"; "Bank Reconciliation Header")
        {
            ...
            dataitem("Bank Account Ledger Entry"; "Bank Account Ledger Entry Rec")
            {
                DataItemLink = "Bank Account No." = FIELD("Bank Account No."), "Statement No." = FIELD("Statement No.");
                DataItemTableView = WHERE("Debit Amount" = FILTER(> 0), Reconciled = filter(false));
            ...
            dataitem("<Bank Acc. Reconciliation Ln1>"; "Bank Account Ledger Entry Rec")
            {
                DataItemLink = "Bank Account No." = FIELD("Bank Account No."), "Statement No." = FIELD("Statement No.");
                DataItemTableView = WHERE("Credit Amount" = FILTER(> 0), Reconciled = filter(false));
            ...
            dataitem("<Bank Acc. Reconciliation pc>"; "Bank Account Ledger Entry Rec")
            {
                DataItemLink = "Bank Account No." = FIELD("Bank Account No."), "Statement No." = FIELD("Statement No.");
                DataItemTableView = WHERE(Reconciled = filter(true), Amount = filter(< 0));
            ...
            dataitem("<Bank Acc. Reconciliation rc>"; "Bank Account Ledger Entry Rec")
            {
                DataItemLink = "Bank Account No." = FIELD("Bank Account No."), "Statement No." = FIELD("Statement No.");
                DataItemTableView = WHERE(Reconciled = filter(true), Amount = filter(> 0));
    ...
    var
        ...
        BankStatementLine1: Record "Bank Account Ledger Entry Rec";
        CompanyInfo: Record "Company Information";
        CompanyInformation: Record "Company Information";
        ReconciliationStatement: Text;
        TotalDifference: Decimal;
        BankStatementLine2: Record "Bank Account Ledger Entry Rec";
        BankRecPresented: Record "Bank Account Ledger Entry Rec";
