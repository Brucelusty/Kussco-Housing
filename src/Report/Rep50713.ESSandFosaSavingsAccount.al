report 50713 "ESS and Fosa Savings Account"
{
    ApplicationArea = All;
    DefaultLayout = RDLC;
    RDLCLayout = 'Layouts\Opened Account Report.rdlc';

    dataset
    {
        dataitem("FOSA Account Applicat. Details";"FOSA Account Applicat. Details")
        {
            DataItemTableView = sorting(Name) where("Account Type" = filter(104|105|106|107|108|109|110|111),created = filter(true), status = filter(Approved));
