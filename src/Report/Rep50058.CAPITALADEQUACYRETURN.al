...
Report 50058 "CAPITAL ADEQUACY RETURN"
{
    ApplicationArea = All;
    // RDLCLayout = 'Layouts/CAPITALADEQUACYRETURN.rdlc';															
    // DefaultLayout = RDLC;

    ExcelLayout = 'Layouts/CAPITALADEQUACYRETURN.xlsx';
    DefaultLayout = Excel;
    ...
    dataset
    {
        dataitem(Company; "Company Information")
        {
            ...
            column(GeneralReserves; GeneralReserves)
            {
            }
            ...
            begin
                ...
                //NetSurplus
                GLAccount.Reset;
                GLAccount.SetRange(GLAccount."No.", '400-999-999');
                GLAccount.SetFilter(GLAccount."Date Filter", Datefilter);
                GLAccount.SetAutocalcFields(Balance);
                if GLAccount.FindSet then begin
                    repeat
                        NetSurplusaftertax += (GLAccount."Net Change" * 50 / 100) * -1;
                    until GLAccount.Next = 0;
                end;
                ...
                GLAccount.Reset;
                GLAccount.SetRange(GLAccount."No.", '100-999-999');
                GLAccount.SetFilter(GLAccount."Date Filter", Datefilter);
                GLAccount.SetAutocalcFields(Balance);
                if GLAccount.FindSet then begin
                    repeat
                ...
                GeneralReserves := 0;
                GLAccount.Reset;
                GLAccount.SetFilter(GLAccount."Capital adequecy", '%1', GLAccount."capital adequecy"::GeneralReserves);
                if GLAccount.FindSet then begin
                    repeat
                        GLEntry.Reset;
                        GLEntry.SetRange(GLEntry."G/L Account No.", GLAccount."No.");
                        ...
                        if GLEntry.FindSet then begin
                            GLEntry.CalcSums(Amount);
                            GeneralReserves += GLEntry.Amount * -1;
                        end;
                    until GLAccount.Next = 0;
                end;
                //gov securities															
                ...
                //Message('%1',TotalDepositsLiabilities);
                ...
                Sub_Total := ShareCapital + CapitalGrants + retainedEarnins + NetSurplusaftertax + StatutoryReserve + Otherreserves + GeneralReserves;
                TotalDeductions := InvestmentsinSubsidiary + OtherDeductions;
                CORECAPITAL := Sub_Total;
