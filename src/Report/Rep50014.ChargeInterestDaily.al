report 50014 "Charge Interest Daily"
{
    ApplicationArea = All;
            ...
            begin
                ...
                if loanapp.Find('-') then begin
                    ...
                    if loanapp.Installments > 12 then
                        //Amount:= ROUND((loanapp.Interest/ 12 / 100) * loanapp."Approved Amount", 1, '>')
                        Amount := ROUND(OutstandingBalance * (loanapp.Interest / 1200 / Divisible), 0.01, '=')
                    else
                        Amount := ROUND(OutstandingBalance * (loanapp.Interest / loanapp.Installments / 100 / Divisible), 0.01, '=');
                    // Amount:= ROUND((loanapp.Interest / loanapp.Installments / 100) * loanapp."Approved Amount", 1, '>');
                    //Amount := ROUND(OutstandingBalance * (loanapp.Interest / 1200 / Divisible), 0.01, '=');
