...
Report 50598 "Post Member Daily Processes"
{
    ApplicationArea = All;
    ProcessingOnly = true;
    ...
    dataset
    {
        ...
        {
            DataItemTableView = where("Membership status" = filter(<> Exited | Deceased), Blocked = filter(" "));
            ...
            begin
                ...
                if ("Current Shares" > 0) and ("Deposits Account No" <> '') and ("Deposits Penalty Exists" = true) and ("Membership status" = "Membership status"::Active) then begin
                    ObjAccounts.Reset;
                    ObjAccounts.SetRange(ObjAccounts."No.", "Deposits Account No");
                    ObjAccounts.SetRange(ObjAccounts.Blocked, ObjAccounts.Blocked::" ");
                    ObjAccounts.SetFilter(ObjAccounts."Membership status", '<>%1&<>%2', ObjAccounts."Membership status"::"Fully Exited", ObjAccounts."Membership status"::Deceased);
            ...
            begin
                ...
                SFactory.FnRunManageAudit_Tracker;//=================================================================Audit Issues Tracker "Membership status"															
                //SFactory.FnRunGetMembershipDormancy"Membership status"(WorkDate);//========================================================Member Dormancy															
                //SFactory.FnRunGetMemberAccountDormancy"Membership status"(WorkDate);//======================================================Account Dormancy															
    ...
    begin
        ...
        ObjMember.SetFilter(ObjMember."Membership status", '<>%1|%2', ObjMember."Membership status"::Exited, ObjMember."Membership status"::Deceased);
        ...
        if ObjMember.Find('-') then begin
            repeat
                ...
                if Abs(ObjMember."Registration Fee Paid") < ObjGensetup."BOSA Registration Fee Amount" then begin
                    ...
                    GenJournalLine."account type"::Customer, ObjMember."No.", WorkDate, VarRegistrationFeeSeparate * -1, 'BOSA', '',
    ...
    begin
        ...
        ObjMember.SetFilter(ObjMember."Membership status", '%1|%2', ObjMember."Membership status"::Active, ObjMember."Membership status"::Dormant);
        if ObjMember.FindSet then begin
            repeat
                ...
                ObjAccounts.SetFilter(ObjAccounts."Membership status", '<>%1', ObjAccounts."Membership status");
    ...
    begin
        ...
        if ObjMember.FindSet then begin
            repeat
                ...
                ObjAccount.SetFilter(ObjAccount."Membership status", '<>%1', ObjAccount."Membership status"::"Fully Exited");
    ...
    begin
        ...
        ObjGensetup.Get;
        ObjMember.CalcFields(ObjMember."FOSA Shares", ObjMember."FOSA Shares Status", ObjMember."FOSA Shares Status", ObjMember."Deposits Account Status");
        ObjMember.Reset;
        ObjMember.SetFilter(ObjMember."Share Capital No", '<>%1', '');
        ...
        if ObjMember.FindSet then begin
            repeat
                ...
                //=========================================================================================Modify Account "Membership status"															
                if ObjAccount.Get(ObjMember."FOSA Shares Account No") then begin
                    ObjAccount."Membership status" := ObjAccount."Membership status"::"Fully Exited";
    ...
    begin
        ...
        ObjMember.SetFilter(ObjMember."Shares Retained", '>=%1', ObjGensetup."Retained Shares");
        ObjMember.SetRange(ObjMember."Account Category", ObjMember."account category"::Individual);
        ObjMember.SetRange(ObjMember."Membership status", ObjMember."Membership status"::Active);
        if ObjMember.FindSet then begin
            repeat
                DOCUMENT_NO := SFactory.FnRunGetNextTransactionDocumentNo;
