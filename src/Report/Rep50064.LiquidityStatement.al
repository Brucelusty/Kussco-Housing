...
Report 50064 "Liquidity Statement"
{
    ApplicationArea = All;
    // RDLCLayout = 'Layouts/LiquidityStatement.rdlc';															
    // DefaultLayout = RDLC;	
    ExcelLayout = 'Layouts/LiquidityStatement.xlsx';
    DefaultLayout = Excel;
    ...
    dataset
    {
        dataitem(Company; "Company Information")
        {
            ...
            begin
                ...
                GLAccount.Reset;
                GLAccount.SetFilter(GLAccount.Liquidity, '%1', GLAccount.Liquidity::BalanceswithotherSaccoSocieties);
                if GLAccount.FindSet then begin
                    repeat
                        GLEntry.Reset;
                        GLEntry.SetRange(GLEntry."G/L Account No.", GLAccount."No.");
                        ...
                        if GLEntry.FindSet then begin
                            GLEntry.CalcSums(Amount);
                            BalanceswithotherSaccoSocieties += GLEntry.Amount;
                        end;
                    until GLAccount.Next = 0;
                end;//20199	
                ...
                if GLAccount.FindSet then begin
                    repeat
                        ...
                        if GLEntry.FindSet then begin
                            GLEntry.CalcSums(Amount);
                            TimeDeposits += GLEntry.Amount * -1;
                        end;
                    until GLAccount.Next = 0;
                end;
                //Matured Liabilities															
                GLAccount.Reset;
                GLAccount.SetFilter(GLAccount.Liquidity, '%1', GLAccount.Liquidity::MaturedLiabilities);
                if GLAccount.FindSet then begin
                    repeat
                        GLEntry.Reset;
                        GLEntry.SetRange(GLEntry."G/L Account No.", GLAccount."No.");
                        ...
                        if GLEntry.FindSet then begin
                            GLEntry.CalcSums(Amount);
                            MaturedLiabilities += GLEntry.Amount * -1;
                        end;
                    until GLAccount.Next = 0;
                end;
                ...
                NETLIQUIDASSETS :=
                LocalNotes + ForeignNotes + BalancesduetoFinanciaInstitutions + BalancesDuetootherSaccosocieties + BalanceswithCommercialBanks + BalanceswithotherFinancialInstitutions + BalanceswithotherSaccoSocieties + TreasuryBills + TreasuryBonds
                + OverdraftsandMatured;
                TotalOtherliabilities := MaturedLiabilities + LiabilitiesMaturingwithin91Days;
                TotalShorTermLiabilities := TotalOtherliabilitiesNew + MaturedLiabilities + TimeDeposits;
                Ratio := (NETLIQUIDASSETS / TotalShorTermLiabilities);
                Minumholding := 0.15;
    ...
    var
        ...
        TotalShorTermLiabilities: Decimal;
