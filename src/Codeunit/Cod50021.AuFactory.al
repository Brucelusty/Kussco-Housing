...
Codeunit 50021 "Au Factory"
{
    ...
    begin
        ...
        //FnCreateGuarantorRecoveryReimbursment('0014017200016');
    ...
    procedure FnCheckIfDocNoAlreadyPosted(DocNo: Code[20]): Boolean
    var
        myInt: Integer;
    ...
    begin
        ...
        vendLedger.SetRange(Reversed, false);
        if vendLedger.Find('-') then begin
            exit(true);
        end;

        bankLedger.Reset();
        bankLedger.SetRange("Document No.", DocNo);
        ...
        glEntries.Reset();
        glEntries.SetRange("Document No.", DocNo);
    ...
    procedure FnRecoverDefaultedLoans(memberNo: Code[20]; recoverDefault: Boolean): Decimal
    var
        myInt: Integer;
        lineNo: Integer;
        accBal: Decimal;
    ...
    begin
        ...
        FosaAcc := FnGetAccountNumber('103', memberNo);
        accBal := FnGetFosaAccountBalance(FosaAcc);
        runBal := accBal;

        ObjLoans.Reset();
        ObjLoans.SetRange("Client Code", memberNo);
        ObjLoans.SetFilter("Loan Product Type", '%1|%2', 'A03', 'M_OD');
        ObjLoans.SetFilter("Expected Date Of Completion", '<%1', Today);
        ObjLoans.SetAutoCalcFields("Total Outstanding Balance");
        ObjLoans.SetFilter("Total Outstanding Balance", '>%1', 0);
        if ObjLoans.Find('-') then begin
            repeat
                ObjLoans.CalcFields("Outstanding Balance", "Outstanding Interest", "Outstanding Penalty");
                if ObjLoans."Outstanding Penalty" > runBal then begin
                    repayment := runBal;
                end else begin
                    repayment := ObjLoans."Outstanding Penalty";
                end;

                lineNo := lineNo + 1000;
                FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, lineNo, GenJournalLine."Transaction Type"::"Loan Penalty Paid", GenJournalLine."Account Type"::Customer,
                FosaAcc, Today, repayment * -1, 'BOSA', ObjLoans."Loan  No.", 'Defaulted ' + ObjLoans."Loan Product Type Name" + ' Penalty Recovery - ' + Format(Today), ObjLoans."Loan  No.", GenJournalLine."Application Source"::Mobile);
                ...
                FosaAcc, Today, repayment, 'BOSA', ObjLoans."Loan  No.", 'Defaulted ' + ObjLoans."Loan Product Type Name" + ' Penalty Recovery - ' + Format(Today), ObjLoans."Loan  No.", GenJournalLine."Application Source"::Mobile);
                runBal := runBal - repayment;

                if ObjLoans."Outstanding Interest" > runBal then begin
                    repayment := runBal;
                end else begin
                    repayment := ObjLoans."Outstanding Interest";
                end;

                lineNo := lineNo + 1000;
                FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, lineNo, GenJournalLine."Transaction Type"::"Interest Paid", GenJournalLine."Account Type"::Customer,
                FosaAcc, Today, repayment * -1, 'BOSA', ObjLoans."Loan  No.", 'Defaulted ' + ObjLoans."Loan Product Type Name" + ' Interest Recovery - ' + Format(Today), ObjLoans."Loan  No.", GenJournalLine."Application Source"::Mobile);

                lineNo := lineNo + 1000;
                FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, lineNo, GenJournalLine."Transaction Type"::" ", GenJournalLine."Account Type"::Vendor,
                FosaAcc, Today, repayment, 'BOSA', ObjLoans."Loan  No.", 'Defaulted ' + ObjLoans."Loan Product Type Name" + ' Interest Recovery - ' + Format(Today), ObjLoans."Loan  No.", GenJournalLine."Application Source"::Mobile);
                runBal := runBal - repayment;

                if ObjLoans."Outstanding Balance" > runBal then begin
                    repayment := runBal;
                end else begin
                    repayment := ObjLoans."Outstanding Balance";
                end;

                lineNo := lineNo + 1000;
                FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, lineNo, GenJournalLine."Transaction Type"::Repayment, GenJournalLine."Account Type"::Customer,
                FosaAcc, Today, repayment * -1, 'BOSA', ObjLoans."Loan  No.", 'Defaulted ' + ObjLoans."Loan Product Type Name" + ' Principal Recovery - ' + Format(Today), ObjLoans."Loan  No.", GenJournalLine."Application Source"::Mobile);

                lineNo := lineNo + 1000;
                FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, lineNo, GenJournalLine."Transaction Type"::" ", GenJournalLine."Account Type"::Vendor,
                FosaAcc, Today, repayment, 'BOSA', ObjLoans."Loan  No.", 'Defaulted ' + ObjLoans."Loan Product Type Name" + ' Principal Recovery - ' + Format(Today), ObjLoans."Loan  No.", GenJournalLine."Application Source"::Mobile);
                runBal := runBal - repayment;
                netBal := accBal - runBal;
                ...
                if ObjMembers.FindFirst() then begin
                    CreationMessage := 'Dear ' + NameStyle.NameStyle(ObjLoans."Client Code") + ', Ksh ' + Format(netBal) + ' has been deducted to repay your defaulted ' + ObjLoans."Loan Product Type Name" + '. Thank you for Banking With Us.';
                    smsManagement.SendSmsWithID(Source::LOAN_DEFAULTED, ObjMembers."Mobile Phone No", CreationMessage, ObjMembers."No.", ObjMembers."FOSA Account No.", false, 200, false, 'CBS', CreateGuid(), 'CBS');
                end;
    ...
    procedure FnMaskString(srcString: Text; startPos: Integer; length: Integer; maskChar: Text): Text
    var
        strI: Text;
        paddedStr: Text[5];
        maskedStr: Text;
    begin
        // strI := 
        paddedStr := PadStr(paddedStr, length, maskChar);
        maskedStr := DelStr(srcString, startPos, length);
        maskedStr := InsStr(maskedStr, paddedStr, startPos);
        Exit(maskedStr);
    end;

    procedure getAccountType(account: Code[50]) Type: Code[50]
    var
        vend: Record Vendor;
    ...
    begin
        ...
        if loanApp.FindFirst() then begin
            LoanProduct := loanApp."Loan Product Type";
        end;
        GenJournalLine.Init;
        GenJournalLine."Journal Template Name" := TemplateName;
        GenJournalLine."Journal Batch Name" := BatchName;
        ...
        GenJournalLine.Description := TransactionDescription;
        GenJournalLine."Application Source" := AppSource;
        GenJournalLine."Loan Product Type" := LoanProduct;
        // GenJournalLine.Validate(GenJournalLine."Currency Code");
        GenJournalLine.Amount := TransactionAmount;
        GenJournalLine."External Document No." := ExternalDocumentNo;
        GenJournalLine.Validate(GenJournalLine.Amount);
    ...
    procedure FnCreateGnlJournalLineSal(TemplateName: Text; BatchName: Text; DocumentNo: Code[30]; LineNo: Integer; TransactionType: Option " ","Registration Fee","Share Capital","Interest Paid","Loan Repayment","Deposit Contribution","Insurance Contribution","Benevolent Fund",Loan,"Unallocated Funds",Dividend,"FOSA Account"; AccountType: Enum "Gen. Journal Account Type"; AccountNo: Code[50]; TransactionDate: Date; TransactionAmount: Decimal; DimensionActivity: Code[40]; ExternalDocumentNo: Code[50]; TransactionDescription: Text; LoanNumber: Code[50]; AppSource: Option " ",CBS,ATM,Mobile,Internet,MPESA,Equity,"Co-op",Family,"SMS Banking"; MemberNo: Code[50]; Receipttype: Option " ","1-Gross salary","2-Salary Processing Fee","3-Excise Duty","4-Loan Repayment","5-Interest Paid","6-STO")
    var
        GenJournalLine: Record "Gen. Journal Line";
        loanApp: Record "Loans Register";
        LoanProduct: Code[60];
    begin
        ...
        GenJournalLine."Line No." := LineNo;
        GenJournalLine."Account Type" := AccountType;
        GenJournalLine."Salary Receipt Type" := Receipttype;
        GenJournalLine."Account No." := AccountNo;
        GenJournalLine."Transaction Type" := TransactionType;
        GenJournalLine."Loan No" := LoanNumber;
        GenJournalLine."Loan Product Type" := LoanProduct;
    ...
    begin
        if ObjVendor.Get(Acc) then begin
            ...
            Bal := ObjVendor.GetAvailableBalance();
        end
    end;
    ...
    begin
        ...
        if loanTypes.Find('-') then begin
            DocNumber := CU.GetNextNo('LOANSB', 0D, true);
            LoansRec.Init;
            ...
            if LoansRec.Get('BLN_00041') then begin
                ...
                LoansRec."Global Dimension 1 Code" := '';
    ...
    procedure FnCalculatePaye(Chargeable: Decimal; OtherRelief: Decimal) PAYE: Decimal
    var
    ...
    begin
        ...
        exit(BAND1 + BAND2 + BAND3 + BAND4 + BAND5 - 2400 - OtherRelief);
    end;
    ...
    begin
        ...
        ObjNextNo:=Obj"No. Series".TryGetNextNo(ObjSalesSetup."Closure  Nos",PostingDate);
          ObjNoSeries.RESET;
        ...
        if ObjMembers.Get(MemberNo) then begin
            ObjMembers."Membership Status" := ObjMembers."Membership Status"::"Awaiting Exit";
            ObjMembers.Modify;
        end;
    ...
    begin
        ...
        ObjProducts.SetFilter(ObjProducts."Global Dimension 1 Code", '<>%1', '');
    ...
    begin
        ...
        if RunningBalance > 0 then begin
            if ObjLoans.Get(VarLoanNoRecovered) then begin
                ...
                if ObjVendor.FindSet then begin
                    ...
                    GenJournalLine."account type"::Vendor, ObjLoans."Loan Debt Collector", VarPostingDate, AmountToDeduct * -1, '', EXTERNAL_DOC_NO,
                    ...
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, VarLoanSettlementAcc, VarPostingDate, AmountToDeduct, '', EXTERNAL_DOC_NO,
                    'Debt Collection Charge + VAT from ' + VarLoanNoRecovered + ' ' + ObjLoans."Client Name", VarLoanNoRecovered, GenJournalLine."recovery transaction type"::"Guarantor Recoverd", VarLoanNoRecovered);
            ...
            if ObjLoans.Find('-') then begin
                ...
                if RunningBalance > 0 then begin
                    ...
                    if VarOutstandingPenalty > 0 then begin
                        ...
                        VarLoanPenaltyBalAccount, AmountToDeduct, '', VarLoanNoRecovered);
                        //--------------------------------Debit Loan Penalty Charged-------------------------------------------------------------------------------
                        ...
                        GenJournalLine."account type"::Member, VarMemberNo, VarPostingDate, AmountToDeduct * -1, '', EXTERNAL_DOC_NO,
            ...
            if ObjLoans.Find('-') then begin
                ...
                if RunningBalance > 0 then begin
                    ...
                    if VarOutstandingInterest > 0 then begin
                        ...
                        VarLoanInterestDueBalAccount, AmountToDeduct, '', VarLoanNoRecovered);
            ...
            if ObjLoans.Find('-') then begin
                ...
                if RunningBalance > 0 then begin
                    ...
                    if VarTotalOutstandingInsurance > 0 then begin
                        ...
                        VarLoanInsuranceBalAccount, AmountToDeduct, '', VarLoanNoRecovered);
                        //--------------------------------Debit Loan Insurance Due-------------------------------------------------------------------------------
        ...
        if ObjLoans.Get(VarLoanNoRecovered) then begin
            ...
            GenJournalLine."account type"::Vendor, VarLoanSettlementAcc, VarPostingDate, VarAmounttoDeduct, '', EXTERNAL_DOC_NO,
        ...
        if ObjVendors.Find('-') then begin
            ...
            if ObjGuarantorLedger.FindSet then begin
                ...
                if ObjAccount.Get(ObjGuarantorLedger."Debt Collector") then begin
                    ...
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, VarFOSAAccountNo, WorkDate, VarDebtCollectorFee, '', EXTERNAL_DOC_NO,
                    ...
                    GenJournalLine."account type"::Vendor, VarDepositNo, WorkDate, VarDebtCollectorFee * -1, '', EXTERNAL_DOC_NO,
            ...
            if ObjGuarantorLedger.FindSet then begin
                repeat
                    ...
                    if VarBalanceinFOSA > 0 then begin
                        ...
                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
                        GenJournalLine."account type"::Vendor, VarDepositNo, WorkDate, VarAmounttoDeduct * -1, '', EXTERNAL_DOC_NO,
            ...
            LineNo := LineNo + 10000;
            SFactory.FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
            GenJournalLine."account type"::Vendor, VarFOSAAccountNo, WorkDate, VarTotalAmounttoDeductFromDefaulter, '', EXTERNAL_DOC_NO,
    ...
    begin
        if IsEmailAddressesValid("Recepient Email") = true then begin
            ...
            CCList.Add(AddCC);
            EmailBody := 'Dear ' + "Recepient Name" + ',';
            EmailBody += '<br><br>';

            EmailBody += Body;
            EmailBody += '<br><br>';
            EmailBody += '<Hr>';
            EmailBody += '</body></font></html>';
            EmailMassage.Create("Recepient Email", Subject, EmailBody);
            Email.Send(EmailMassage);
    ...
    procedure FnGetOutstandingInterest(VarLoanNo: Code[30]; CuttOff: Date) VarOutstandingInterest: Decimal
    var
        ObjLoans: Record "Loans Register";
    ...
    begin
        ...
        ObjLoans.Reset;
        ObjLoans.SetRange(ObjLoans."Loan  No.", VarLoanNo);
        ObjLoans.SetAutoCalcFields(ObjLoans."Outstanding Interest");
        ObjLoans.SetFilter(ObjLoans."Expected Date of Completion", '>%1', CuttOff);
        if ObjLoans.Find('-') then begin
            ...
            if ObjLoans."Outstanding Interest" > 0 then begin
                ...
                IntPaid := 0;
                DetCust.Reset();
                DetCust.setrange(DetCust."Customer No.", ObjLoans."Client Code");
                DetCust.setrange(DetCust."Loan No", ObjLoans."Loan  No.");
                DetCust.SetFilter(DetCust."Transaction Type", '%1', DetCust."Transaction Type"::"Interest Paid");
                DetCust.SetFilter(DetCust."Posting Date", '>%1', CuttOff);
                if DetCust.FindSet() then begin
                    DetCust.CalcSums(DetCust.Amount);
                    IntPaid := DetCust.Amount;
                end;

                DetCust.Reset();
                DetCust.setrange(DetCust."Customer No.", ObjLoans."Client Code");
                DetCust.setrange(DetCust."Loan No", ObjLoans."Loan  No.");
                DetCust.SetFilter(DetCust."Transaction Type", '%1|%2', DetCust."Transaction Type"::"Interest Due", DetCust."Transaction Type"::"Interest Paid");
                DetCust.SetFilter(DetCust."Posting Date", '..%1', CuttOff);
                if DetCust.FindSet() then begin
                    DetCust.CalcSums(DetCust.Amount);
                    VarOutstandingInterest := DetCust.Amount;
                end;

                VarOutstandingInterest := VarOutstandingInterest + IntPaid;
                if VarOutstandingInterest < 0 then
                    VarOutstandingInterest := 0;
        ...
        if ObjLoans.Find('-') then begin
            ObjLoans.CalcFields(ObjLoans."Outstanding Interest");
            if ObjLoans."Outstanding Interest" > 0 then begin
                VarOutstandingInterest := ObjLoans."Outstanding Interest";
                if VarOutstandingInterest < 0 then
                    VarOutstandingInterest := 0;
    ...
    begin
        ...
        if ObjLoans.FindSet then begin
            ...
            if ObjLoans.Find('-') then begin
                ...
                VarLoanPayoffAmount := ObjLoans."Outstanding Balance" + ObjLoans."Outstanding Interest";//+ VarOutstandingPenalty;//+ (VarOutstandingInsurance + VarInsurancePayoff);
              ...
              ObjLoans."Penalty Charged", ObjLoans."Penalty Paid", ObjLoans."Interest Paid Historical", ObjLoans."Penalty Paid Historical");
                if RunningBalance > 0 then begin
                    ...
                    if VarOutstandingInterest > 0 then begin
                        ...
                        SFactory.FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Interest Paid",
                        GenJournalLine."account type"::Member, VarMemberNo, VarPostingDate, AmountToDeduct * -1, '', EXTERNAL_DOC_NO,
                        'PayOff:Interest Paid', VarLoanNoRecovered, GenJournalLine."recovery transaction type"::"Guarantor Recoverd", VarLoanNoRecovered);//Kit
    ...
    begin
        ...
        if ObjAccounts.FindSet then begin
            ...
            if ObjAccountTypes.Get(ObjAccounts."Account Type") then begin
                ...
                ObjDailyAccrual."Global Dimension 1 Code" := '';
    ...
    procedure CheckIfScheduleIsGenerated(LoanNo: Code[40]) Checked: Boolean
    var
        LoanSchedule: Record "Loan Repayment Schedule";
    begin
        Checked := false;
    ...
    var
        ...
        WhichDay: Integer;
    begin
        //Message('here %1-%2', LoansRec."Approved Amount", LoansRec."Loan  No.");
        LoansRec.Reset;
        LoansRec.SetRange(LoansRec."Loan  No.", VarLoanNo);
        LoansRec.SetFilter(LoansRec."Approved Amount", '>%1', 0);
        //LoansRec.SETFILTER(LoansRec.Posted,'=%1',TRUE);
        if LoansRec.Find('-') then begin
            if ((LoansRec."Repayment Start Date" <> 0D)) then begin
                ...
                LoansRec.CalcFields("Disbursed Amount", "Total Outstanding Balance");
                // if LoansRec."Total Outstanding Balance" > 0 then begin
                // LoanAmount := LoansRec."Disbursed Amount";
                // LBalance := LoansRec."Disbursed Amount";
                // end else begin
                LoanAmount := LoansRec."Approved Amount";
                LBalance := LoansRec."Approved Amount";
                // end;
                ...
                RunDate := LoansRec."Repayment Start Date";
                InstalNo := 0;
                //message('%1', LoansRec."repayment frequency");
                //Repayment Frequency
                if LoansRec."Repayment Frequency" = LoansRec."repayment frequency"::Daily then
                    RunDate := CalcDate('-1D', RunDate)
                else
                ...
                repeat
                    ...
                    //message('RunDate%1Lbalance%2Mode%3',RunDate,LBalance,LoansRec."Repayment Method");
                    if LoansRec."Repayment Method" = LoansRec."repayment method"::Constants then begin
                        LoansRec.TestField(LoansRec.Repayment);
                        if LBalance < LoansRec.Repayment then
                            LPrincipal := LBalance
                ...
                LoansRec.Repayment := LInterest + LPrincipal;
                LoansRec."Loan Principle Repayment" := LPrincipal;
                LoansRec."Loan Interest Repayment" := LInterest;
                //  LoansRec.Modify;
            end;
        end;
        Commit;
    ...
    procedure FnGenerateLoanRepaymentScheduleZ(VarLoanNo: Code[30])
    var
        LoansRec: Record "Loans Register";
        RSchedule: Record "Loan Repayment Schedule";
    ...
    begin
        ...
        if LoansRec.Find('-') then begin
            ...
            repeat
                ...
                loanType.Reset();
                loanType.SetRange(Code, LoansRec."Loan Product Type");
                if loanType.Find('-') then begin
                    if loanType."Exempt Interest" = true then begin
                        exemptInterest := true;
                    end else
                        exemptInterest := false;
                end;

                if exemptInterest = true then begin
                    LPrincipal := LoanAmount / RepayPeriod;
                    LInterest := 0;
                end else begin
                ...
                RSchedule."Loan Balance" := LBalance;
                RSchedule.Insert;
                //Message('here%1',LInterest + LPrincipal);
                LBalance := LBalance - LPrincipal;
                WhichDay := Date2dwy(RSchedule."Repayment Date", 1);
            until LBalance < 1
            ...
            if ObjLoans.Find('-') then begin
                ...
                if RunningBalance > 0 then begin
                    ...
                    if ObjVendor.FindSet then begin
                        ...
                        GenJournalLine."account type"::Vendor, VarLoanSettlementAcc, VarPostingDate, AmountToDeduct, '', '',
                        ...
                        GenJournalLine."account type"::Vendor, ObjLoans."Loan Debt Collector", VarPostingDate, AmountToDeduct * -1, '', VarLoanNoRecovered,
                        'Debt Collection Charge + VAT from ' + VarLoanNoRecovered + ObjLoans."Client Name", VarLoanNoRecovered, GenJournalLine."recovery transaction type"::"Guarantor Recoverd", VarLoanNoRecovered);
                    end;
                ...
                if RunningBalance > 0 then begin
                    ...
                    if ObjVendor.FindSet then begin
                        ...
                        if VarCurrMonthDepositArrears > 50 then begin
                            ...
                            GenJournalLine."account type"::Vendor, VarAccountNo, VarPostingDate, VarActualPenalty, '', '',
                            'BOSA Deposits Penalty', '', GenJournalLine."application source"::" ");

                            LineNo := LineNo + 10000;
                            FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                            GenJournalLine."account type"::"G/L Account", ObjGensetup."Penalty On Deposit Arrears A/C", VarPostingDate, VarActualPenalty * -1, '', '',
                            ...
                            LineNo := LineNo + 10000;
                            FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
                            GenJournalLine."account type"::Vendor, VarAccountNo, VarPostingDate, VarActualTax, '', '',
                            'Tax: BOSA Deposits Penalty', '', GenJournalLine."application source"::" ");

                            LineNo := LineNo + 10000;
                            FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                            GenJournalLine."account type"::"G/L Account", ObjGensetup."Excise Duty Account", VarPostingDate, VarActualTax * -1, '', '',
            ...
            if ObjAccount.FindSet then begin
                repeat
                    ...
                    if (VarRunningBal > 0) and (AvailableBal >= 20) then begin
                        ...
                        LineNo := LineNo + 10000;
                        FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
                        GenJournalLine."account type"::Vendor, VarAccountNo, VarPostingDate, AmountToDeduct * -1, '', '',
                        'BOSA Deposits Transfer ' + Format(VarPostingDate, 0, '<Month Text,3> <Year4>') + ' from ' + ObjAccount."No.", '', GenJournalLine."application source"::" ");

                        LineNo := LineNo + 10000;
                        FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                        GenJournalLine."account type"::Vendor, ObjAccount."No.", VarPostingDate, AmountToDeduct, '', '',
            ...
            IF "Current Shares">VarBenfundVariance THEN
              BEGIN
                ...
                VarAmountPosted:="Current Shares";
                    ...
                    GenJournalLine."Account Type"::Vendor,"Deposits Account No",WORKDATE,VarAmountPosted,'','',
                    ...
                    GenJournalLine."Account Type"::Vendor,"Benevolent Fund No",WORKDATE,VarAmountPosted*-1,'','',
                  ...
                  //==========================================================================================================3. DEBIT BENFUND A/C
                    LineNo:=LineNo+10000;
                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE,BATCH_NAME,DOCUMENT_NO,LineNo,GenJournalLine."Transaction Type"::"Benevolent Fund",
                    GenJournalLine."Account Type"::Vendor,"Benevolent Fund No",WORKDATE,VarAmountPosted,'','',
                    ...
                    LineNo:=LineNo+10000;
                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE,BATCH_NAME,DOCUMENT_NO,LineNo,GenJournalLine."Transaction Type"::"Benevolent Fund",
                    GenJournalLine."Account Type"::"G/L Account",ObjGensetup."Benevolent Fund Account",WORKDATE,VarAmountPosted*-1,'','',
                       ...
                       (ObjMemberAccount."Transaction Alerts" = ObjMemberAccount."transaction alerts"::"All Transactions") then begin
                        ...
                        if ObjAccountLedger.FindSet then begin
                            repeat
                                ...
                                GenJournalLine."account type"::Vendor, ObjAccountLedger."Vendor No.", WorkDate, ObjSaccoGensetup."SMS Alert Fees", '', '',
                                'Credit Transaction Alert Fee', '', GenJournalLine."application source"::CBS);

                                //============================================================================================2. Credit SMS G/L Account
                                LineNo := LineNo + 10000;
                                FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                                GenJournalLine."account type"::"G/L Account", ObjSaccoGensetup."SMS Alert Fee Account", WorkDate, ObjSaccoGensetup."SMS Alert Fees" * -1, '', '',
                                ...
                                LineNo := LineNo + 10000;
                                FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                                GenJournalLine."account type"::Vendor, ObjAccountLedger."Vendor No.", WorkDate, (ObjSaccoGensetup."SMS Alert Fees" * ObjSaccoGensetup."Excise Duty(%)" / 100), '', '',
                                'Tax: Credit Transaction Alert Fee', '', GenJournalLine."application source"::CBS);

                                //===========================================================================================4. Credit SMS TAX G/L Account
                                LineNo := LineNo + 10000;
                                FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                                GenJournalLine."account type"::"G/L Account", ObjSaccoGensetup."SMS Alert Fee Account", WorkDate, (ObjSaccoGensetup."SMS Alert Fees" * ObjSaccoGensetup."Excise Duty(%)" / 100) * -1, '', '',
        ...
        if ObjSweepingInstructions.FindSet then begin
            ...
            if ObjSweepingInstructions."Check Minimum Threshold" = true then begin
                if (VarMonitorAvailableBalance < ObjSweepingInstructions."Minimum Account Threshold") and (VarInvestmentAvailableBalance > 0) then begin
                    ...
                    LineNo := LineNo + 10000;
                    FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, ObjSweepingInstructions."Investment Account", WorkDate, VarAmounttoTransferMin, '', '',
                    ...
                    LineNo := LineNo + 10000;
                    FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, ObjSweepingInstructions."Monitor Account", WorkDate, VarAmounttoTransferMin * -1, '', '',
            ...
            if ObjSweepingInstructions."Check Maximum Threshold" = true then begin
                ...
                if (VarMonitorAvailableBalance > ObjSweepingInstructions."Maximum Account Threshold") then begin
                    ...
                    LineNo := LineNo + 10000;
                    FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, ObjSweepingInstructions."Monitor Account", WorkDate, VarAmounttoTransferMax, '', '',
                    ...
                    LineNo := LineNo + 10000;
                    FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, ObjSweepingInstructions."Investment Account", WorkDate, VarAmounttoTransferMax * -1, '', '',
    ...
    begin
        ...
        if ObjAccount.FindSet then begin
            ...
            if ObjAccountType.Get(ObjAccount."Account Type") then begin
                ...
                LineNo := LineNo + 10000;
                FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::"G/L Account", ObjAccountType."Interest Expense Account", VarPostingDate, VarInterestEarned, '', '',
                'Fixed Deposit Interest Earned for ' + ObjAccount."No.", '', GenJournalLine."application source"::CBS);

                //============================================================================================2. Credit Payable GL Account
                LineNo := LineNo + 10000;
                FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::"G/L Account", ObjAccountType."Interest Payable Account", VarPostingDate, VarInterestEarned * -1, '', '',
                'Fixed Deposit Interest Earned for ' + ObjAccount."No.", '', GenJournalLine."application source"::CBS);
    ...
    begin
        ...
        if ObjAccount.FindSet then begin
            ...
            if ObjAccountType.Get(ObjAccount."Account Type") then begin
                ...
                '', '', 'Fixed Deposit Interest Transfer to ' + ObjAccount."No.", '', GenJournalLine."application source"::CBS);
                ...
                '', '', 'Fixed Deposit Interest Earned', '', GenJournalLine."application source"::CBS);

                //============================================================================================3. Tax: Debit Member FD Account
                LineNo := LineNo + 10000;
                FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::Vendor, ObjAccount."No.", VarProcessDate, ObjAccount."Untranfered Interest" * (ObjGenSetup."Withholding Tax (%)" / 100), '', '',
                ...
                ObjAccount."Untranfered Interest" * (ObjGenSetup."Withholding Tax (%)" / 100) * -1, '', '',
                ...
                if ObjAccount."On Term Deposit Maturity" = ObjAccount."on term deposit maturity"::"Pay to FOSA Account_ Deposit+Interest" then begin
                    ...
                    LineNo := LineNo + 10000;
                    FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, ObjAccount."No.", VarProcessDate, ObjAccount.Balance, '', '',
                    'Fixed Deposit Transfer to ' + VarFDFOSAAccount, '', GenJournalLine."application source"::CBS);

                    //=====================================================================2. Credit Member FOSA Account
                    LineNo := LineNo + 10000;
                    FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, VarFDFOSAAccount, VarProcessDate, ObjAccount.Balance * -1, '', '',
                ...
                if ObjAccount."On Term Deposit Maturity" = ObjAccount."on term deposit maturity"::"Roll Back Deposit Only" then begin
                    ...
                    LineNo := LineNo + 10000;
                    FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, ObjAccount."No.", VarProcessDate, VarUntransferedInterest, '', '',
                    'Fixed Deposit Interest Transfer to ' + VarFDFOSAAccount, '', GenJournalLine."application source"::CBS);

                    //==================================================================2. Credit Member FOSA Account
                    LineNo := LineNo + 10000;
                    FnCreateGnlJournalLine(JournalTemplate, JournalBatch, DocNo, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, VarFDFOSAAccount, VarProcessDate, VarUntransferedInterest * -1, '', '',
    ...
    begin
        ...
        VarDocumentNo := 'Test Salaries';//NoSeriesMgt.GetNextNo(ObjNoSeries."Transaction Document No", 0D, true);
        exit(VarDocumentNo);
        // end;
    end;
                ...
                repeat
                    ...
                    if VarBalanceinFOSA > 0 then begin
                        ...
                        if ObjBOSARefundLedger."Account No Recovered" <> VarFOSAAccountNo then begin
                            ...
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                            GenJournalLine."account type"::Vendor, VarFOSAAccountNo, WorkDate, VarAmounttoDeduct, '', EXTERNAL_DOC_NO,
                            ...
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
                            GenJournalLine."account type"::Vendor, ObjBOSARefundLedger."Account No Recovered", WorkDate, VarAmounttoDeduct * -1, '', EXTERNAL_DOC_NO,
    ...
    begin
        ...
        if ObjLoan.FindSet then begin
            ...
            if ObjLoanCollateralDetails.FindSet then begin
                ...
                ObjCollateralRegister.SetRange(ObjCollateralRegister."Document No", ObjLoanCollateralDetails."Collateral Register Doc");
    ...
    procedure FnGenerateLoanRepaymentScheduleZero(VarLoanNo: Code[30])
    var
        LoansRec: Record "Loans Register";
        RSchedule: Record "Loan Repayment Schedule";
        detCust: Record "Detailed Cust. Ledg. Entry";
        ...
        if LoansRec.Find('-') then begin
            if ((LoansRec."Repayment Start Date" <> 0D)) then begin
                ...
                repeat
                    ...
                    RSchedule."Loan Balance" := LBalance;
                    RSchedule.Insert;
                    //Message('Ins%1',InstalNo);
                    LBalance := LBalance - LPrincipal;
                    WhichDay := Date2dwy(RSchedule."Repayment Date", 1);
                until LBalance < 1;

                LoansRec.Repayment := LPrincipal;
                LoansRec."Loan Principle Repayment" := LPrincipal;
                LoansRec."Loan Interest Repayment" := 0;
                LoansRec.Modify;
            end;
        end;
    ...
    begin
        ...
        if ObjStandingOrders.FindSet then begin
            if (ObjStandingOrders."Next Run Date" = VarStandingRunDate) or (ObjStandingOrders."Next Attempt Date" = VarStandingRunDate) then begin
                ...
                if AvailableBal >= ObjStandingOrders.Amount then begin
                    ...
                    FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, ObjStandingOrders."Source Account No.", Today, ObjStandingOrders.Amount, '', '',
                    ...
                    IF ObjStandingOrders."Destination Account Type"=ObjStandingOrders."Destination Account Type"::"Other Banks Account" THEN
                      BEGIN
                        ...
                        ObjReceiptAllocations.SETFILTER(ObjReceiptAllocations."Transaction Type",'<>%1',ObjReceiptAllocations."Transaction Type"::Repayment);
                            ...
                            REPEAT
                              //==============================================================================1. CREDIT DESTINATION  A/C
                               ...
                               FnCreateGnlJournalLine(BATCH_TEMPLATE,BATCH_NAME,DOCUMENT_NO,LineNo,ObjReceiptAllocations."Transaction Type",
                                ObjReceiptAllocations."Account Type",ObjReceiptAllocations."Member No",TODAY,ObjReceiptAllocations.Amount*-1,'','',
                                 ...
                                 BEGIN
                                   REPEAT
                                    ...
                                    //===========================================================================1.1. MONTHLY INTEREST  A/C
                                      ...
                                      ObjReceiptAllocations."Account Type",ObjReceiptAllocations."Member No",TODAY,VarScheduleInterest*-1,'','',
                                      ObjStandingOrders."Standing Order Description",ObjReceiptAllocations."Loan No.",GenJournalLine."Application Source"::CBS);
                                    ...
                                    //------------------------------------1.2 CREDIT MONTHLY INSURANCE  A/C---------------------------------------------------------------------------------------------
                                      ...
                                      ObjReceiptAllocations."Account Type",ObjReceiptAllocations."Member No",TODAY,VarScheduleInsurance*-1,'','',
                                      ObjStandingOrders."Standing Order Description",ObjReceiptAllocations."Loan No.",GenJournalLine."Application Source"::CBS);
                                    ...
                                    //------------------------------------1. CREDIT PRINCIPLE  A/C---------------------------------------------------------------------------------------------
                                      LineNo:=FnRunGetNextJvLineNo(BATCH_TEMPLATE,BATCH_NAME);
                                      FnCreateGnlJournalLine(BATCH_TEMPLATE,BATCH_NAME,DOCUMENT_NO,LineNo,ObjReceiptAllocations."Transaction Type",
                                      ObjReceiptAllocations."Account Type",ObjReceiptAllocations."Member No",TODAY,(ObjReceiptAllocations.Amount-(VarScheduleInterest+VarScheduleInsurance))*-1,'','',
                                      ObjStandingOrders."Standing Order Description",ObjReceiptAllocations."Loan No.",GenJournalLine."Application Source"::CBS);
                    ...
                    if ObjStandingOrders."Destination Account Type" = ObjStandingOrders."destination account type"::"Member Account" then begin
                        ...
                        LineNo := FnRunGetNextJvLineNo(BATCH_TEMPLATE, BATCH_NAME);
                        FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                        GenJournalLine."account type"::Vendor, ObjStandingOrders."Destination Account No.", Today, ObjStandingOrders.Amount * -1, '', '',
                    ...
                    if ObjStandingOrders."Destination Account Type" = ObjStandingOrders."destination account type"::"Other Banks Account" then begin
                        ...
                        LineNo := FnRunGetNextJvLineNo(BATCH_TEMPLATE, BATCH_NAME);
                        FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                        GenJournalLine."account type"::"Bank Account", ObjGenSetUp."Standing Order Bank", Today, ObjStandingOrders.Amount * -1, '', '',
                        ObjStandingOrders."Destination Account Narrations", '', GenJournalLine."application source"::CBS);
                    end;
                    ...
                    VarStoFeeAccount, VarStoFeeSuccess, '', '');
                    ...
                    FnCreateGnlJournalLineBalanced(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, ObjStandingOrders."Source Account No.", Today, ObjStandingOrders."Standing Order Description", GenJournalLine."bal. account type"::"G/L Account",
                    ObjGenSetUp."Excise Duty Account", VarStoFeeSuccess * (ObjGenSetUp."Excise Duty(%)" / 100), '', '');
        ...
        if ObjStandingOrders.FindSet then begin
            if (ObjStandingOrders."Next Run Date" = VarStandingRunDate) or (ObjStandingOrders."Next Attempt Date" = VarStandingRunDate) then begin
                ...
                VarSourceAccountType, ObjStandingOrders."Source Account No.", Today, ObjStandingOrders.Amount, '', '',
                ...
                FnCreateGnlJournalLineBranch(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                VarDestinationAccountType, ObjStandingOrders."Destination Account No.", Today, ObjStandingOrders.Amount * -1, '', '',
                    ...
                    repeat
                        ...
                        or ((ObjAccountsII."Overdraft Sweeping Source" = ObjAccountsII."overdraft sweeping source"::"Specific FOSA Account") and
                            ...
                            if (VarAvailableOtherFOSAAccounts > 0) and (VarODBalance < 0) then begin
                                ...
                                LineNo := LineNo + 10000;
                                FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                                GenJournalLine."account type"::Vendor, ObjAccounts."No.", WorkDate, VarAmountDeducted, '', '',
                                'Overdraft Sweeping to - ' + ObjAccountsII."No.", '', GenJournalLine."application source"::CBS);

                                //------------------------------------2. Credit OD Account---------------------------------------------------------------------------------------------
                                LineNo := LineNo + 10000;
                                FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                                GenJournalLine."account type"::Vendor, ObjAccountsII."No.", WorkDate, VarAmountDeducted * -1, '', '',
                ...
                if ObjAccounts.FindSet then begin
                    repeat
                        ...
                        if VarAvailableOtherFOSAAccounts < 0 then begin
                            ...
                            LineNo := LineNo + 10000;
                            FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                            GenJournalLine."account type"::Vendor, ObjAccountsII."No.", WorkDate, VarAvailableOtherFOSAAccounts * -1, '', '',
                            'Overdraft Sweeping to - ' + ObjAccounts."No.", '', GenJournalLine."application source"::CBS);

                            //------------------------------------2. Credit FOSA Account---------------------------------------------------------------------------------------------
                            LineNo := LineNo + 10000;
                            FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                            GenJournalLine."account type"::Vendor, ObjAccounts."No.", WorkDate, VarAvailableOtherFOSAAccounts, '', '',
        ...
        if ObjUnPaidStoCharge.FindSet then begin
            repeat
                ...
                if VarBalanceinFOSA >= ObjUnPaidStoCharge."Amount Charged" then begin
                    ...
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, ObjUnPaidStoCharge."Account No", WorkDate, VarChargeAmount, '', EXTERNAL_DOC_NO,
                    ...
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                    GenJournalLine."account type"::Vendor, ObjUnPaidStoCharge."Account No", WorkDate, VarChargeTax, '', EXTERNAL_DOC_NO,
                    ...
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
                    GenJournalLine."account type"::"G/L Account", VarChargeAccount, WorkDate, VarChargeAmount * -1, '', EXTERNAL_DOC_NO,
                    ...
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
                    GenJournalLine."account type"::"G/L Account", ObjGenSetUp."Excise Duty Account", WorkDate, VarChargeTax * -1, '', EXTERNAL_DOC_NO,
                ...
                if ObjMember.FindSet then begin
                    ...
                    if ObjScheduledStatement."Output Format" = ObjScheduledStatement."output format"::EXCEL then begin
                        ...
                        if ObjScheduledStatement."Output Format" = ObjScheduledStatement."output format"::Pdf then begin
                            ...
                            Report.SaveAs(Report::"Member Account Statement(App)", '', ReportFormat::Pdf, Outsr, RecRef);
        ...
        if ObjAccount.FindSet then begin
            repeat
                if ObjAccountType.Get(ObjAccount."Account Type") then begin
                    ...
                    ObjAccount."Untranfered Interest", '', '',
                    ...
                    ObjAccount."Untranfered Interest" * -1, '', '',
                    ...
                    (ObjAccount."Untranfered Interest" * ObjGenSetUp."Withholding Tax (%)" / 100), '', '',
        ...
        if ObjMember.FindSet then begin
            ...
            if ObjNoSeries.FindSet then begin
                ...
                if VarDocumentNo <> '' then begin
                    ...
                    ObjAccountII."Global Dimension 1 Code" := '';
                    ...
                    if ObjLoans.FindSet then begin
                        ...
                        if VarLoansCount = 1 then begin
                            ...
                            if ObjRepamentSchedule.FindSet then begin
                                ...
                                LineNo := LineNo + 10000;
                                FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                                GenJournalLine."account type"::Vendor, ObjAccounts."No.", WorkDate, VarAmounttoDeduct, '', '',
                                ...
                                LineNo := LineNo + 10000;
                                FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::Repayment,
                                GenJournalLine."account type"::Member, VarMemberNo, WorkDate, VarAmounttoDeduct * -1, '', '',
            ...
            if ObjLoans.Find('-') then begin
                ...
                if ObjVendor.FindSet then begin
                    ...
                    if RunningBalance > 0 then begin
                        ...
                        LineNo := LineNo + 10000;
                        FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                        GenJournalLine."account type"::Vendor, VarFOSAAccount, WorkDate, AmountToDeduct, '', '',
                        'Debt Collection Charge + VAT from ' + VarLoanNo + ObjLoans."Client Name", '', GenJournalLine."application source"::CBS);

                        LineNo := LineNo + 10000;
                        FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                        GenJournalLine."account type"::Vendor, ObjLoans."Loan Debt Collector", WorkDate, AmountToDeduct * -1, '', VarLoanNo,
    ...
    begin
        ...
        Accounts."Date of Birth":="Date of Birth";
        Accounts."Global Dimension 1 Code":='';
            ...
            repeat
                ...
                if VarContributionsTotal > 2 then begin
                    ...
                    if ObjAccounts.FindFirst then begin
                        ...
                        LineNo := LineNo + 10000;
                        FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                        GenJournalLine."account type"::Vendor, ObjAccounts."No.", WorkDate, ObjGenSetup."Recruitment Commission" * -1, '', EXTERNAL_DOC_NO,
                        'Member Recruitment Comm. - ' + Format(WorkDate, 0, '<Month Text,3> <Year4>'), '', GenJournalLine."recovery transaction type"::"Guarantor Recoverd");

                        LineNo := LineNo + 10000;
                        FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                        GenJournalLine."account type"::"G/L Account", ObjGenSetup."Recruitment Comm. Expense GL", WorkDate, ObjGenSetup."Recruitment Commission", '', EXTERNAL_DOC_NO,
    ...
    begin
        ...
        if ObjAccount.FindSet then begin
            ...
            if ObjAccountII.Get(ObjAccount."OD Debt Collector") then begin
                ...
                LineNo := LineNo + 10000;
                FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::Vendor, VarAccountNo, WorkDate, VarDebtCollectorFee, '', EXTERNAL_DOC_NO,
                'Debt Collector Fee on OD', '', GenJournalLine."recovery transaction type"::"Guarantor Paid", '');

                //======================================================================================================Recovery to Debt Collector
                LineNo := LineNo + 10000;
                FnCreateGnlJournalLineGuarantorRecovery(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::Vendor, ObjAccount."OD Debt Collector", WorkDate, VarDebtCollectorFee * -1, '', EXTERNAL_DOC_NO,
                ...
                if ObjMember."Member Last Transaction Date" <> 0D then begin
                    ...
                    if (ObjMember."Member Last Transaction Date" = 0D) and (ObjMember."Registration Date" <> 0D) then begin
                        ...
                        if VarDormancyBaseDate < VarActionDate then begin
                            ...
                            ObjMember.Status := ObjMember.Status::Active;
    ...
    begin
        ...
        if ObjMembers.FindSet then begin
            ...
            ObjMemberAccount.SetRange(ObjMemberAccount."Global Dimension 1 Code", '');
            if ObjMemberAccount.FindSet then begin
                repeat
        ...
        if ObjAccount.Get(VarPiggyBankAccount) then begin
            ...
            if ObjAccountType.FindSet then begin
                ...
                if (VarExistingPiggyBank = true) or (ObjAccountType."Default Piggy Bank Issuance" = false) then begin
                    ...
                    LineNo := LineNo + 10000;
                    FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ", GenJournalLine."account type"::Vendor,
                    ObjAccount."No.", WorkDate, VarPiggyBankFee, '', DOCUMENT_NO, VarTransactionNarrationDebit, '', GenJournalLine."application source"::" ");

                    LineNo := LineNo + 10000;
                    FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ", GenJournalLine."account type"::Vendor,
                   ObjAccount."No.", WorkDate, VarTaxAmount, '', DOCUMENT_NO, 'Tax: Piggy Bank Fees', '', GenJournalLine."application source"::" ");
                    ...
                    ObjAccountType."Piggy Bank Fee Account", WorkDate, VarPiggyBankFee * -1, '', DOCUMENT_NO, VarTransactionNarrationCredit, '', GenJournalLine."application source"::" ");

                    //=======================================================================================Credit Tax:ATM Card Fee G/L Account
                    LineNo := LineNo + 10000;
                    FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ", GenJournalLine."account type"::"G/L Account",
                    ObjGenSetUp."Excise Duty Account", WorkDate, VarTaxAmount * -1, '', DOCUMENT_NO, 'Tax: Piggy Bank Fees - ' + ObjAccount."No.", '', GenJournalLine."application source"::" ");
                    ...
                    if (ObjAccountType."Default Piggy Bank Issuance" = true) and (VarExistingPiggyBank = false) then begin
                        ...
                        LineNo := LineNo + 10000;
                        FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ", GenJournalLine."account type"::"G/L Account",
                        ObjGenSetUp."New Piggy Bank Debit G/L", WorkDate, VarPiggyBankFee, '', DOCUMENT_NO, 'Piggy Bank Issue to ' + ObjAccount."No." + ' ' + ObjAccount.Name, '',
                        ...
                        LineNo := LineNo + 10000;
                        FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ", GenJournalLine."account type"::"G/L Account",
                        ObjGenSetUp."New Piggy Bank Credit G/L", WorkDate, VarPiggyBankFee * -1, '', DOCUMENT_NO, 'Piggy Bank Issue to ' + ObjAccount."No." + ' ' + ObjAccount.Name, '',
                        GenJournalLine."application source"::" ");
                    end;
    ...
    procedure LoanPerformance(ClientCount: Code[40]; AsAt: Date) LoanCount: Integer;
    var
    //myInt: Integer;
    begin
        ObjLoans.reset;
        ObjLoans.setrange("Client Code", ClientCount);
        ObjLoans.SetFilter("Loans Category", '%1|%2', ObjLoans."Loans Category"::Doubtful, ObjLoans."Loans Category"::Loss);
        if ObjLoans.FindSet() then begin
            repeat
    ...
    procedure LoanUnderLossBlance(ClientCount: Code[40]; AsAt: Date) LoanBalance: Decimal;
    var
    //myInt: Integer;
    begin
        ObjLoans.reset;
        ...
        if ObjLoans.FindSet() then begin
            repeat
                LoanBalance += ObjLoans."Outstanding Balance";
            until ObjLoans.Next() = 0;
        end;
        exit(LoanBalance);
    end;

    procedure FnGenerateLoanSchedule(LoanNumber: Code[60]);
    var
        RSchedule: Record "Loan Repayment Schedule";
        LInterest: Decimal;
        LPrincipal: Decimal;
        ...
        IF Loans.FINDFIRST THEN BEGIN
            ...
            ELSE
                ...
                ELSE
                    ...
                    ELSE
                        IF Loans."Repayment Frequency" = Loans."Repayment Frequency"::Quaterly THEN
    ...
    local procedure FnGetStartDate(DisbursementDate: Date)
    var
    ...
    procedure FnCalculateAvailableBalance(FosaAccount: Code[40]) Balance: Decimal
    var
    ...
    begin
        Vendor.Reset();
        Vendor.SetRange(Vendor."No.", FosaAccount);
        if Vendor.FindFirst() then begin

            MinBalance := 0;
            if Vendor."Account Type" = '103' then
                MinBalance := 1000
            else
                MinBalance := 0;
            ...
            AvailableBalance := 0;
            AvailableBalance := Vendor.Balance - ((Vendor."Uncleared Cheques" - Vendor."Cheque Discounted Amount") + PendingAmount + Vendor."ATM Transactions" + Vendor."Flagged Transactions" + Vendor."EFT Transactions" + MinBalance + Vendor."Mobile Transactions"/* + Vendor."Amount to freeze"*/);
            //+Vendor."Flagged Transactions"
    ...
    procedure FnCalculateAvailableBalanceKit(FosaAccount: Code[40]) Balance: Decimal
    var
        MBuffer: Record "Mpesa Withdawal Buffer";
        AvailableBalance: Decimal;
        PendingAmount: Decimal;
    ...
    begin
        ...
        if Vendor.FindFirst() then begin
            ...
            AvailableBalance := 0;
            AvailableBalance := Vendor.Balance - ((Vendor."Uncleared Cheques" - Vendor."Cheque Discounted Amount") + PendingAmount + Vendor."ATM Transactions" + Vendor."EFT Transactions" + MinBalance + Vendor."Mobile Transactions" + Vendor."Amount to freeze");
            //+Vendor."Flagged Transactions"
            if AvailableBalance > 0 then
                Balance := AvailableBalance
            else
    ...
    procedure FnCreateGnlJournalLines(TemplateName: Text; BatchName: Text; DocumentNo: Code[30]; LineNo: Integer; TransactionType: Option " ","Registration Fee","Share Capital","Interest Paid","Loan Repayment","Deposit Contribution","Insurance Contribution","Benevolent Fund",Loan,"Unallocated Funds",Dividend,"FOSA Account"; AccountType: enum "Gen. Journal Account Type"; AccountNo: Code[50]; TransactionDate: Date; TransactionAmount: Decimal; DimensionActivity: Code[40]; TransactionDescription: Text; LoanNumber: Code[50])
    var
        GenJournalLine: Record "Gen. Journal Line";
    begin
        GenJournalLine.INIT;
    ...
    procedure FnInsertSalarydata(DocumentNo: code[80]; MemberNo: Code[80]; VendorNo: Code[80]; Description: Text[250]; SalAmount: Decimal; ExpectedAmount: Decimal)
    var
        CheckoffData: Record "Salary Deductions Data";
        LastNo: Decimal;
        Members: Record Customer;
        Name: Text[200];
    begin
        LastNo := 0;
        if CheckoffData.FindLast() then
            LastNo := CheckoffData."Entry No";
        Members.reset;
        ...
        CheckoffData.Init();
        CheckoffData."Entry No" := LastNo + 1;
        CheckoffData."Member No" := MemberNo;
        CheckoffData."Document No" := DocumentNo;
        CheckoffData.name := Name;
        CheckoffData."Vendor No" := VendorNo;
        CheckoffData.Description := Description;
        CheckoffData.Amount := SalAmount;
        CheckoffData."Expected Amount" := ExpectedAmount;
        CheckoffData.Insert(true);
    end;


    procedure ClearCheckoffData(DocumentNo: code[80])
    var
        CheckData: Record "Salary Deductions Data";
    begin
        CheckData.Reset();
        CheckData.SetRange(CheckData."Document No", DocumentNo);
        if CheckData.findset then begin
            Checkdata.DeleteAll(true);
        end;
    end;



    procedure FnGetAccountNumber(AccountType: Code[40]; BOSAAccount: Code[40]) AccountNumber: Code[80]
    var
        Vendor: Record Vendor;

    begin
        Vendor.Reset();
        Vendor.SetRange(Vendor."BOSA Account No", BOSAAccount);
    ...
    procedure FnCheckMembershipRequirements(BOSAAccount: Code[40]; LoanType: Code[40])
    var
        Vendor: Record Vendor;
    ...
    begin
        SaccoSetup.get();
        loanProduct.Reset();
        loanProduct.SetRange(loanProduct.Code, LoanType);
        loanProduct.SetRange(loanProduct."Ignore ShareCapital", false);
        if loanProduct.FindFirst() then begin
            Vendor.Reset();
            Vendor.SetRange(Vendor."BOSA Account No", BOSAAccount);
            Vendor.SetRange(Vendor."Account Type", '002');
            if Vendor.FindFirst() then begin
                Vendor.CalcFields(Vendor.Balance);
                Customer.get(Vendor."BOSA Account No");
                if Customer."Account Category" = Customer."Account Category"::Individual then begin
                    if Vendor.Balance < SaccoSetup."Retained Shares" then begin
                        Message('Member has not met minimum shares.');
                    end;
                end;
                if Customer."Account Category" = Customer."Account Category"::Corporate then begin
                    if Vendor.Balance < SaccoSetup."Retained Shares" then begin
                        Message('Member has not met minimum shares.');
                    end;
                end;
        ...
        Customer.Reset();
        Customer.SetRange(Customer."No.", BOSAAccount);
        if Customer.FindFirst() then begin
            if Customer.Status <> Customer.Status::Active then
                Error('Member Account should be Active.');
        end;
    ...
    begin
        ...
        IF Loans.FindFirst() then BEGIN
            ...
            Vendor.Reset();
            Vendor.SetRange(Vendor."BOSA Account No", Loans."Client Code");
            Vendor.SetFilter(Vendor."Account Type", '103');
            if Vendor.FindFirst() then begin
                VBalance := FnCalculateAvailableBalance(Vendor."No.");
    ...
    procedure CheckNumberOfTransactions(AccountNo: Code[40]) NotOkay: Boolean
    var
        MpesaBuffer: Record "Mpesa Withdawal Buffer";
        // mpesaTrans: Record mpes
    ...
    begin
        MpesaBuffer.Reset();
        MpesaBuffer.SetRange(MpesaBuffer."Vendor No", AccountNo);
        MpesaBuffer.SetRange(MpesaBuffer."Transaction Date", Today);
        MpesaBuffer.SetRange(Posted, true);
        MpesaBuffer.SetRange(Reversed, false);
        if MpesaBuffer.FindSet then begin
            NoOfTransactions := MpesaBuffer.Count;
            // Error('Transactions%1',NoOfTransactions);
            NotOkay := false;
            if NoOfTransactions > 3 then begin
                NotOkay := true;
                usersetups.Reset();
                UserSetups.SetRange(UserSetups."Send Red Flagged SMS", true);
                if UserSetups.FindSet() then begin
                    repeat
                        SMSFlag := 'Account ' + AccountNo + ' has exceeded the number of times to transact today.';
                        smsManagement.SendSmsWithID(Source::MBANKING, UserSetups."Phone No.", SMSFlag, AccountNo, AccountNo, TRUE, 200, TRUE, 'CBS', CreateGuid(), 'CBS');
    ...
    procedure GetAccountBalanceBtnTwoDatesShares(AccountNo: Code[80]; FromDate: Date; ToDate: Date) Balance: Decimal
    var
        DetEntry: Record "Detailed Vendor Ledg. Entry";
    begin
        balance := 0;
        DetEntry.Reset();
        DetEntry.SetRange(DetEntry."Vendor No.", AccountNo);
        DetEntry.SetFilter(DetEntry."Posting Date", '..%1', CalcDate('CM', ToDate));
        DetEntry.SetFilter(DetEntry."Document No.", '<>%1', 'DEPOSBBF310324');
        if DetEntry.FindSet() then begin
            repeat

                Balance := Balance + DetEntry.Amount;
            until DetEntry.Next() = 0;
        end;
        Balance := -Balance;
    end;

    procedure GetAccountBalanceBtnTwoDatesSharess(AccountNo: Code[80]; FromDate: Date; ToDate: Date) Balance: Decimal
    var
        DetEntry: Record "Detailed Vendor Ledg. Entry";
    begin
        balance := 0;
    ...
    procedure GetAccountBalanceBtnTwoDates(AccountNo: Code[80]; FromDate: Date; ToDate: Date) Balance: Decimal
    var
        DetEntry: Record "Detailed Vendor Ledg. Entry";
    begin
        balance := 0;
        DetEntry.Reset();
        DetEntry.SetRange(DetEntry."Vendor No.", AccountNo);
        DetEntry.SetFilter(DetEntry."Posting Date", '%1..%2', CalcDate('-CM', ToDate), CalcDate('CM', ToDate));
        DetEntry.SetFilter(DetEntry."Document No.", '<>%1', 'DEPOSBBF310324');
        if DetEntry.FindSet() then begin
            repeat
    ...
    procedure GetAccountBalanceBtnTwoDatesOld(AccountNo: Code[80]; FromDate: Date; ToDate: Date) Balance: Decimal
    var
        DetEntrys: Record "Member Ledger Entries";
    begin
        balance := 0;
        DetEntrys.Reset();
        DetEntrys.SetRange(DetEntrys."Customer No.", AccountNo);
        DetEntrys.SetFilter(DetEntrys."Posting Date", '%1..%2', CalcDate('-CM', ToDate), CalcDate('CM', ToDate));
        // DetEntrys.SetRange(DetEntrys."Transaction Type", DetEntrys."Transaction Type"::"Deposit Contribution");
        DetEntrys.SetRange(DetEntrys."Transaction Type", DetEntrys."Transaction Type"::"Deposit Contribution");
        if DetEntrys.FindSet() then begin
            //repeat
            DetEntrys.CalcSums(DetEntrys.Amount);
            Balance := DetEntrys.Amount;
            // until DetEntry.Next() = 0;
        end;
        Balance := -Balance;
    end;

    procedure GetAccountBalanceBtnTwoDatesOld2023(AccountNo: Code[80]; FromDate: Date; ToDate: Date) Balance: Decimal
    var
        DetEntrys: Record "Member Ledger Entries";
    begin
        balance := 0;
        DetEntrys.Reset();
        DetEntrys.SetRange(DetEntrys."Customer No.", AccountNo);
        DetEntrys.SetFilter(DetEntrys."Posting Date", '..%1', ToDate);
        DetEntrys.SetRange(DetEntrys."Transaction Type", DetEntrys."Transaction Type"::"Deposit Contribution");
        if DetEntrys.FindSet() then begin
            //repeat
            DetEntrys.CalcSums(DetEntrys.Amount);
    ...
    procedure GetAccountBalanceBtnTwoDatesOldPCKBF2022(AccountNo: Code[80]; FromDate: Date; ToDate: Date) Balance: Decimal
    var
        DetEntrys: Record "Member Ledger Entries";
        DetEn: Record "Member Ledger Entries";
        SpecifiCBal: Decimal;
        CollDate: Date;
        Det: Record "Member Ledger Entries";
        SalFound: boolean;
    begin
        balance := 0;
        DetEn.Reset();
        DetEn.SetRange(DetEn."Customer No.", AccountNo);
        DetEn.SetFilter(DetEn."Posting Date", '..%1', 20211231D);
        DetEn.SetRange(DetEn."Transaction Type", DetEn."Transaction Type"::"Deposit Contribution");
        if DetEn.FindSet() then begin
            //repeat
            DetEn.CalcSums(DetEn.Amount);
            Balance := DetEn.Amount;
            // until DetEntry.Next() = 0;
        ...
        SalFound := false;
        Det.Reset();
        Det.SetRange(Det."Customer No.", AccountNo);
        Det.SetFilter(Det."Posting Date", '..%1', 20211231D);
        Det.SetRange(Det."Transaction Type", Det."Transaction Type"::"Deposit Contribution");
        Det.SetFilter(Det."Document No.", 'PCK SALARY*');
        if Det.FindFirst() then begin
            SalFound := true;
        end;
        if SalFound = true then begin
            DetEntrys.Reset();
            DetEntrys.SetRange(DetEntrys."Customer No.", AccountNo);
            DetEntrys.SetFilter(DetEntrys."Posting Date", '%1..%2', 20220101D, ToDate);
            DetEntrys.SetRange(DetEntrys."Transaction Type", DetEntrys."Transaction Type"::"Deposit Contribution");
            if DetEntrys.FindSet() then begin
                //repeat
                DetEntrys.CalcSums(DetEntrys.Amount);
                SpecifiCBal := DetEntrys.Amount;
                // until DetEntry.Next() = 0;
            end;
            ...
            Det.Reset();
            Det.SetRange(Det."Customer No.", AccountNo);
            Det.SetFilter(Det."Posting Date", '%1..%2', 20220101D, ToDate);
            Det.SetRange(Det."Transaction Type", Det."Transaction Type"::"Deposit Contribution");
            Det.SetFilter(Det."Document No.", 'PCK SALARY*');
            if Det.FindFirst() then begin
                CollDate := Det."Posting Date";
            end;

            if CollDate <> 0D then begin
                DetEntrys.Reset();
                DetEntrys.SetRange(DetEntrys."Customer No.", AccountNo);
                DetEntrys.SetFilter(DetEntrys."Posting Date", '%1..%2', CalcDate('-CM', CollDate), ToDate);
                DetEntrys.SetRange(DetEntrys."Transaction Type", DetEntrys."Transaction Type"::"Deposit Contribution");
                if DetEntrys.FindSet() then begin
                    //repeat
                    DetEntrys.CalcSums(DetEntrys.Amount);
        ...
        //  MESSAGE('sPECIFIC%1Balance%2Found%3', SpecifiCBal, Balance, SalFound);
        Balance := Balance + SpecifiCBal;
        Balance := -Balance;
    end;


    procedure FnCheckIfSalaryPostedOlds(AccountNo: Code[80]) Posted: Boolean
    var
        DetEntry: Record "Detailed Vendor Ledg. Entry";
        Det: Record "Member Ledger Entries";
    begin
    ...
    procedure GetAccountBalanceBtnTwoDatesOldPCKEarner(AccountNo: Code[80]; FromDate: Date; ToDate: Date) Balance: Decimal
    var
        DetEntrys: Record "Member Ledger Entries";
    begin
    ...
    procedure GetAccountBalanceBtnTwoDatesPCK(AccountNo: Code[80]; FromDate: Date; ToDate: Date) Balance: Decimal
    var
        DetEntry: Record "Detailed Vendor Ledg. Entry";
    begin
    ...
    procedure FnCheckIfSalaryPosted(AccountNo: Code[80]; FromDate: Date; ToDate: Date) Posted: Boolean
    var
        DetEntry: Record "Detailed Vendor Ledg. Entry";
    begin
        Posted := false;
        ...
        DetEntry.SetRange(DetEntry."Vendor No.", AccountNo);
        DetEntry.SetFilter(DetEntry."Posting Date", '%1..%2', CalcDate('-CM', ToDate), CalcDate('CM', ToDate));
        DetEntry.SetFilter(DetEntry."Document No.", '%1', '*SAL*');
        if DetEntry.FindFirst() then begin
            //  Message('Here%1',ToDate);
            Posted := true;
    ...
    procedure FnCheckIfSalaryPostedold(AccountNo: Code[80]; FromDate: Date; ToDate: Date) Posted: Boolean
    var
        DetEnt: Record "Member Ledger Entries";
    begin
        Posted := false;
        DetEnt.Reset();
        DetEnt.SetRange(DetEnt."Customer No.", AccountNo);
        DetEnt.SetFilter(DetEnt."Posting Date", '%1..%2', CalcDate('-CM', ToDate), CalcDate('CM', ToDate));
        DetEnt.SetFilter(DetEnt."Document No.", '%1', 'PCK SALARY*');
        if DetEnt.FindFirst() then begin
            Posted := true;
        end;

    end;

    procedure FnCheckIfSalaryIfRefund(AccountNo: Code[80]; FromDate: Date; ToDate: Date) Posted: Boolean
    var
        DetEnt: Record "Member Ledger Entries";
    begin
        Posted := false;
        ...
        DetEnt.SetRange(DetEnt."Customer No.", AccountNo);
        DetEnt.SetFilter(DetEnt."Posting Date", '%1..%2', CalcDate('-CM', ToDate), CalcDate('CM', ToDate));
        DetEnt.SetRange(DetEnt.Reversed, false);
        DetEnt.SetFilter(DetEnt.Amount, '>%1', 0);
        if DetEnt.FindFirst() then begin
            Posted := true;
        end;
    ...
    procedure FnCheckIfRefundNew(AccountNo: Code[80]; FromDate: Date; ToDate: Date) Posted: Boolean
    var
        DetEntry: Record "Detailed Vendor Ledg. Entry";
    begin
        Posted := false;
        ...
        DetEntry.SetRange(DetEntry."Vendor No.", AccountNo);
        DetEntry.SetFilter(DetEntry."Posting Date", '%1..%2', CalcDate('-CM', ToDate), CalcDate('CM', ToDate));
        DetEntry.SetFilter(DetEntry.Amount, '>%1', 0);
        if DetEntry.FindFirst() then begin
            Posted := true;
