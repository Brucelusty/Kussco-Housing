Codeunit 50088 "Mobile Banking Code"
{
    var
        ...
        MOBILELive: Codeunit CloudPESALivetest;
    ...
    BEGIN
        ...
        END;
        ObjMemberRegister.RESET;
        ObjMemberRegister.SetRange(ObjMemberRegister."No.", FnGetAccountusingPhone(username));
        IF ObjMemberRegister.FIND('-') THEN BEGIN
    ...
    BEGIN
        ...
        IF ObjHREmployees.FIND('-') THEN BEGIN
            ...
            //ObjPaybillATrans.SETRANGE(ObjPaybillATrans."Client Code",ObjMemberRegister."No.");
            //ObjPaybillATrans.SETRANGE(ObjLoansRegister.Posted,TRUE);
    ...
    BEGIN
        ...
        IF ObjMemberRegister.FIND('-') THEN BEGIN
            ...
            Res += '"bal_code":"DEP"},';
            Res += '{"account_name":"Chama Savings",';
            Res += '"balances":"0",';
            Res += '"bal_code":"RSK"}';
            Res += ']}';
        END;
        ...
        IF ObjMemberRegister.FIND('-') THEN BEGIN
            ...
            IF ObjVendor.FINDSET THEN BEGIN
                ...
                REPEAT
                    ...
                    Res += '"account_status":"' + FORMAT(ObjVendor.Status) + '",';
                    Res += '"payroll_no":"' + FORMAT(ObjVendor."Payroll/Staff No2") + '",';
                    Res += '"balance":"' + FORMAT(ObjVendor."Balance (LCY)" - AccountTypesSavingProducts."Minimum Balance") + '",';
                    Res += '"bal_code":"' + AccountTypesSavingProducts.Code + '"},';
                UNTIL ObjVendor.NEXT = 0;
                Res := COPYSTR(Res, 1, STRLEN(Res) - 1);
            END;
        ...
        IF ObjMemberRegister.FIND('-') THEN BEGIN
            ...
            IF ObjVendor.FINDSET THEN BEGIN
                ...
                IF ObjVendorLedgerEntry.FIND('-') THEN BEGIN
                    ...
                    UNTIL (ObjVendorLedgerEntry.NEXT = 0) OR (count = 10);
                    Res := COPYSTR(Res, 1, STRLEN(Res) - 1);
                end;
                Res += ']}'
            END;
    ...
    BEGIN
        ...
        ObjPaybillATrans."Account Name" := mpesaname;
    ...
    BEGIN
        BEGIN
            ...
            IF ObjMemberRegister.FIND('-') THEN BEGIN
                IF Product_code = 'M-BANKING ADVANCE' THEN
                    Res := MOBILELive.AdvanceEligibilitySalary(ObjMemberRegister."FOSA Account No.", period)
                ELSE
                    IF Product_code = 'SALARYMOBILE' THEN
    ...
    BEGIN
        BEGIN
            ...
            ObjMemberRegister.SETRANGE(ObjMemberRegister."ID No.", account_no);
            IF ObjMemberRegister.FIND('-') THEN BEGIN
                IF Product_code = 'M-EPA' THEN
                    Res := MOBILELive.PostNormalLoan(doc_no, ObjMemberRegister."No.", amount, period)
                ELSE
                    IF Product_code = 'SALARYMOBILE' THEN
                        Res := MOBILELive.PostNormalLoan(doc_no, ObjMemberRegister."No.", amount, period)
                    ELSE BEGIN
                ...
                if Res = 'TRUE' then begin
                    ...
                    Res += '"SecurityCredential":"",';
    ...
    BEGIN
        ...
        END ELSE
            IF Res = 'TRUE' THEN BEGIN
                ...
                Res += '"account_no":"' + ObjMobileUsers."Account No" + '",';
                Res += '"transactionNo":"' + doc_no + '",';
                Res += '"SecurityCredential":"",';
                Res += '"description":"Successful"}';
            END ELSE
                IF Res = 'INSUFFICIENT' THEN BEGIN
                    Res := '{"success":false,';
    ...
    BEGIN
        ObjMobileUsers.RESET;
        ObjMobileUsers.SETRANGE(ObjMobileUsers."Account No", FnGetphoneno(telephone));
        IF ObjMobileUsers.FIND('-') THEN BEGIN
    ...
    BEGIN
        ...
        IF Transactions.FIND('-') = FALSE THEN BEGIN
            ...
            IF TransactionType = 4 THEN BEGIN
                Res := FnGetMinistatementUSSD(MemberNo, accountNo);
                IF Res <> '' THEN BEGIN
            ...
            IF TransactionType = 6 THEN BEGIN
                Res := FnGetallLoansBalancesUSSD(MemberNo);
                IF Res <> '' THEN BEGIN
                    SMSMessage(sessionID, MemberNo, telephone, Res);
                END;
            END;
            ...
            IF TransactionType = 3 THEN BEGIN
                Res := FnGetaccountBalancesUSSD(MemberNo);
                IF Res <> '' THEN BEGIN
                    SMSMessage(sessionID, MemberNo, telephone, Res);
                END;
            END;
    ...
    BEGIN
        ...
        IF ObjMemberRegister.FIND('-') THEN BEGIN
            ObjMemberRegister.CALCFIELDS("Shares Retained", "Current Shares", "Total Loans Outstanding", "Outstanding Interest");
            Res += 'Fixed Deposit Ksh.' + FORMAT(ObjMemberRegister."Shares Retained");
            Res += 'Savings Ksh.' + FORMAT(ObjMemberRegister."Current Shares");
            Res += '"Loan balances Ksh.' + FORMAT(ObjMemberRegister."Total Loans Outstanding" + ObjMemberRegister."Outstanding Interest");
        END;
    END;
    ...
    begin
        ...
        //send to kra url
        IsSuccessful := Client.Get('https://quicksms.advantasms.com/api/services/sendsms/?mobile=' + mobile + '&shortcode=&message=' + smsbody + '&apikey=&partnerID=3554', Response);
    ...
    BEGIN
        ...
        ObjMemberRegister.SETRANGE(ObjMemberRegister."ID No.", member_no);
        IF ObjMemberRegister.FIND('-') THEN BEGIN
            Rst := MOBILELive.PostNormalLoan(sessionid, ObjMemberRegister."No.", amount, period);
