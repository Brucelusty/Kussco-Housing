Codeunit 50081 "PORTALIntegrationAUold"
{
    ...
    begin
        ...
        BEGIN
            ...
            IF objMember.FIND('-') THEN BEGIN
                ...
                CustomerLedgerEntries.SetRange("Transaction Type", CustomerLedgerEntries."Transaction Type"::"Deposit Contribution");
    ...
    var
        ...
        MemberStatement: Report "Member Account Statement(Ver1)";
    ...
    var
        ...
        StatementOutstream: OutStream;
        StatementInstream: InStream;
        LoanGuaranteed: Report "Loans Guaranteed Report";
        Base64Convert: Codeunit "Base64 Convert";
    begin
        objMember.Reset;
        objMember.SetRange(objMember."No.", MemberNo);
    ...
    procedure FnLoanBosaStatement(MemberNo: Code[50]; var Base64Txt: Text)
    var
        Filename: Text[100];
        TempBlob: Codeunit "Temp Blob";
        StatementOutstream: OutStream;
        StatementInstream: InStream;
        LoanStatement: Report "Loan Statement BOSA";
        Base64Convert: Codeunit "Base64 Convert";
    begin
        objMember.Reset;
        objMember.SetRange(objMember."No.", MemberNo);
    ...
    var
        ...
        StatementOutstream: OutStream;
        StatementInstream: InStream;
        LoanGuarantors: Report "Loan Statement BOSA";
        Base64Convert: Codeunit "Base64 Convert";
    begin
        objMember.Reset;
        objMember.SetRange(objMember."No.", MemberNo);
    ...
    begin
        ...
        objMember.SetRange(objMember."No.", MemberNo);
        if objMember.Find('-') then begin
            info := objMember."No." + ':' + objMember.Name + ':' + objMember."E-Mail" + ':' + Format(objMember."Membership Status") + ':' + objMember."Mobile Phone No"
             + ':' + objMember."ID No." + ':' + Format(objMember."Date Of Birth") + ':' + objMember.Pin
             + ':' + objMember."Employer Code" + ':' + objMember."Employer Name" + ':' + objMember.Designation + ':' + objMember."FOSA Account No.";
        end;
    end;

    procedure FnFosaAccounts(MemberNo: Code[20]) info: Text
    begin
        Vendor.Reset;
        Vendor.SetRange(Vendor."BOSA Account No", MemberNo);
        if Vendor.Find('-') then begin
            info := Vendor."No." + ':' + Vendor.Name + ':' + Vendor."Account Type Name";
        end;
    end;
    ...
    begin
        ...
        IF objMember.FIND('-') THEN BEGIN
            ...
            if Vendor.FindFirst() then
                objMember.CALCFIELDS("Shares Retained", "Total Committed Shares", "Current Savings");
            objMember.CALCFIELDS("Current Shares");
            objMember.CALCFIELDS("Shares Retained", "Dividend Amount", "Un-allocated Funds");
            info := FORMAT(ROUND(objMember."Current Savings" * -1, 0.01, '>')) + ':' + FORMAT(ROUND(objMember."Shares Retained", 0.01, '>')) + ':' + FORMAT(ROUND(objMember."Holiday Savings", 0.01, '>')) + ':'
            + FORMAT(ROUND(objMember."Dependant Savings 1", 0.01, '>')) + ':' + FORMAT(ROUND(objMember."Dependant Savings 2", 0.01, '>'))
            + ':' + FORMAT(ROUND(objMember."Dependant Savings 3", 0.01, '>')) + ':'
            + FORMAT(ROUND(objMember."Utafiti Housing", 0.01, '>')) + ':' + FORMAT(ROUND(objMember."Dividend Amount", 0.01, '>')) + ':' + FORMAT(ROUND(Vendor."Salary Average Amount", 0.01, '>'));
    ...
    procedure fnRegister(IDNumber: Code[15]; MemberNo: Code[10]; FTP: Integer) Registered: Boolean
    var
    ...
    begin
        ...
        if OnlineUsers.Find('-') then begin
            ...
            if OnlineUsers.Modify(true) then
                smtpc.Create(OnlineUsers."Email Address", 'Airport Sacco Member Portal Password Reset OTP', 'Your One Time Password To Access Airport Sacco portal is: ' + otp, true);
            ...
            if objMember.Find('-') then begin
                ...
                if OnlineUsers.Insert(true) then
                    smtpc.Create(objMember."E-Mail", 'Airport Sacco Member Portal OTP', 'Your one time password to access Airport Sacco portal is: ' + otp, true);
                smtp.Send(smtpc, Enum::"Email Scenario"::Default);
                Registered := true;
                exit(Registered);
            end else
                Error('You are currently not registered as member of Airport Sacco! Please contact Airport Sacco');
        end;
    end;
