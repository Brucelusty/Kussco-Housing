codeunit 50086 "Mobile Banking Engine"
{
        ...
        if SessionValid then begin
            case RequestAction of
                ...
                'get-business-details':
                    begin
                        OutputJson := GetBusinessDetails(RequestDataJson);
                    end;
                'balance-enquiry':
                    begin
                ...
                'balance-enquiry-mobile':
                    begin
                        OutputJson := BalanceEnquiryMobileApp(RequestDataJson);
                    end;
                'statement-enquiry'://internal-funds-transfer-loan
                    begin
                        OutputJson := StatementEnquiry(RequestDataJson);
                    end;

                'statement-enquiry-mapp'://internal-funds-transfer-loan
                    begin
                        OutputJson := StatementEnquiryMobile(RequestDataJson);
                    end;
                ...
                'get-specific-loan-types-details-with-dur-amount':
                    begin
                        OutputJson := GetSpecificLoanTypesDetailsWithDurAmount(RequestDataJson);
                    end;
                'get-loan-types':
                    begin
    ...
    begin
        ...
        if IdentifierType = 'MSISDN' THEN begin
            ...
            // Customer.SetRange("Mobile Banking Registered", true);
            if Customer.FindFirst() then begin
                if Customer."Mobile Banking Registered" = false then begin
                    SetResponseStatus(ResponseJson, 'error', 'Error', 'The member has not been registered for mobile banking.');
                    Found := true;
                end else begin
                    ...
                    DataJson.Add('payroll_no', Customer."Payroll No");
        ...
        if IdentifierType = 'ACCOUNT_NUMBER' THEN begin
            Customer.Reset();
            Customer.SetRange(Customer."FOSA Account No.", UpperCase(Identifier));
            // Customer.SetRange("Mobile Banking Registered", true);
            if Customer.FindFirst() then begin
                if Customer."Mobile Banking Registered" = false then begin
                    SetResponseStatus(ResponseJson, 'error', 'Error', 'The member has not been registered for mobile banking.');
    ...
    begin
        ...
        if AccountType = 'ALL' THEN BEGIN
            ...
            if Memb.FindFirst() then begin
                Vendors.Reset();
                Vendors.SetRange(Vendors."BOSA Account No", Memb."No.");
                if Vendors.Findset then begin
                    SetResponseStatus(ResponseJson, 'success', 'Success', 'Request processed successfully');
                    repeat
                        AccountNumber := Vendors."BOSA Account No";

                        Vendors.CalcFields(Vendors.Balance);
                        PendingAmount := 0;
                        MBuffer.Reset();
                        MBuffer.SetRange(MBuffer."Vendor No", Vendors."No.");
                        MBuffer.SetRange(MBuffer.Posted, false);
                        MBuffer.SetRange(MBuffer.Reversed, false);
                        if MBuffer.FindSet() then begin
                            MBuffer.CalcSums(MBuffer."Amount Requested");
                        ...
                        AccountTypes.Get(Vendors."Account Type");
                        Vendors.CalcFields(Vendors."Balance (LCY)");
                        if Vendors."Account Type" = '103' then begin
                            AvailableBalance := 0;
                            AvailableBalance := Vendors."Balance (LCY)" - ((Vendors."Uncleared Cheques" - Vendors."Cheque Discounted Amount") + PendingAmount + Vendors."ATM Transactions" + Vendors."EFT Transactions" + 1000 + Vendors."Mobile Transactions" + Vendors."Amount to freeze");
                        end;
                        if Vendors."Account Type" <> '103' then begin
                            AvailableBalance := 0;
                            AvailableBalance := Vendors."Balance (LCY)";
                        end;

                        AccountObject.Add('account_name', Vendors."Account Type");
                        AccountObject.Add('account_label', AccountTypes.Description);
                        AccountObject.Add('account_number', Vendors."No.");
                        AccountObject.Add('account_type', Format(AccountTypes."Account Location"));
                        AccountObject.Add('account_balance', AvailableBalance);
                        AccountObject.Add('account_status', UpperCase(Format(Vendors.Status)));
                        AccountObject.Add('can_withdraw', UpperCase(Format(AccountTypes.Withrawable)));
                        AccountObject.Add('max_withdrawable_amount', AccountTypes."Maximum Withdrawal Amount");
                        AccountObject.Add('can_deposit', 'YES');
                        AccountObject.Add('max_deposit_amount', AccountTypes."Maximum Allowable Deposit");
                    ...
                    repeat
                        ...
                        IF AccountTypes.FindFirst() then BEGIN
                            ...
                            if MBuffer.FindSet() then begin
                                MBuffer.CalcSums(MBuffer."Amount Requested");
                                PendingAmount := MBuffer."Amount Requested";
                                //  Error('Available%1', AvailableBalance);
                            end;
                            AvailableBalance := 0;
                            AvailableBalance := Vendor.Balance - ((Vendor."Uncleared Cheques" - Vendor."Cheque Discounted Amount") + PendingAmount + Vendor."ATM Transactions" + Vendor."EFT Transactions" + 1000 + Vendor."Mobile Transactions" + Vendor."Amount to freeze");
                            AccountObject.Add('account_name', Vendor."Account Type");
            ...
            if IdentifierType = 'MSISDN' then BEGIN
                ...
                if Memb.FindFirst() then begin
                    ...
                    if Vendor.FindFirst() then begin
                        ...
                        detVend.SetRange("Posting Date", Today);
                        detVend.SetRange(Reversed, false);
                        detVend.SetRange(Description, 'Mobile App login Charges');
                        if detVend.find('-') = false then begin
                            IF AvailableBalance >= 30 then BEGIN
                                ...
                                'Mobile App login Charges' + ' ' + Vend."No.", '');//
    ...
    begin
            ...
            if Vend.FindFirst() then begin
                if Vend.Get(AccountNumber) then begin
                    ...
                    if Vend."Account Type" = '103' then begin
                        ...
                        LoansRegister.SetRange("Client Code", Vend."BOSA Account No");
                        // LoansRegister.SetFilter("Employer Code", '%1|%2', 'KENYA POSTAL', 'POSTAL CORP');
                        LoansRegister.SetFilter("Loan Product Type", '%1|%2', 'A03', 'M_OD');
                        ...
                        AvailableBalance := AvailableBalance - DefaultedAmount;
                    end;
                    if Vend."Account Type" <> '103' then begin
                    ...
                    SaccoGen.Get();
                    if AvailableBalance < (SaccoGen."Balance Enquiry Charge" + SaccoGen."Vendor Enquiry Charge") then begin
        ...
        if Found = false then begin
            SetResponseStatus(ResponseJson, 'error', 'Error', Responses);
        end;
        ResponseJson.Add(Data, DataJson);
    end;


    local procedure BalanceEnquiryMobileApp(RequestJson: JsonObject) ResponseJson: JsonObject
    var
        DataJson: JsonObject;
        IdentifierType: Text;
        AccountObject: JsonObject;
    ...
    begin
                    ...
                    if SavAccount.FindFirst() then begin
                        ...
                        if StatementType = 'MINI_STATEMENT' then begin
                            ...
                            if Members.FindFirst() then begin
                                Vendor.get(Members."FOSA Account No.");
                                PendingAmount := 0;
                                MBuffer.Reset();
                                MBuffer.SetRange(MBuffer."Vendor No", Vendor."No.");
                                MBuffer.SetRange(MBuffer.Posted, false);
                                ...
                                Vendor.CalcFields(Vendor.Balance, Vendor."Mobile Transactions");
                                AvailableBalance := 0;
                                AvailableBalance := Vendor.Balance - ((Vendor."Uncleared Cheques" - Vendor."Cheque Discounted Amount") + PendingAmount + Vendor."ATM Transactions" + Vendor."EFT Transactions" + 1000 + Vendor."Mobile Transactions" + Vendor."Amount to freeze");
                                if AvailableBalance < (SaccoGen."Ministatement Charge" + SaccoGen."Vendor MiniStatement Charge") then begin
                                    ChargeFound := false;
                                end;
                                //Error('Vendor%1 Available%2Pending%3Mobile%4',Vendor."No.",AvailableBalance,PendingAmount, PendingAmount + Vendor."ATM Transactions" + Vendor."EFT Transactions" + 1000 + Vendor."Mobile Transactions" + Vendor."Amount to freeze");
                                if AvailableBalance >= (SaccoGen."Ministatement Charge" + SaccoGen."Vendor MiniStatement Charge") then begin
                                    SetResponseStatus(ResponseJson, 'success', 'Success', 'Request processed successfully');
        ...
        if ChargeFound = true then begin
            ResponseJson.Add(Data, DataJson);
        end;

        if ChargeFound = false then begin
            SetResponseStatus(ResponseJson, 'error', 'Error', 'Insufficient Balance');
        end;
    end;

    local procedure StatementEnquiryMobile(RequestJson: JsonObject) ResponseJson: JsonObject
    var
        DataJson: JsonObject;
        IdentifierType: Text;
        Identifier: Text;
        ...
        if PesaType = 'PESA_OUT' then begin
            ...
            if WithdrawalBuffer.Find('-') = false then begin
                ...
                if Vend.FindFirst() then begin
                    ...
                    AvailableBalance := 0;
                    // AvailableBalance := Vend.Balance - ((Vend."Uncleared Cheques" - Vend."Cheque Discounted Amount") + TotalAmount + PendingAmount + Vend."ATM Transactions" + Vend."EFT Transactions" + 1000 + Vend."Mobile Transactions" + Vend."Amount to freeze");
                    AvailableBalance := Vend.GetAvailableBalance() - TotalAmount;
                    ...
                    if FnOkayMember(Vend."No.", Vend."Account Type") <> true then begin
                        Response := 'The member account is not active.';
                    end;

                    If AUFactory.CheckNumberOfTransactions(Vend."No.") = true then begin
                        Response := 'You have exceeded the number of times to transact today.';
                    end;

                    accTypes.Get(Vend."Account Type");
                    if accTypes."Product Type" = accTypes."Product Type"::"Non-Withdrawable" then begin
                        Response := 'You cannot transact from this account';
                    end;

                    if Response = '' then begin
                        if Amount > AvailableBalance then begin
                            Response := 'You have insufficient balance in your FOSA account.';
                        end else begin
                            Response := 'Success';
    ...
    begin
        ...
        if TransactionStatus = 'CONFIRMED' then begin
            ...
            BufferTable.SetRange(Reversed, false);
            if BufferTable.FindSet() then begin
                Vend.Reset();
                Vend.SetRange("No.", BufferTable."Vendor No");
                if Vend.Find('-') then begin
                    // if (Vend.GetAvailableBalance() + BufferTable."Amount Requested") < BufferTable."Amount Requested" then begin
                    //     SetResponseStatus(ResponseJson, 'FAIL', 'fail', 'Insufficient Funds');
                    // end else begin
                    MpesaMobile.Reset();
                    ...
                    if not MpesaMobile.FindFirst() then begin
                        ...
                        if Category = 'MPESA_WITHDRAWAL' then begin
                            ...
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::Vendor, BufferTable."Vendor No", BufferTable."Transaction Date", BufferTable."Amount Requested", 'FOSA', DOCUMENT_NO,
                            'Mobile withdrawal to ' + BeneficiaryName + ' ' + Vend."No.", '');//

                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::Vendor, BufferTable."Vendor No", BufferTable."Transaction Date", TotalAmount, 'FOSA', DOCUMENT_NO,
                            'Mobile charges' + ' ' + Vend."No.", '');//

                            //Balancing Account
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::"Bank Account", 'BNK00013', BufferTable."Transaction Date", BufferTable."Amount Requested" * -1, 'FOSA', DOCUMENT_NO,
                            'Mobile withdrawal' + ' ' + Vend."No.", '');

                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::"Bank Account", 'BNK00013', BufferTable."Transaction Date", MpesaComm * -1, 'FOSA', DOCUMENT_NO,
                            'Mobile Trans Mpesa' + ' ' + Vend."No.", '');

                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::Vendor, VendorCommAccount, BufferTable."Transaction Date", VendorComm * -1, 'FOSA', DOCUMENT_NO,
                            'Mobile Trans Mpesa Comm' + ' ' + Vend."No.", '');

                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::"G/L Account", SaccoCommissionAccount, BufferTable."Transaction Date", SaccoCommission * -1, 'FOSA', DOCUMENT_NO,
                            'Mobile Trans Mpesa Comm' + ' ' + Vend."No.", '');

                            SaccoGen.Get();
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::"G/L Account", SaccoGen."Excise Duty Account", BufferTable."Transaction Date", ExciseDuty * -1, 'FOSA', DOCUMENT_NO,
                        ...
                        if Category = 'BANK_TRANSFER' then begin
                            ...
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::Vendor, BufferTable."Vendor No", BufferTable."Transaction Date", ((SaccoGen."Excise Duty(%)" / 100) * TotalAmount), 'FOSA', DOCUMENT_NO,
                            ...
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::"G/L Account", VendorCommAccount, BufferTable."Transaction Date", VendorComm * -1, 'FOSA', DOCUMENT_NO,
                            'Mobile Trans Mpesa Comm' + ' ' + Vend."No.", '');
                            ...
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::"G/L Account", SaccoGen."Excise Duty Account", BufferTable."Transaction Date", ((SaccoGen."Excise Duty(%)" / 100) * TotalAmount) * -1,
                        ...
                        SetResponseStatus(ResponseJson, 'FAIL', 'Fail', 'Duplicate Transaction');
                    end;
                end;
                // end;
            end else
                SetResponseStatus(ResponseJson, 'FAIL', 'Fail', 'Invalid originator ID');
            ResponseJson.Add(Data, DataJson);
        end;
        ...
        if TransactionStatus = 'REVERSE_CONFIRMED' then begin
            ...
            if BufferTable.FindSet() then begin
                ...
                SetResponseStatus(ResponseJson, 'FAIL', 'Fail', 'originator ID already reversed');
            end;
            ResponseJson.Add(Data, DataJson);
        end;
    end;

    local procedure GetBusinessDetails(RequestJson: JsonObject) ResponseJson: JsonObject
    var
        DataJson: JsonObject;
        OriginatorID: Text;
        TransactionStatus: Text;
        ...
        Source: Option NEW_MEMBER,NEW_ACCOUNT,LOAN_ACCOUNT_APPROVAL,DEPOSIT_CONFIRMATION,CASH_WITHDRAWAL_CONFIRM,LOAN_APPLICATION,LOAN_APPRAISAL,LOAN_GUARANTORS,LOAN_REJECTED,LOAN_POSTED,LOAN_DEFAULTED,SALARY_PROCESSING,TELLER_CASH_DEPOSIT,TELLER_CASH_WITHDRAWAL,TELLER_CHEQUE_DEPOSIT,FIXED_DEPOSIT_MATURITY,INTERACCOUNT_TRANSFER,ACCOUNT_STATUS,STATUS_ORDER,EFT_EFFECTED,ATM_APPLICATION_FAILED,ATM_COLLECTION,MBANKING,MEMBER_CHANGES,CASHIER_BELOW_LIMIT,CASHIER_ABOVE_LIMIT,INTERNETBANKING,CRM,MOBILE_PIN;
    begin
        BusinessShortCode := SelectJsonToken(RequestJson, '$.business_short_code').AsValue.AsText;
        IdentifierType := SelectJsonToken(RequestJson, '$.identifier_type').AsValue.AsText;
        Identifier := SelectJsonToken(RequestJson, '$.identifier').AsValue.AsText;
        ...
        Vend.Reset();
        // Vend.SetRange(Vend."Mobile Phone No", Identifier);
        Vend.SetRange(Vend."Account Type", '111');
        Vend.SetRange(Vend."Business Short Code(Store)", BusinessShortCode);
        if Vend.FindFirst() then begin
            Vend.CalcFields(Vend."Business Logo(Store)");
            SetResponseStatus(ResponseJson, 'success', 'Success', 'Transaction posted successfully');
            DataJson.Add('business_name', Vend."Business Name(Store)");
            DataJson.Add('business_short_code', Vend."Business Short Code(Store)");
            DataJson.Add('business_type', Vend."Business Type(Store)");
            DataJson.Add('business_status', Format(Vend."Business Status(Store)"));
            DataJson.Add('business_location', Vend.Location);
            DataJson.Add('business_contact', Vend."Business Contact(Store)");
            DataJson.Add('business_email', Vend."Business Email(Store)");
            DataJson.Add('business_phone', Vend."Business Phone(Store)");
            DataJson.Add('business_website', Vend."Business Website(Store)");
            DataJson.Add('business_logo', Format(Vend."Business Logo(Store)"));
            DataJson.Add('business_description', Vend."Business Description(Store)");
        end else
            SetResponseStatus(ResponseJson, 'Error', 'Error', 'Account does not exist.');
        ResponseJson.Add(Data, DataJson);
    end;
    ...
    begin
        ...
        if SavAccount.FindFirst() then begin
            ...
            // if SavAccount.Balance >= Amount then begin
            if SavAccount.GetAvailableBalance() >= Amount then begin
                ...
                if FOSAAccounts.FindFirst() then begin
                    ...
                    Vend.Reset();
                    Vend.SetRange("BOSA Account No", FOSAAccounts."BOSA Account No");
                    ...
                    if LoansRegister.FindFirst then begin
                        ...
                        LoansRegister.CalcFields("Outstanding Balance", "Outstanding Interest", "Outstanding Penalty");
                        if LoansRegister."Outstanding Penalty" > 0 then begin
                            PenAmount := 0;
                            if LoansRegister."Outstanding Penalty" > RunBal then begin
                                PenAmount := RunBal;
                            end else begin
                                PenAmount := LoansRegister."Outstanding Penalty";
                            end;
                            ...
                            GenJournalLine."Account Type"::Customer, LoansRegister."Client Code", Today, PenAmount * -1, 'FOSA', DOCUMENT_NO,
                        ...
                        if LoansRegister."Outstanding Interest" > 0 then begin
                            IntAmount := 0;
                            if LoansRegister."Outstanding Interest" > RunBal then begin
                                IntAmount := RunBal;
                            end else begin
                        ...
                        if LoansRegister."Outstanding Balance" > 0 then begin
                            PrincipleAMount := 0;
                            if LoansRegister."Outstanding Balance" > RunBal then begin
                        ...
                        if RunBal > 0 then begin
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::Vendor, depositsAcc, Today, RunBal * -1, 'FOSA', DOCUMENT_NO,
                            'Excess Funds Transfer' + ' ' + SavAccount."No.", LoansRegister."Loan  No.");
                        end;

                        CreationMessage := 'Dear ' + BName.NameStyle(Members."No.") + ', your ' + LProducts."Product Description" + ' loan repayment of Kshs ' + Format(Amount) + ' from ' + AccountTypes.Description + ' has been processed successfully on ' + Format(Today) + ' at ' + Format(time) + 'REF:' + SourceReference;
                    ...
                    SetResponseStatus(ResponseJson, 'error', 'Error', 'Account not found');
            end else begin
                SetResponseStatus(ResponseJson, 'error', 'Error', 'Insufficient balance');

                DataJson.Add('transaction_reference', SourceReference);
                DataJson.Add('transaction_status', 'FAIL');
                DataJson.Add('transaction_status_description', 'Account has Insufficient Funds');
                ResponseJson.Add(Data, DataJson);
            end;
        end else
            SetResponseStatus(ResponseJson, 'error', 'Error', 'Account not found');
    ...
    var
        ...
        VendorComm: Decimal;
        ExciseDuty: Decimal;
        AccountTypesX: Record "Account Types-Saving Products";
        TotalAmount: Decimal;
        VendorCommAccount: Code[20];
        ExciseDutyAccount: Code[20];
        TotalAmountAccount: Code[20];
    ...
    begin
        ...
        if SavAccount.FindFirst() then begin
            ...
            AvailableBalance := 0;
            SavAccount.CalcFields(SavAccount.Balance);
            AvailableBalance := SavAccount.Balance - ((SavAccount."Uncleared Cheques" - SavAccount."Cheque Discounted Amount") + PendingAmount + SavAccount."ATM Transactions" + SavAccount."EFT Transactions" + 1000 + SavAccount."Mobile Transactions" + SavAccount."Amount to freeze");

            //Error('Balance%1Amount%2PendingAmount%3SavBal%4',AvailableBalance,Amount,PendingAmount,SavAccount.Balance);

            if (AvailableBalance > Amount) and (FnOkayMember(SavAccount."No.", SavAccount."Account Type") = true) then begin
                // Error('Available%1Amount%2', AvailableBalance, Amount);
                ...
                if FOSAAccounts.FindFirst() then begin
                    ...
                    if Vend.Find('-') then begin
                        ...
                        VendB.Reset();
                        VendB.SetRange(VendB."No.", DestinationAccount);
                        if VendB.FindFirst() then begin

                            CreationMessage := 'Dear ' + NameStyle.NameStyle(Members."No.") + ', your transfer of Ksh ' + Format(Amount) + ' from your ' + AccountTypes.Description + ' to ' + NameStyle.NameStyle(VendB."BOSA Account No") + '''s ' + AccountType.Description + ' has been processed successfully on ' + Format(Today) + ' at ' + Format(time) + 'REF:' + SourceReference;
                            smsManagement.SendSmsWithID(Source::INTERACCOUNT_TRANSFER, Members."Mobile Phone No", CreationMessage, Members."No.", Members."FOSA Account No.", TRUE, 200, TRUE, 'CBS', CreateGuid(), 'CBS');

                            AccountTypes.Get(VendB."Account Type");
                            Cust.Reset();
                            Cust.SetRange(Cust."No.", VendB."BOSA Account No");
                            if Cust.FindFirst() then begin
                                CreationMessageDest := 'Dear ' + NameStyle.NameStyle(Cust."No.") + ',  Ksh ' + Format(Amount) + ' from ' + Members.Name + ' has been credited to your ' + AccountTypes.Description + ' account on' + Format(Today) + ' at ' + Format(time) + 'REF:' + SourceReference;
                                smsManagement.SendSmsWithID(Source::INTERACCOUNT_TRANSFER, Cust."Mobile Phone No", CreationMessageDest, Cust."No.", Cust."FOSA Account No.", TRUE, 200, TRUE, 'CBS', CreateGuid(), 'CBS');
                            end;
                        end;
    ...
    var
        ...
        PercentageGra: Decimal;
        Cust: Record Customer;
        StatusP: Option Failed,Posted,Pending;
        Source: Option NEW_MEMBER,NEW_ACCOUNT,LOAN_ACCOUNT_APPROVAL,DEPOSIT_CONFIRMATION,CASH_WITHDRAWAL_CONFIRM,LOAN_APPLICATION,LOAN_APPRAISAL,LOAN_GUARANTORS,LOAN_REJECTED,LOAN_POSTED,LOAN_DEFAULTED,SALARY_PROCESSING,TELLER_CASH_DEPOSIT,TELLER_CASH_WITHDRAWAL,TELLER_CHEQUE_DEPOSIT,FIXED_DEPOSIT_MATURITY,INTERACCOUNT_TRANSFER,ACCOUNT_STATUS,STATUS_ORDER,EFT_EFFECTED,ATM_APPLICATION_FAILED,ATM_COLLECTION,MBANKING,MEMBER_CHANGES,CASHIER_BELOW_LIMIT,CASHIER_ABOVE_LIMIT,INTERNETBANKING,CRM,MOBILE_PIN;
    begin
        OriginatorID := SelectJsonToken(RequestJson, '$.originator_id').AsValue.AsText;
        PesaType := SelectJsonToken(RequestJson, '$.pesa_type').AsValue.AsText;
        ...
        if MPESATRANS.Find('-') = false then begin
            ...
            Customer.SetFilter(Status, '%1|%2', Cust.Status::Active, Cust.Status::Dormant);
            Customer.SetFilter("Membership Status", '%1|%2', Cust."Membership Status"::Active, Cust."Membership Status"::Dormant);
            if Customer.FindFirst() then begin
                LoanRegister.Reset();
                ...
                if LoanRegister.Find('-') then begin
                    ...
                    if AvailableBalance < TotalAmount then begin
                        ...
                        Loans.Reset();
                        Loans.SetRange(Loans."Loan  No.", TransactionReceiverAccount);
                        if Loans.FindFirst() then begin
                            RunBal := 0;
                            RunBal := NetCharges;
                            Loans.CalcFields(Loans."Outstanding Interest", Loans."Outstanding Balance", Loans."Outstanding Penalty");
                            if Loans."Outstanding Penalty" > 0 then begin
                                OutPen := 0;
                                if RunBal > Loans."Outstanding Penalty" then
                                    Outpen := Loans."Outstanding Penalty"
                                else
                                    OutPen := RunBal;
                                LineNo := LineNo + 10000;
                                SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::"Loan Penalty Paid",
                                GenJournalLine."Account Type"::Customer, Loans."Client Code", Today, OutPen * -1, 'FOSA', DOCUMENT_NO,
                                 'Mobile Loan Penalty Repayment' + ' ' + Customer."No.", transactionReceiverAccount);
                                RunBal := RunBal - OutPen;
                            end;
                            if Loans."Outstanding Interest" > 0 then begin
                                OutInt := 0;
                                if RunBal > Loans."Outstanding Interest" then
                                    OutInt := Loans."Outstanding Interest"
                                else
                                    OutInt := RunBal;
                                LineNo := LineNo + 10000;
                                SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::"Interest Paid",
                                GenJournalLine."Account Type"::Customer, Loans."Client Code", Today, OutInt * -1, 'FOSA', DOCUMENT_NO,
                                 'Mobile Loan Interest Repayment' + ' ' + Customer."No.", transactionReceiverAccount);
                            ...
                            if RunBal > 0 then begin
                                if Loans."Outstanding Balance" > 0 then begin
                                    ...
                                    if RunBal > Loans."Outstanding Balance" then
                                        Repay := Loans."Outstanding Balance"
                                    else
                                        Repay := RunBal;
                                    LineNo := LineNo + 10000;
                                    SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::Repayment,
                                    GenJournalLine."Account Type"::Customer, Loans."Client Code", Today, Repay * -1, 'FOSA', DOCUMENT_NO,
                            ...
                            if RunBal > 0 then begin
                                LineNo := LineNo + 10000;
                                SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                                GenJournalLine."Account Type"::Vendor, Customer."FOSA Account No.", Today, RunBal * -1, 'FOSA', DOCUMENT_NO,
                                 'Mobile Excess' + ' ' + Customer."No.", transactionReceiverAccount);
                                RunBal := RunBal - Repay;
                            end;
                    ...
                    if not PaybillTransactions.FindFirst() then begin
                        //Cust.get();
                        AuPaybillAutomations.FnInsertPaybillTransactionsMobile(OriginatorID, '', Amount, TransactionReceiverIdentifier, TransactionReceiverAccount, '', 0,
                         '', TransactionInitiatorIdentifier, TransactionInitiatorName, '', '', '', Customer."ID No.");
                    end;
            ...
            if Found = false then begin
                ...
                if not PaybillTransactions.FindFirst() then begin
                    ...
                    AuPaybillAutomations.FnInsertPaybillTransactions(OriginatorID, '', Amount, TransactionReceiverIdentifier, TransactionReceiverAccount, '', 0,
                     TransactionSenderReference, TransactionInitiatorIdentifier, TransactionInitiatorName, '', '', '');
                end ELSE begin
                    DataJson.Add('transaction_reference', OriginatorID);
            ...
            if Members.FindFirst() then begin
                ...
                if ProdFactory.FindFirst() then begin
                    ...
                    repeat
                        ...
                        Maxloan := FnGetMobileAdvanceEligibility(Members."No.", ProdFactory.Code, ProdFactory."Product Description", ProdFactory."Max. Loan Amount", ProdFactory."Default Installements");
                ...
                if ProdFactory.FindFirst() then begin
                    ...
                    repeat
                        ...
                        if (ProdFactory.Code = 'A03') or (ProdFactory.Code = 'A16') then begin
                            Maxloan := FnGetMobileAdvanceEligibility(Members."No.", ProdFactory.Code, Msg, ProdFactory."Max. Loan Amount", ProdFactory."Default Installements");
                        end;
                        if ProdFactory.Code = 'A01' then begin
                            Maxloan := GetSalaryLoanQualifiedAmount(Members."No.", ProdFactory.Code, ProdFactory."Max. Loan Amount", Msg, ProdFactory."Default Installements")
                        end;
                        ...
                        if ProdFactory.Code = 'A10' then begin
                            Maxloan := GetReloadedLoanQualifiedAmount(Members."No.", ProdFactory.Code, ProdFactory."Max. Loan Amount", Msg, ProdFactory."Default Installements")
                        end;
    ...
    begin
        ...
        if IdentifierType = 'MEMBER_NUMBER' THEN BEGIN
            ...
            if Members.Findlast() then begin
                ...
                if ProdFactory.FindSet() then begin
                    SetResponseStatus(ResponseJson, 'success', 'Success', 'Request processed successfully');
                    repeat
                        Maxloan := 0;
                        //Maxloan := GetGrossSalary(Members."No.", ProdFactory.Code);
    ...
    local procedure GetSpecificLoanTypesDetailsWithDurAmount(RequestJson: JsonObject) ResponseJson: JsonObject
    var
        DataJson: JsonObject;
        IdentifierType: Text;
        Identifier: Text;
    ...
    begin
        ...
        Identifier := SelectJsonToken(RequestJson, '$.identifier').AsValue.AsText;
        LoanType := SelectJsonToken(RequestJson, '$.loan_type').AsValue.AsText;
        duration := SelectJsonToken(RequestJson, '$.loan_type_duration').AsValue().AsInteger;
        amount := SelectJsonToken(RequestJson, '$.loan_type_amount').AsValue().AsDecimal;
            ...
            if Members.Findlast() then begin
                ...
                if ProdFactory.FindSet() then begin
                    ...
                    repeat
                        ...
                        //Maxloan := GetGrossSalary(Members."No.", ProdFactory.Code);
                        if (ProdFactory.Code = 'A03') or (ProdFactory.Code = 'A16') then begin
                            Maxloan := FnGetMobileAdvanceEligibility(Members."No.", ProdFactory.Code, Msg, ProdFactory."Max. Loan Amount", duration);
                        end;
                        if ProdFactory.Code = 'A01' then begin
                            // FnCheckLoanIfPendingGuar(Members."BOSA Account No", ProdFactory.Code);
                            Maxloan := GetSalaryLoanQualifiedAmount(Members."No.", ProdFactory.Code, ProdFactory."Max. Loan Amount", Msg, duration)
                        end;
                        if ProdFactory.Code = 'M_OD' then begin
                            Maxloan := GetOverdraftLoanQualifiedAmount(Members."No.", ProdFactory.Code, ProdFactory."Max. Loan Amount", Msg)
                        end;
                        if ProdFactory.Code = 'A10' then begin
                            // FnCheckLoanIfPendingGuar(Members."BOSA Account No", ProdFactory.Code);
                            Maxloan := GetReloadedLoanQualifiedAmount(Members."No.", ProdFactory.Code, ProdFactory."Max. Loan Amount", Msg, duration)
                        end;
    ...
    begin
        ...
        if IdentifierType = 'MSISDN' THEN BEGIN
            ...
            if Members.FindFirst() then begin
                ...
                Loans.SetRange(Loans."Archive Loan", false);
                if Loans.FindFirst() then begin
                    SetResponseStatus(ResponseJson, 'success', 'Success', 'Request processed successfully');
                    repeat
        ...
        if not LoanRec.FindFirst() then begin
            if IdentifierType = 'MSISDN' THEN BEGIN
                ...
                if Members.Find('-') then begin
                    ...
                    if FnCheckLoanIfPendingGuar(Members."No.", LoanTypeID) = '' then begin
                        LoansRegister.Reset;
                        ...
                        LoansRegister.SetAutoCalcFields(LoansRegister."Total Outstanding Balance", LoansRegister."Outstanding Balance");
                        LoansRegister.SetFilter(LoansRegister."Total Outstanding Balance", '>0');
                        ...
                        if LoanProductSetup.FindFirst() then begin
                            ...
                            LoanRec."Employer Code" := Members."Employer Code";
                            LoanRec."Main Sector" := '8000';
                            LoanRec."Sub-Sector" := '8200';
                            LoanRec."Specific Sector" := '8210';
                            //  LoanRec."Requested Amount" := amount;
                            LoanRec."Repayment Frequency" := LoanRec."Repayment Frequency"::Monthly;
                            LoanRec."Requested Amount" := amount;
                            ...
                            LoanRec."Loan Product Type Name" := LoanProductSetup."Product Description";//inserting mobile loan name

                            if LoanProductSetup.Code = 'A03' then
                                LoanRec."Recovery Mode" := LoanRec."Recovery Mode"::Mobile
                            else
                            ...
                            RatingLoanLimit := GetCreditRatingLimit(Members."No.", LoanRec."Loan Product Type");
                            CreditRating.INIT;
                            ...
                            if LoanProductSetup.FindFirst() then begin
                                ...
                                LoanRec.Installments := Installments;
                                LoanRec."Loan Disbursement Date" := Today;
                                LoanRec."Disbursed By Time" := Time;
                                // LoanRec.Validate("Loan Disbursement Date");
                                ...
                                LoanRec."Loan Product Type Name" := LoanProductSetup."Product Description";//updating mobile loan name

                                //LoanRec.Validate(LoanRec."Loan Product Type");
                                LoanRec.INSERT(true);
                                // Mob.ProcessAdvanceLoansFinal(SourceReference);
                                FosaAcc.GET(LoanRec."Account No");
                                ...
                                'Mobile loan ' + LoanRec."Loan Product Type Name" + ' ' + Members."No.", '');
                                ...
                                /* if LoanType."Charge Interest Upfront" = true then begin
                                    ...
                                    'Mobile Loan ' + ' ' + Members."No.", LoanRec."Loan  No.");Kitu
                                    ...
                                    IF PCharges.FIND('-') THEN BEGIN
                                        REPEAT
                                            ...
                                            IF AuFactory.FnChargeExcise(PCharges.Code) THEN BEGIN
                                                ...
                                                LineNo := LineNo + 10000;
                                                AuFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                                                GenJournalLine."Account Type"::"G/L Account", GenSetUp."Excise Duty Account", Today, (PChargeAmount * (GenSetUp."Excise Duty(%)" / 100)) * -1, 'BOSA', LoansRegister."Loan  No.",
                                                PCharges.Description + '-' + LoansRegister."Client Code" + '-' + LoansRegister."Loan Product Type Name" + '-' + LoansRegister."Loan  No." + '- Excise Duty', LoansRegister."Loan  No.", GenJournalLine."Application Source"::" ");
                                                //-----------------Recover---------------------------------
                                                LineNo := LineNo + 10000;
                                                AuFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                                                GenJournalLine."Account Type"::Vendor, LoansRegister."Account No", Today, (PChargeAmount * (GenSetUp."Excise Duty(%)" / 100)), 'BOSA', LoansRegister."Loan  No.",
                                                PCharges.Description + '-' + LoansRegister."Loan Product Type Name" + ' - Excise Duty', LoansRegister."Loan  No.", GenJournalLine."Application Source"::" ");
                                            END
                                        //----------------END 10% EXCISE--------------------------------------------
                                ...
                                if (LoanType."Charge Interest Upfront" <> true) and (LoanType."Recovery Mode" = LoanType."Recovery Mode"::Mobile) then begin
                                    ...
                                    'Mobile Trans interest Upfront' + ' ' + Members."No.", LoanRec."Loan  No.");
                                ...
                                if (LoanType."Charge Interest Upfront" = true) and (LoanType."Recover Upfront Interest" = true) then begin
                                    LineNo := LineNo + 10000;
                                    SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                                    GenJournalLine."Account Type"::"G/L Account", LoanType."Loan Interest Account", Today, ((LoanRec.Installments * LoanType."Interest rate") * Amount / 100) * -1, '', DOCUMENT_NO,
                                    'Mobile Trans interest Upfront' + ' ' + Members."No.", '');

                                    LineNo := LineNo + 10000;
                                    SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::"Interest Due",
                                    GenJournalLine."Account Type"::Customer, LoanRec."Client Code", Today, ((LoanRec.Installments * LoanType."Interest rate") * Amount / 100), '', DOCUMENT_NO,
                                    'Mobile Loan ' + ' ' + Members."No.", LoanRec."Loan  No.");

                                    LineNo := LineNo + 10000;
                                    SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                                    GenJournalLine."Account Type"::Vendor, Members."FOSA Account No.", Today, ((LoanRec.Installments * LoanType."Interest rate") * Amount / 100), '', DOCUMENT_NO,
                                    'Mobile Trans interest Repayment' + ' ' + LoanRec."Loan Product Type Name" + ' ' + Members."No.", '');

                                    LineNo := LineNo + 10000;
                                    SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::"Interest Paid",
                                    GenJournalLine."Account Type"::Customer, Members."No.", Today, ((LoanRec.Installments * LoanType."Interest rate") * Amount / 100) * -1, '', DOCUMENT_NO,
                                    'Mobile Trans interest Repayment' + ' ' + Members."No.", LoanRec."Loan  No.");

                                end;

                                if (LoanType."Charge Interest Upfront" = true) and (LoanType."Recover Upfront Interest" = false) then begin
                                    LineNo := LineNo + 10000;
                                    SFactory.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                                    GenJournalLine."Account Type"::"G/L Account", LoanType."Loan Interest Account", Today, (LoanRec.Installments * LoanType."Interest rate" * Amount / 100) * -1, '', DOCUMENT_NO,
                                    'Mobile Trans interest Upfront' + ' ' + Members."No.", LoanRec."Loan  No.");
                                ...
                                CreationMessage := 'Dear ' + BName.NameStyle(Members."No.") + ', your ' + LProducts."Product Description" + ' loan has been disbursed to your FOSA Account ';
                                ...
                                LoanRec."Loan Disbursement Date" := Today;
                                LoanRec."Disbursed By Time" := Time;
                                LoanRec."Disbursed By" := UserId;
                                LoanRec.Validate(LoanRec."Loan Disbursement Date");
                                LoanRec."Loan Status" := LoanRec."Loan Status"::Issued;
                                LoanRec.Posted := true;
                                /*                             AuFactory.FnGenerateLoanRepaymentSchedule(LoanRec."Loan  No.");
                                                            RSchedule.Reset();
                                                            RSchedule.SetRange(RSchedule."Loan No.", LoanRec."Loan  No.");
                                                            if RSchedule.FindFirst() then begin
                                                                LoanRec."Loan Principle Repayment" := Round(RSchedule."Principal Repayment", 1, '>');
                                                                LoanRec."Loan Interest Repayment" := Round(RSchedule."Monthly Interest", 1, '>');
                                                                LoanRec.Repayment := Round(RSchedule."Monthly Repayment", 1, '>');
                                                                LoanRec.Modify();
                                                            end; */
                                LoanRec.Modify();
                            end;
                        ...
                        SetResponseStatus(ResponseJson, 'error', 'Error', 'Dear member, you have a loan pending guarantorship. Kindly reject or add guarantors.');
                end;
                    ...
                    if FnCheckLoanIfPendingGuar(Members."No.", LoanTypeID) = '' then begin
                        ...
                        if not LoansRegister.FindFirst() then begin
                            ...
                            if LoanProductSetup.FindFirst() then begin
                                ...
                                LoanRec."Sub-Sector" := '8200';
                                LoanRec."Specific Sector" := '8210';
                                LoanRec.Validate("Specific Sector");
                                //LoanRec."Loan Disbursement Date":=Today;
                                //LoanRec.Validate("Loan Disbursement Date");
                                LoanRec."Repayment Frequency" := LoanRec."Repayment Frequency"::Monthly;
                                LoanRec."Mode Of Application" := LoanRec."Mode Of Application"::Mobile;
                                ...
                                LoanRec."Loan Product Type Name" := LoanProductSetup."Product Description"; //updating loan name
                                        ...
                                        IF PCharges.FIND('-') THEN BEGIN
                                            REPEAT
                                                ...
                                                IF AuFactory.FnChargeExcise(PCharges.Code) THEN BEGIN
                                                    ...
                                                    AuFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                                                    GenJournalLine."Account Type"::"G/L Account", GenSetUp."Excise Duty Account", Today, (PChargeAmount * (GenSetUp."Excise Duty(%)" / 100)) * -1, 'BOSA', LoansRegister."Loan  No.",
                                                    PCharges.Description + '-' + LoansRegister."Client Code" + '-' + LoansRegister."Loan Product Type Name" + '-' + LoansRegister."Loan  No." + '- Excise(20%)', LoansRegister."Loan  No.", GenJournalLine."Application Source"::" ");
                                                    //-----------------Recover---------------------------------
                                                    LineNo := LineNo + 10000;
                                                    AuFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                                                    GenJournalLine."Account Type"::Vendor, LoansRegister."Account No", Today, (PChargeAmount * (GenSetUp."Excise Duty(%)" / 100)), 'BOSA', LoansRegister."Loan  No.",
                                                    PCharges.Description + '-' + LoansRegister."Loan Product Type Name" + ' - Excise(20%)', LoansRegister."Loan  No.", GenJournalLine."Application Source"::" ");
                                                END
                                            //----------------END 10% EXCISE--------------------------------------------
                                            UNTIL PCharges.NEXT = 0;
                                        END;
    ...
    procedure GetCreditRatingLimit(memberNo: Code[20]; loanProduct: Code[20]) limit: Decimal;
    var
        creditRate: Record "Credit Rating";
        lnProduct: Record "Loan Products Setup";
        loanRec: Record "Loans Register";
        vend: Record Vendor;
        cust: Record Customer;
        ...
        maxLoanLimit: Decimal;
        Msg: Text[250];
        salaryEarner: Boolean;
    begin
        vend.Reset();
        vend.SetRange("BOSA Account No", memberNo);
        vend.SetRange("Account Type", '103');
        if vend.Find('-') then begin
            if lnProduct.Get(loanProduct) then begin
                MaxLoanAmtPossible := 0;
                if (lnProduct.Code = 'A03') or (lnProduct.Code = 'A16') then begin
                    MaxLoanAmtPossible := FnGetMobileAdvanceEligibility(vend."No.", lnProduct.Code, Msg, lnProduct."Max. Loan Amount", lnProduct."Default Installements");
                end;
                if lnProduct.Code = 'A01' then begin
                    MaxLoanAmtPossible := GetSalaryLoanQualifiedAmount(vend."No.", lnProduct.Code, lnProduct."Max. Loan Amount", Msg, lnProduct."Default Installements")
                end;
                if lnProduct.Code = 'M_OD' then begin
                    MaxLoanAmtPossible := GetOverdraftLoanQualifiedAmount(vend."No.", lnProduct.Code, lnProduct."Max. Loan Amount", Msg)
                end;
                if lnProduct.Code = 'A10' then begin
                    MaxLoanAmtPossible := GetReloadedLoanQualifiedAmount(vend."No.", lnProduct.Code, lnProduct."Max. Loan Amount", Msg, lnProduct."Default Installements")
                end;
                limit := MaxLoanAmtPossible;
            end;
        end;
    end;
        ...
        if IdentifierType = 'MEMBER_NUMBER' then begin
            ...
            if Members.FindFirst() then begin
                ...
                if LoanRec.FindFirst() then begin
                    ...
                    LoanRec."Loan Status" := LoanRec."Loan Status"::Rejected;
                    LoanRec."Approval Status" := LoanRec."Approval Status"::Rejected;
                    LoanRec.Modify();
                    Found := true;
                end;
            end;
        ...
        if Members.get(Identifier) then begin
            ...
            if Loans.FindSet() then begin
                ...
                repeat
                    ...
                    LoanObject.Add('loan_amount', Loans."Approved Amount");
                    LoanObject.Add('loan_balance', Loans."Total Outstanding Balance");
                    LoanObject.Add('installment_amount', Loans.Repayment);
                    LoanObject.Add('loan_defaulted_amount', Loans."Amount in Arrears");
                    LoanObject.Add('loan_issued_date', Loans."Loan Disbursement Date");
                    LoanObject.Add('loan_end_date', Loans."Expected Date of Completion");
                    LoanObject.Add('loan_performance', Format(Loans."Loans Category"));
                    LoanObject.Add('loan_performance_description', StrSubstNo('Loan is %1', UpperCase(Format(Loans."Loans Category"))));
    ...
    var
        ...
        RequesteUser: Text;
    begin
        RequestId := SelectJsonToken(RequestJson, '$.request.id').AsValue.AsText;
        RequestAction := SelectJsonToken(RequestJson, '$.request.action').AsValue.AsText;

        RequestSessionId := SelectJsonToken(RequestJson, '$.request.session_id').AsValue.AsText;
        RequesteUser := SelectJsonToken(RequestJson, '$.request.user').AsValue.AsText;
    ...
    begin
        UserName := SelectJsonToken(RequestJson, '$.request.user').AsValue.AsText;
        SessionId := SelectJsonToken(RequestJson, '$.request.session_id').AsValue.AsText;
    ...
    begin
            ...
            if Loans.FindFirst() then begin
                ...
                if Members.FindFirst() then begin
                    ...
                    if not LoanG.FindFirst() then begin
                        ...
                        CreationMessage := 'Dear ,' + BName.NameStyle(Members."No.") + ', ' + Loans."Client Name" + ' has requested you to guarantee ' + LoanProduct."Product Description" + ' of amount Ksh ' + Format(Loans."Requested Amount") + ' .Dial *882# to Accept or Reject.';
        ...
        if TransactionInitiatorIdentifierType = 'MSISDN' then begin
            ...
            Loans.SetRange(Loans."Loan  No.", TransactionInitiatorAccount);
            if Loans.FindFirst() then begin
                if FnCheckLoanIfExistingGuarantor(Loans."Client Code", Loans."Loan Product Type") = false then begin
                    Members.Reset();
                    Members.SetRange(Members."Mobile Phone No", UpperCase(TransactionSourceIdentifier));
                    if Members.FindFirst() then begin
                        ...
                        if not LoanG.FindFirst() then begin
                            ...
                            FnCheckIfGuarantorsMet(TransactionInitiatorAccount);
                            CreationMessage := 'Dear ,' + BName.NameStyle(Members."No.") + ', ' + Loans."Client Name" + ' has requested you guarantee ' + Loans."Loan Product Type" + ' of amount Ksh ' + Format(Loans."Requested Amount");
                            smsManagement.SendSmsWithID(Source::LOAN_GUARANTORS, Members."Mobile Phone No", CreationMessage, Members."No.", Members."FOSA Account No.", TRUE, 200, TRUE, 'CBS', CreateGuid(), 'CBS');
                            SetResponseStatus(ResponseJson, 'success', 'Success', 'Request processed successfully');
                        end else begin
                            SetResponseStatus(ResponseJson, 'error', 'Error', 'Guarantor Already Exists');
                    ...
                    SetResponseStatus(ResponseJson, 'error', 'Error', 'Member has an existing loan.');
                end;
            end else begin
                SetResponseStatus(ResponseJson, 'error', 'Error', 'Loan not found');
            end;
    ...
    procedure CheckIfMemberHasASimilarProduct(MemberNo: code[60]; LoanProduct: code[60])
    var
        Loans: Record "Loans Register";
    begin
        loans.Reset();
        Loans.SetRange(Loans."Client Code", MemberNo);
        Loans.SetRange(Loans."Loan Product Type", LoanProduct);
        loans.SetAutoCalcFields(Loans."Outstanding Balance");
        Loans.SetFilter(Loans."Outstanding Balance", '>%1', 0);
        if Loans.FindFirst() then begin
            Error('Member has a similar product');
        end;
    end;
                ...
                if Members."Current Shares" >= TransactionInitiatorAmount then begin
                    ...
                    if Loans.FindFirst() then begin
                        ...
                        if UpperCase(TransactionInitiatorOption) = 'ACCEPT' then begin
                            ...
                            loansG."Date Accepted" := CurrentDateTime;
                            loansG."Date Responded" := CurrentDateTime;
    ...
    procedure GetGrossSalary(AccountNo: Code[20]; ProductType: Code[10]) GrossSalary: Decimal
    var
        ...
        IF saccoAccount.FINDFIRST THEN BEGIN
            ...
            FOR i := 3 DOWNTO 1 DO BEGIN
                ...
                END ELSE BEGIN
                    ...
                    PayrollMonthlyTransactions.SETRANGE(PayrollMonthlyTransactions."Transaction Code", 'GPAY');
                    IF PayrollMonthlyTransactions.FINDFIRST THEN
                        SalaryAmount[i] := PayrollMonthlyTransactions.Amount;// - (1 / 3 * PayrollMonthlyTransactions.Amount);//Changed to cater for the 1/3 basic staff rule
                END;
            END;


            //  Error('Sal1 %1Sal2 %2Sal3 %3 SalStart%4 SalEnd%5', SalaryAmount[1], SalaryAmount[2], SalaryAmount[3], SalStart[i], SalEnd[i]);
            ...
            IF Members."Employer Code" = 'POSTAL CORP' THEN BEGIN
                ...
                SalaryProcessingLines.RESET;
                // SalaryProcessingLines.SETFILTER(SalaryProcessingLines."Posting Date", '%1..%2', CalcDate('-4M', today), Today);
                SalaryProcessingLines.SETRANGE(SalaryProcessingLines."FOSA Account No", AccountNo);
                SalaryProcessingLines.SETFILTER(SalaryProcessingLines."Salary Type", '%1|%2', SalaryProcessingLines."Salary Type"::Salary, SalaryProcessingLines."Salary Type"::Pension);
                IF SalaryProcessingLines.FindLast() THEN BEGIN
                    GrossSalary := SalaryProcessingLines."Gross Amount";
                    // ERROR('gROSS%1 aCCOUNT%2', GrossSalary,AccountNo);
                end;
            END;
    ...
    local procedure GetGrossSalaryDescription(AccountNo: Code[20]; ProductType: Code[10]) msg: Text[400]
    var
        saccoAccount: Record Vendor;
        SalaryProcessingLines: Record "Salary Details";
        PayrollMonthlyTransactions: Record "prPeriod Transactions.";
    ...
    begin
        ...
        END;
        // Error('Gross%1', GrossSalary);
        if GrossSalary = 0 then begin
            msg := 'You must have consecutive salaries for the past 3 months.';
        end else
            msg := '';
        EXIT(msg);
    end;
    ...
    procedure GetSalaryLoanQualifiedAmount(AccountNo: Code[20]; LoanProductType: Code[20]; var LoanLimit: Decimal; var Remark: Text[250]; appliedDuration: Integer): Decimal
    var
        LoanBalance: Decimal;
        ...
        PayrollMonthlyTransactionss: Record "prPeriod Transactions.";
        StaffDeduction: Decimal;
        LastPeriod: Date;
    begin
        MaxLoanAmount := 0;
        Remark := '';
        ...
        IF saccoAccount.FINDFIRST THEN BEGIN
            ...
            IF SaccoAcc.FINDFIRST THEN BEGIN
                IF SaccoAcc.Status <> SaccoAcc.Status::Active THEN BEGIN
                    LoanLimit := 0;
                    Remark := 'Your deposit contribution is inactive';
                    EXIT;
                END;
            END;
            ...
            IF LoanType.GET(LoanProductType) THEN BEGIN
                ...
                EmployerCode := Members."Employer Code";
                //Error('Return Complex');
                MaxLoanAmount := GetGrossSalary(AccountNo, LoanProductType);

                LoanLimit := 0;
                LoanRepaymentRecFromSal := 0;

                if Members."Employer Code" = 'STAFF' then begin
                    SalaryLoans.RESET;
                    SalaryLoans.SETRANGE("Client Code", saccoAccount."BOSA Account No");
                    SalaryLoans.SETFilter("Recovery Mode", '%1|%2', SalaryLoans."Recovery Mode"::Salary, SalaryLoans."Recovery Mode"::Checkoff);
                    SalaryLoans.SETFILTER("Outstanding Balance", '>0');
                    ...
                    END;
                    StaffDeduction := 0;
                    LastPeriod := calcdate('-1D', calcdate('-CM', Today));
                    PayrollMonthlyTransactionss.SETRANGE(PayrollMonthlyTransactionss."Employee Code", Members."Payroll No");
                    PayrollMonthlyTransactionss.SETRANGE(PayrollMonthlyTransactionss."Transaction Code", 'TOT-DED');
                    PayrollMonthlyTransactionss.SetFilter(PayrollMonthlyTransactionss."Payroll Period", '%1..%2', calcdate('-CM', LastPeriod), LastPeriod);
                    IF PayrollMonthlyTransactionss.FindLast() THEN begin
                        StaffDeduction := PayrollMonthlyTransactionss.Amount;
                    end;
                end else begin
                    SalaryLoans.RESET;
                ...
                END;
                IF Members."Employer Code" <> 'STAFF' THEN begin
                    SalaryPostingCharge := 144;
                end else begin
                    SalaryPostingCharge := 0;
                end;
                IF Members."Employer Code" <> 'STAFF' THEN begin
                    NetSalaryAmount := ((MaxLoanAmount * 0.72) - ((LoanRepaymentRecFromSal) + (STODeductions) + (SalaryPostingCharge)));
                END;
                IF Members."Employer Code" = 'STAFF' THEN begin
                    NetSalaryAmount := ((MaxLoanAmount) - ((StaffDeduction) + (1 / 3 * MaxLoanAmount) + (STODeductions) + (SalaryPostingCharge)));
                end;
                ...
                LoanLimit := ROUND(NetSalaryAmount / ((LoanType."Interest rate" / 12 / 100) / (1 - POWER((1 + (LoanType."Interest rate" / 12 / 100)), -appliedDuration))), 100, '<');
    ...
    procedure FnGetMobileAdvanceEligibility(AccountNo: Code[20]; LoanProductType: Code[20]; var Msg: Text[250]; var LoanLimit: Decimal; loanDur: Integer): Decimal
    var
        LoanBalance: Decimal;
        MaxLoanAmount: Decimal;
        saccoAccount: Record Vendor;
            ...
            IF LoanType.GET(LoanProductType) THEN BEGIN
                ...
                IF Loans.FINDSET THEN BEGIN
                    REPEAT
                        ...
                        LoanBalance += (Loans."Outstanding Balance" + loans."Outstanding Interest" + Loans."Outstanding Penalty");
                    UNTIL Loans.NEXT = 0;
                END;
                // Error('Internal Error');
                LoanRep += LoanBalance;

                MaxLoanAmount := LoanType."Maximimum Amount Non-Salaried";
                /* 
                                Loans.RESET;
                ...
                IF NOT CreditRating.FINDFIRST THEN BEGIN
                    ...
                    RatingLoanLimit := LoanType."Min. Loan Amount";
                END ELSE BEGIN
                    //Subsequent Loan
                    CreditRating.RESET;
                    CreditRating.SETCURRENTKEY("Date Entered");
                    ...
                    IF CreditRating.FINDLAST THEN BEGIN
                        IF CreditRating."Loan Limit" <= 0 THEN begin
                            // CreditRating."Loan Limit" := SaccoSetup."Initial Loan Limit";
                            CreditRating."Loan Limit" := LoanType."Min. Loan Amount";
                            RatingLoanLimit := LoanType."Min. Loan Amount";
                        end else begin
                            loanReg.Reset();
                            loanReg.SetRange("Loan  No.", CreditRating."Loan No.");
                            if loanReg.Find('-') then begin
                                loanReg.CalcFields("Total Outstanding Balance", "Last Pay Date");
                                if (loanReg."Expected Date of Completion" >= Today) then begin
                                    RatingLoanLimit := CreditRating."Loan Limit";
                                end else if (loanReg."Total Outstanding Balance" > 0) then begin
                                    RatingLoanLimit := CreditRating."Loan Limit";
                                end else begin
                                    RatingLoanLimit := CreditRating."Loan Limit" + LoanType."Loan Increment";
                                end;
                            end;
                            // loanReg.Reset();
                            // loanReg.SetRange("Loan  No.", CreditRating."Loan No.");
                            // loanReg.SetFilter("Total Outstanding Balance");
                            // if loanReg.Find('-') then begin
                            //     loanReg.CalcFields("Total Outstanding Balance", "Last Pay Date");
                            //     // gracePeriodDate := CalcDate('<7D>', loanReg."Expected Date of Completion");
                            //     // refDate := CalcDate('<90D>', loanReg."Last Pay Date");
                            //     if (loanReg."Expected Date of Completion" >= Today) and (loanReg."Total Outstanding Balance" >= 0) then begin
                            //         RatingLoanLimit := CreditRating."Loan Limit";
                            //         // end else if (loanReg."Total Outstanding Balance" = 0) and ((gracePeriodDate < loanReg."Last Pay Date") or (refDate > Today)) then begin
                            //         //     RatingLoanLimit := LoanType."Min. Loan Amount";
                            //     end else begin
                            //         RatingLoanLimit := CreditRating."Loan Limit" + LoanType."Loan Increment";
                            //     end;
                ...
                PenaltyCounter.SETRANGE(Reversed, false);
                IF PenaltyCounter.FINDLAST THEN BEGIN
                    PenaltyCounter.CalcFields("Date Penalty Paid");
                    ...
                    END ELSE BEGIN
                        ...
                        END ELSE BEGIN
                            ...
                            NumberOfMonths := 0;
                            IF TODAY > CALCDATE('<90D>', DayLoanPaid) THEN BEGIN
                                NumberOfMonths := ROUND(((TODAY - CALCDATE('<90D>', DayLoanPaid)) / 30), 1, '<');
                                // RatingLoanLimit := LoanType."Min. Loan Amount" + (LoanType."Loan Increment" * NumberOfMonths)
                                RatingLoanLimit := LoanType."Min. Loan Amount";
                            END;
                        END;
                    END;
                ...
                IF LoanProductType = 'A16' THEN BEGIN
                    ...
                    if Members."Employer Code" = 'STAFF' then begin
                        ...
                        SalaryLoans.SetFilter("Recovery Mode", '%1|%2', SalaryLoans."Recovery Mode"::Salary, SalaryLoans."Recovery Mode"::Checkoff);
                        SalaryLoans.SETFILTER("Outstanding Balance", '>0');
                        SalaryLoans.SETFILTER("Loan Product Type", '<> A16');
                        IF SalaryLoans.FINDSET THEN BEGIN
                            REPEAT
                                SalaryLoans.CALCFIELDS(SalaryLoans."Outstanding Balance");
                    ...
                    IF Members."Employer Code" = 'STAFF' THEN begin
                        StaffDeduction := 0;
                        LastPeriod := calcdate('-1D', calcdate('-CM', Today));
                        //  Error('Memb%1',saccoAccount."BOSA Account No");
                        ...
                        NetSalaryAmount := ((GrossSalaryAmount) - ((StaffDeduction) + (1 / 3 * GrossSalaryAmount) + (STODeductions) + (SalaryPostingCharge)));
                    end;
                    IF NetSalaryAmount < 0 THEN begin
                        NetSalaryAmount := 0;
                    end;

                    // Error('GrossSalary%1GrossSalary72%2LoanRepayments-Diff%3SameloanOut%4STo%5SalaryPostingCharge%6StaffDed%7', GrossSalaryAmount, (GrossSalaryAmount * 0.72), (LoanRepayments - Repaymentdiff), NetSalaryAmount, STODeductions, SalaryPostingCharge, StaffDeduction);

                    LoanLimit := ROUND(NetSalaryAmount / ((LoanType."Interest rate" / 12 / 100) / (1 - POWER((1 + (LoanType."Interest rate" / 12 / 100)), -loanDur//LoanType."Default Installements"
                    ))), 1000, '<');
        ...
        if LoansRegister.FindFirst() then begin
            if Members.get(LoansRegister."Client Code") then begin
                ...
                if LoansRegister."Requested Amount" <= GAmount then begin
                    ...
                    CheckIfMemberHasASimilarProduct(LoansRegister."Client Code", LoansRegister."Loan Product Type");
                    ...
                    LoansRegister."Disbursed By Time" := Time;
                    LoansRegister."Disbursed By" := UserId;
                    LoansRegister.Validate(LoansRegister."Loan Disbursement Date");
    ...
    local procedure FnOkayMember(MemberNo: Code[60]; AccType: Code[20]) Okayed: Boolean
    var
        Members: Record Vendor;
        GenJournalLine: Record "Gen. Journal Line";
        Guarantorss: record "Loans Guarantee Details";
    begin
        ...
        Members.SetFilter(Members."Account Type", AccType);
    ...
    begin
        ...
        if Members.FindFirst() then begin
            if Members."Registration Date" = 0D then begin
                Response := 'You are less than 6 months old in the SACCO';
            end;
            if Members."Registration Date" <> 0D then begin
        ...
        Vendor.SetRange(Vendor."Account Type", '101');
        if Vendor.FindFirst() then begin
            Vendor.CalcFields(Vendor.Balance);
            if Vendor.Balance < SaccoSetup."Retained Shares" then
                Response := 'You have not met the minimum share capital of Kshs.' + Format(SaccoSetup."Retained Shares") + '.';
        end else begin
            Response := 'Your Shares Capital Account was not found.';
        end;
    ...
    begin
        ...
        if Members.FindFirst() then begin
            IF (Members."Shares Retained" < SaccoGen."Retained Shares") THEN BEGIN
                Response := 'You have not met the minimum share capital of Kshs.' + Format(SaccoGen."Retained Shares") + '.';
            end;
        end;
    end;
    ...
    begin
        ...
        IF Loans.FINDFIRST THEN BEGIN
            LoanProduct.Get(Loans."Loan Product Type");
            Response := 'You have defaulted ' + LoanProduct."Product Description" + 'loan';
        end;
    end;
    ...
    begin
        ...
        Loans.SetAutoCalcFields(Loans."Total Outstanding Balance", loans."Outstanding Balance");
        Loans.SETFILTER(Loans."Total Outstanding Balance", '>%1', 0);
        IF Loans.FINDFIRST THEN BEGIN
            LoanProduct.Get(Loans."Loan Product Type");
            Response := 'You have an existing ' + LoanProduct."Product Description" + '.';
        end else begin
            vend.Reset();
            vend.SetRange("BOSA Account No", ClientCode);
            vend.SetRange("Account Type", '103');
            if vend.Find('-') then begin
                Response := GetGrossSalaryDescription(vend."No.", ProductCode);
            end;
        end;
    ...
    procedure FnCheckLoanIfPendingGuar(ClientCode: Code[40]; ProductCode: Code[40]) Response: Text[400]
    var
        Members: Record Customer;
        Loans: Record "Loans Register";
    ...
    begin
        Response := '';
        // loanGuar.Reset();
        // loanGuar.SetRange("Member No", ClientCode);
        // loanGuar.SetRange(loanGuar."Acceptance Status", loanGuar."Acceptance Status"::Pending);
        // if loanGuar.Find('-') then begin
        Loans.RESET;
        Loans.SETRANGE(Loans."Client Code", ClientCode);
        Loans.SETFILTER(Loans."Loan Product Type", ProductCode);
        Loans.SETRange(Loans."Archive Loan", false);
        Loans.SetRange(Loans."Requires Guarantors Mobile", true);
        loans.SetRange(loans.Posted, false);
        Loans.SetRange(loans."Approval Status", loans."Approval Status"::Pending);
        IF Loans.FINDFIRST THEN BEGIN
            LoanProduct.Get(Loans."Loan Product Type");
            Response := 'You have a pending ' + LoanProduct."Product Description" + '. Kindly reject or add guarantors.';
        end;
        // end;
    ...
    local procedure FnCheckLoanIfExistingGuarantor(ClientCode: Code[40]; ProductCode: Code[40]) Response: Boolean
    var
        Members: Record Customer;
        Loans: Record "Loans Register";
        LoanProduct: Record "Loan Products Setup";
    begin
        ...
        Loans.SETRANGE(Loans."Client Code", ClientCode);
        Loans.SETFILTER(Loans."Loan Product Type", ProductCode);
        Loans.SetAutoCalcFields(Loans."Outstanding Balance", Loans."Total Outstanding Balance");
        Loans.SETFILTER(Loans."Total Outstanding Balance", '>%1', 0);
        IF Loans.FINDFIRST THEN BEGIN
            LoanProduct.Get(Loans."Loan Product Type");
            Response := true;
    ...
    procedure GetReloadedLoanQualifiedAmount(AccountNo: Code[20]; LoanProductType: Code[20]; var LoanLimit: Decimal; var Remark: Text[250]; loanDuration: integer): Decimal
    var
        LoanBalance: Decimal;
        MaxLoanAmount: Decimal;
        saccoAccount: Record Vendor;
        ...
        IF saccoAccount.FINDFIRST THEN BEGIN
            ...
            IF LoanType.GET(LoanProductType) THEN BEGIN
                ...
                if Members."Employer Code" = 'STAFF' then begin
                    ...
                    SalaryLoans.SETRANGE("Client Code", saccoAccount."BOSA Account No");
                    SalaryLoans.SETFilter("Recovery Mode", '%1|%2', SalaryLoans."Recovery Mode"::Salary, SalaryLoans."Recovery Mode"::Checkoff);
                    SalaryLoans.SetAutoCalcFields(SalaryLoans."Outstanding Balance");
                    SalaryLoans.SETFILTER("Outstanding Balance", '>0');
                    IF SalaryLoans.FINDSET THEN BEGIN
                        REPEAT
                            SalaryLoans.CALCFIELDS(SalaryLoans."Outstanding Balance");
                ...
                //Error('TotalLimit%1', LoanRepaymentRecFromSal);
                ...
                IF Members."Employer Code" <> 'STAFF' THEN begin
                    NetSalaryAmount := ((MaxLoanAmount * (LoanType."Loan Appraisal %") / 100) - ((LoanRepaymentRecFromSal) + (STODeductions) + (SalaryPostingCharge)));
                end;
                IF Members."Employer Code" = 'STAFF' THEN begin
                    NetSalaryAmount := ((MaxLoanAmount) - ((StaffDeduction) + (1 / 3 * MaxLoanAmount) + (STODeductions) + (SalaryPostingCharge)));
                end;
                ...
                //Error('Net%1Baisic%2Sto%3Charge%4#rd%5Loan%6deduction%7', NetSalaryAmount, MaxLoanAmount, STODeductions, SalaryPostingCharge, 1 / 3 * MaxLoanAmount, LoanRepaymentRecFromSal, StaffDeduction);
                CoopSetup.RESET;
                CoopSetup.SETRANGE("Transaction Type", CoopSetup."Transaction Type"::"Loan Disbursement");
                IF CoopSetup.FINDFIRST THEN BEGIN
                    // GetCharges(CoopSetup."Transaction Type", VendorCommission, SaccoFee, Safcom, 0);
                ...
                LoanLimit := ROUND(NetSalaryAmount / ((LoanType."Interest rate" / 12 / 100) / (1 - POWER((1 + (LoanType."Interest rate" / 12 / 100)), -loanDuration))), 1000, '<');
    ...
    procedure GetOverdraftLoanQualifiedAmount(AccountNo: Code[20]; LoanProductType: Code[20]; var LoanLimit: Decimal; var Remark: Text[250]): Decimal
    var
        LoanBalance: Decimal;
        MaxLoanAmount: Decimal;
        saccoAccount: Record vendor;
    ...
    begin
        ...
        IF saccoAccount.FINDFIRST THEN BEGIN
            ...
            IF LoanType.GET(LoanProductType) THEN BEGIN
                ...
                LoanLimit := ROUND(((NetSalaryAmount) - ((LoanRepayments) + (STODeductions) + (SalaryPostingCharge))), 100, '<');
