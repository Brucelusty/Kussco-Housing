//************************************************************************
Codeunit 50070 "WorkflowIntegration"
{
    trigger OnRun()
    begin
    ...
    procedure CheckMemberAllowanceWorkflowEnabled(var membAllowance: Record "Sacco Meetings"): Boolean
    begin
        if not IsMemberAllowanceWorkflowEnabled(membAllowance) then
            Error(NoWorkflowEnabledErr);

        exit(true);
    end;

    procedure IsMemberAllowanceWorkflowEnabled(var membAllowance: Record "Sacco Meetings"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(membAllowance, SurestepWFEvents.RunWorkflowOnSendMemberAllowanceForApprovalCode));
    end;

    [IntegrationEvent(false, false)]
    procedure OnSendMemberAllowanceForApproval(var membAllowance: Record "Sacco Meetings")
    begin
    end;

    [IntegrationEvent(false, false)]
    procedure OnCancelMemberAllowanceRequest(var membAllowance: Record "Sacco Meetings")
    begin
    end;



    procedure CheckFileMovementApprovalsWorkflowEnabled(var FileMovement: Record "File Movement Header"): Boolean
    begin
        if not IsFileMovementApprovalsWorkflowEnabled(FileMovement) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;

    procedure IsFileMovementApprovalsWorkflowEnabled(var FileMovement: Record "File Movement Header"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(FileMovement, SurestepWFEvents.RunWorkflowOnSendFileMovementForApprovalCode));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendFileMovementForApproval(var FileMovement: Record "File Movement Header")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelFileMovementApprovalRequest(var FileMovement: Record "File Movement Header")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnRejectFileMovementApprovalRequest(var FileMovement: Record "File Movement Header")
    begin
    end;



    procedure CheckSavingsVarApprovalsWorkflowEnabled(var savingsVar: Record "Savings Variation"): Boolean
    begin
        if not IsSavingsVarApprovalsWorkflowEnabled(savingsVar) then
            Error(NoWorkflowEnabledErr);
    ...
    procedure IsSavingsVarApprovalsWorkflowEnabled(var savingsVar: Record "Savings Variation"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(savingsVar, SurestepWFEvents.RunWorkflowOnSendSavingsVarDocForApprovalCode));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendSavingsVarDocForApproval(var savingsVar: Record "Savings Variation")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelSavingsVarApprovalRequest(var savingsVar: Record "Savings Variation")
    begin
    end;


    procedure CheckLoanLimitApprovalsWorkflowEnabled(var creditRating: Record "Credit Rating"): Boolean
    begin
        if not IsLoanLimitApprovalsWorkflowEnabled(creditRating) then
            Error(NoWorkflowEnabledErr);

        exit(true);
    end;

    procedure IsLoanLimitApprovalsWorkflowEnabled(var creditRating: Record "Credit Rating"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(creditRating, SurestepWFEvents.RunWorkflowOnSendLoanLimitDocForApprovalCode));
    end;

    [IntegrationEvent(false, false)]
    procedure OnSendLoanLimitDocForApproval(var creditRating: Record "Credit Rating")
    begin
    end;

    [IntegrationEvent(false, false)]
    procedure OnCancelLoanLimitApprovalRequest(var creditRating: Record "Credit Rating")
    begin
    end;
    ...
    procedure CheckLeaveSupWorkflowEnabled(var leaveSup: Record "Leave Supervisor Approval"): Boolean
    begin
        if not IsLeaveSupWorkflowEnabled(leaveSup) then
            Error(NoWorkflowEnabledErr);
    ...
    procedure IsLeaveSupWorkflowEnabled(var leaveSup: Record "Leave Supervisor Approval"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(leaveSup, SurestepWFEvents.RunWorkflowOnSendLeaveSuperForApprovalCode));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendLeaveSuperForApproval(var leaveSup: Record "Leave Supervisor Approval")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelLeaveSuperApprovalRequest(var leaveSup: Record "Leave Supervisor Approval")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnRejectLeaveSuperApprovalRequest(var leaveSup: Record "Leave Supervisor Approval")
    begin
    end;



    procedure CheckStaffPerformanceAppraisalWorkflowEnabled(var pAppraisal: Record "HR Appraisal Header"): Boolean
    begin
        if not IsStaffPerformanceAppraisalWorkflowEnabled(pAppraisal) then
            Error(NoWorkflowEnabledErr);
    ...
    procedure IsStaffPerformanceAppraisalWorkflowEnabled(var pAppraisal: Record "HR Appraisal Header"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(pAppraisal, SurestepWFEvents.RunWorkflowOnSendStaffPerformanceAppraisalForApprovalCode));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendForStaffPerformanceAppraisalApproval(var pAppraisal: Record "HR Appraisal Header")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelStaffPerformanceAppraisalApprovalRequest(var pAppraisal: Record "HR Appraisal Header")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnRejectStaffPerformanceAppraisalApprovalRequest(var pAppraisal: Record "HR Appraisal Header")
    begin
    end;


    procedure CheckFRFeePayApprovalsWorkflowEnabled(var FRFeePay: Record "Funeral Rider Processing"): Boolean
    begin
        if not IsFRFeePayApprovalsWorkflowEnabled(FRFeePay) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;

    procedure IsFRFeePayApprovalsWorkflowEnabled(var FRFeePay: Record "Funeral Rider Processing"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(FRFeePay, SurestepWFEvents.RunWorkflowOnSendFRFeePayForApprovalCode));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendFRFeePayForApproval(var FRFeePay: Record "Funeral Rider Processing")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelFRFeePayApprovalRequest(var FRFeePay: Record "Funeral Rider Processing")
    begin
    end;



    procedure CheckMobileApplicationApprovalsWorkflowEnabled(var MobileApplication: Record "MOBILE Applications"): Boolean
    begin
        if not IsMobileApplicationApprovalsWorkflowEnabled(MobileApplication) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;
    procedure IsMobileApplicationApprovalsWorkflowEnabled(var MobileApplication: Record "MOBILE Applications"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(MobileApplication, SurestepWFEvents.RunWorkflowOnSendMobileApplicationForApprovalCode));
    end;
    ...
    [IntegrationEvent(false, false)]
    procedure OnCancelMobileApplicationApprovalRequest(var MobileApplication: Record "MOBILE Applications")
    begin
    end;

    [IntegrationEvent(false, false)]
    procedure OnRejectMobileApplicationApprovalRequest(var MobileApplication: Record "MOBILE Applications")
    begin
    end;
    ...
    procedure CheckLoanAppliedApprovalsWorkflowEnabled(var LoanApplied: Record "Loans Register"): Boolean
    begin

        if not IsLoanAppliedApprovalsWorkflowEnabled(LoanApplied) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;


    procedure IsLoanAppliedApprovalsWorkflowEnabled(var LoanApplied: Record "Loans Register"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(LoanApplied, SurestepWFEvents.RunWorkflowOnSendLoanAppliedForApprovalCode));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendLoanAppliedForApproval(var LoanApplied: Record "Loans Register")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelLoanAppliedApprovalRequest(var LoanApplied: Record "Loans Register")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnRejectLoanAppliedApprovalRequest(var LoanApplied: Record "Loans Register")
    begin
    end;


    procedure CheckLoanApplicationsApprovalsWorkflowEnabled(var LoanApplications: Record "Loans Register"): Boolean
    begin

        if not IsLoanApplicationsApprovalsWorkflowEnabled(LoanApplications) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;


    procedure IsLoanApplicationsApprovalsWorkflowEnabled(var LoanApplications: Record "Loans Register"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(LoanApplications, SurestepWFEvents.RunWorkflowOnSendLoanApplicationsForApprovalCode));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendLoanApplicationsForApproval(var LoanApplications: Record "Loans Register")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelLoanApplicationsApprovalRequest(var LoanApplications: Record "Loans Register")
    begin
    end;
    ...
    procedure OnRejectLoanDisbursementApprovalRequest(var LoanDisbursement: Record "Loan Disburesment-Batching")
    begin
    end;
    ...
    procedure CheckSTOApprovalsWorkflowEnabled(var STO: Record "Standing Orders"): Boolean
    begin
        if not IsStandingOrderApprovalsWorkflowEnabled(STO) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;

    procedure IsSTOApprovalsWorkflowEnabled(var STO: Record "Standing Orders"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(STO, SurestepWFEvents.RunWorkflowOnSendSTOForApprovalCode));
    end;

    [IntegrationEvent(false, false)]
    procedure OnSendSTOForApproval(var STO: Record "Standing Orders")
    begin
    end;

    [IntegrationEvent(false, false)]
    procedure OnCancelSTOApprovalRequest(var STO: Record "Standing Orders")
    begin
    end;
    ...
    procedure OnRejectLeaveApplicationApprovalRequest(var LeaveApplication: Record "HR Leave Application")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    procedure OnRejectEFTRTGSApprovalRequest(var EFTRTGS: Record "EFT/RTGS Header")
    begin
    end;
    ...
    procedure CheckBOSAReceiptApprovalsWorkflowEnabled(var BOSAReceipt: Record "Receipts & Payments"): Boolean
    begin
        if not IsBOSAReceiptApprovalsWorkflowEnabled(BOSAReceipt) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;

    procedure IsBOSAReceiptApprovalsWorkflowEnabled(var BOSAReceipt: Record "Receipts & Payments"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(BOSAReceipt, SurestepWFEvents.RunWorkflowOnSendBOSAReceiptForApprovalCode));
    end;

    [IntegrationEvent(false, false)]
    procedure OnSendBOSAReceiptForApproval(var BOSAReceipt: Record "Receipts & Payments")
    begin
    end;

    [IntegrationEvent(false, false)]
    procedure OnCancelBOSAReceiptApprovalRequest(var BOSAReceipt: Record "Receipts & Payments")
    begin
    end;

    [IntegrationEvent(false, false)]
    procedure OnRejectBOSAReceiptApprovalRequest(var BOSAReceipt: Record "Receipts & Payments")
    begin
    end;
    ...
    var
        ...
        supervisorLeave: Record "Leave Supervisor Approval";
        ...
        memberAllowance: Record "Sacco Meetings";
        ...
        case RecRef.Number of
            ...
            Database::"Payments Header":
                begin
                    ...
                    // := PaymentHeader."Bank Account Name";
            ...
            Database::"Sacco Meetings":
                begin
                    RecRef.SetTable(memberAllowance);
                    memberAllowance.CalcFields("Total Actual Allowance");
                    ApprovalAmount := memberAllowance."Total Actual Allowance";
                    ApprovalAmountLCY := memberAllowance."Total Actual Allowance";
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."Document Type"::MemberAllowance;
                    ApprovalEntryArgument."Document No." := memberAllowance."Meeting No";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                end;
            Database::"Savings Variation":
                begin
                    ...
                    ApprovalAmount := savingsVar."New Savings";
                    ApprovalAmountLCY := savingsVar."New Savings";
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::SavingsVariation;
                    ApprovalEntryArgument."Document No." := savingsVar."Variation No";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                end;
            ...
            Database::"Funeral Rider Processing":
                begin
                    ...
                    ApprovalAmount := 0;
                    ApprovalAmountLCY := 0;
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::FRFeePay;
                    ApprovalEntryArgument."Document No." := FRFeePay."FR No.";
                    // := MembershipApplication."No.";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                    ApprovalEntryArgument."Currency Code" := '';
            ...
            Database::"Leave Supervisor Approval":
                begin
                    RecRef.SetTable(supervisorLeave);
                    ApprovalAmount := 0;
                    ApprovalAmountLCY := 0;
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::LeaveSupervisor;
                    ApprovalEntryArgument."Document No." := Format(supervisorLeave."No.");
                    // := MembershipApplication."No.";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                    ApprovalEntryArgument."Currency Code" := '';
            ...
            Database::"MOBILE Applications":
                begin
                    ...
                    ApprovalAmount := 0;
                    ApprovalAmountLCY := 0;
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::MobileApplication;
            ...
            Database::"Loans Register":
                begin
                    ...
                    ApprovalAmount := LoanApplied."Requested Amount";
                    ApprovalAmountLCY := LoanApplied."Approved Amount";
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::LoanApplication;
                    ApprovalEntryArgument."Document No." := LoanApplied."Loan  No.";
                    // := LoanApplication."Client Code";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                    ApprovalEntryArgument."Currency Code" := LoanApplied."Product Currency Code";
                end;
            Database::"Receipts & Payments":
                begin
                    RecRef.SetTable(bosaReceipt);
                    bosaReceipt.CalcFields("Allocated Amount");
                    ApprovalAmount := bosaReceipt."Allocated Amount";
                    ApprovalAmountLCY := bosaReceipt."Allocated Amount";
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::BOSAReceipt;
                    ApprovalEntryArgument."Document No." := bosaReceipt."Transaction No.";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                    ApprovalEntryArgument."Currency Code" := '';
                end;
            ...
            Database::"Guarantorship Substitution H":
                begin
                    RecRef.SetTable(GuarSub);
                    GuarSub.CalcFields("Amount Guaranteed");
                    ApprovalAmount := GuarSub."Amount Guaranteed";
                    ApprovalAmountLCY := GuarSub."Amount Guaranteed";
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::GuarantorSub;
                    ApprovalEntryArgument."Document No." := GuarSub."Document No";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                    ApprovalEntryArgument."Currency Code" := '';
                end;
            ...
            Database::"Membership Exist":
                begin
                    ...
                    ApprovalAmount := MWithdrawal."Amount To Disburse";
                    ApprovalAmountLCY := MWithdrawal."Amount To Disburse";
            ...
            Database::"ATM Card Applications":
                begin
                    ...
                    if ATMCard."Request Type"= ATMCard."Request Type"::"Pin Replacement" then begin
                        ApprovalAmount := 0;
                        ApprovalAmountLCY := 0;
                        ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::ATMPinReplacement;
            ...
            Database::"Treasury Transactions":
                begin
                    ...
                    //// := TTransactions."From Account Name" + ' to ' + TTransactions."To Account Name";
                    ApprovalEntryArgument.Amount := 0;
                    ApprovalEntryArgument."Amount (LCY)" := 0;
                    ApprovalEntryArgument."Currency Code" := '';
                end;
            ...
            Database::"Sacco Transfers":
                begin
                    ...
                    // := SaccoTransfer."Source Account Name";
            ...
            Database::"Cheque Discounting":
                begin
                    ...
                    // := ChequeDiscounting."Account Name";
            ...
            Database::"Imprest Surrender Header":
                begin
                    ...
                    // := ImprestSurrender."Received From";
            ...
            Database::"Credit Rating":
                begin
                    ...
                    ApprovalAmount := 0;
                    ApprovalAmountLCY := 0;
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::CreditRating;
                    ApprovalEntryArgument."Document No." := creditRating."Loan No.";
                    ApprovalEntryArgument.Amount := creditRating."New Limit";
                    ApprovalEntryArgument."Amount (LCY)" := 0;
                    ApprovalEntryArgument."Currency Code" := '';
                end;
            ...
            Database::"Member Agent/Next Of Kin Chang":
                begin
                    ...
                    // := MemberAgentNOKChange."Member Name";
                    ApprovalEntryArgument.Amount := 0;
                    ApprovalEntryArgument."Amount (LCY)" := 0;
                    ApprovalEntryArgument."Currency Code" := '';
                end;
            ...
            Database::"House Groups Registration":
                begin
                    ...
                    // := HouseRegistration."House Group Name";
                    ApprovalEntryArgument.Amount := 0;
                    ApprovalEntryArgument."Amount (LCY)" := 0;
                    ApprovalEntryArgument."Currency Code" := '';
                end;
            ...
            Database::"EFT/RTGS Header":
                begin
                    ...
                    EFTRTGS.CalcFields(Total);
                    ApprovalAmount := EFTRTGS.Total;
                    ApprovalAmountLCY := EFTRTGS.Total;
                    if ApprovalAmount >= 1000000 then begin
                        ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::RTGS;
                    end else ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::EFT;
            ...
            Database::"File Movement Header":
                begin
                    ...
                    ApprovalAmount := 0;
                    ApprovalAmountLCY := 0;
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::FileMovement;
                    ApprovalEntryArgument."Document No." := fileMove."No.";
                    // := MembershipApplication."No.";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                    ApprovalEntryArgument."Currency Code" := '';
            ...
            Database::"OverDraft Application":
                begin
                    ...
                    // := OverDraft."Over Draft Account Name";
            ...
            Database::"Member Sweeping Instructions":
                begin
                    ...
                    // := SweepingInstructions."Member Name";
            ...
            Database::"Cheque Receipts-Family":
                begin
                    ...
                    // := Format(InwardChequeClearing."Transaction Date", 0, '<Day,2> <Month Text,3> <Year4>') + ' Inwards';
                    ApprovalEntryArgument.Amount := 0;
                    ApprovalEntryArgument."Amount (LCY)" := 0;
                    ApprovalEntryArgument."Currency Code" := '';
                end;
            ...
            Database::"Paybill Processing Header":
                begin
                    ...
                    // := InvalidPaybillTransactions.Remarks;
            ...
            Database::"Internal PV Header":
                begin
                    ...
                    // := InternalPV."Transaction Description";
            ...
            Database::"Salary Processing Headerr":
                begin
                    ...
                    // := SProcessing."Transaction Description";
            ...
            Database::"Member Exit Batch":
                begin
                    RecRef.SetTable(exitBatch);
                    exitBatch.CalcFields("Total to Disburse");
                    ApprovalAmount := exitBatch."Total to Disburse";
                    ApprovalAmountLCY := exitBatch."Total to Disburse";
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::ExitBatch;
                    ApprovalEntryArgument."Document No." := exitBatch."Exit Batch No.";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                    ApprovalEntryArgument."Currency Code" := '';
                end;
            ...
            Database::"BOD Honoraria":
                begin
                    ...
                    honorarium.CalcFields(honorarium."Total Amount");
                    ApprovalAmount := honorarium."Total Amount";
                    ApprovalAmountLCY := honorarium."Total Amount";
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::BODHonorarium;
                    ApprovalEntryArgument."Document No." := honorarium."Honoraria No";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                end;
            ...
            Database::"ESS Refund Batch":
                begin
                    ...
                    ESSRefundBatch.CalcFields(ESSRefundBatch."Total Disbursed");
                    ApprovalAmount := ESSRefundBatch."Total Disbursed";
                    ApprovalAmountLCY := ESSRefundBatch."Total Disbursed";
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::ESSRefund;
                    ApprovalEntryArgument."Document No." := ESSRefundBatch."ESSRef Batch No.";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                end;
            ...
            Database::"HR Appraisal Header":
                begin
                    ...
                    ApprovalAmount := 0;
                    ApprovalAmountLCY := 0;
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::"Performance Appraisal";
                    ApprovalEntryArgument."Document No." := pAppraisal."No.";
                    ApprovalEntryArgument.Amount := ApprovalAmount;
                    ApprovalEntryArgument."Amount (LCY)" := ApprovalAmountLCY;
                end;
    ...
    begin
     ...
     procedure CheckMemberExitBatchApprovalsWorkflowEnabled(var MemberExitBatch: Record "Member Exit Batch"): Boolean
    begin
        if not IsMemberExitBatchApprovalsWorkflowEnabled(MemberExitBatch) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;


    procedure IsMemberExitBatchApprovalsWorkflowEnabled(var MemberExitBatch: Record "Member Exit Batch"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(MemberExitBatch, SurestepWFEvents.RunWorkflowOnSendMemberExitBatchForApprovalCode));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendMemberExitBatchForApproval(var MemberExitBatch: Record "Member Exit Batch")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelMemberExitBatchApprovalRequest(var MemberExitBatch: Record "Member Exit Batch")
    begin
    end;


     procedure CheckESSRefundBatchApprovalsWorkflowEnabled(var ESSRefundBatch: Record "ESS Refund Batch"): Boolean
    begin
        if not IsESSRefundBatchApprovalsWorkflowEnabled(ESSRefundBatch) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;

    procedure IsESSRefundBatchApprovalsWorkflowEnabled(var ESSRefundBatch: Record "ESS Refund Batch"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(ESSRefundBatch, SurestepWFEvents.RunWorkflowOnSendESSRefundBatchForApprovalCode));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendESSRefundBatchForApproval(var ESSRefundBatch: Record "ESS Refund Batch")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelESSRefundBatchApprovalRequest(var ESSRefundBatch: Record "ESS Refund Batch")
    begin
    end;


    procedure CheckBODHonorariumApprovalsWorkflowEnabled(var honorarium: Record "BOD Honoraria"): Boolean
    begin
        if not IsBODHonorariumApprovalsWorkflowEnabled(honorarium) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;


    procedure IsBODHonorariumApprovalsWorkflowEnabled(var honorarium: Record "BOD Honoraria"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(honorarium, SurestepWFEvents.RunWorkflowOnSendBODHonorariumForApprovalCode));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendBODHonorariumForApproval(var honorarium: Record "BOD Honoraria")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelBODHonorariumApprovalRequest(var honorarium: Record "BOD Honoraria")
    begin
    end;
