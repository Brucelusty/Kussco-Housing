...
Codeunit 50038 "CloudPESALivetest"
{
    ...
    var
        ...
        SurePESATrans: Record "Mobile Banking Transactions";
                ...
                IF objvendor2.FIND('-') THEN BEGIN
                    IF (TempBalance > amount + MobileCharges + SurePESACharge) THEN BEGIN
                        ...
                        msg := 'You have transfered KES ' + FORMAT(amount) + ' from Account ' + objvendor."Account Type" + ' to ' + objvendor2."Account Type" +
                         ' .Thank you for using Test Sacco Mobile.';
                    ...
                    if (accTo = 'Share capital') or (accTo = 'Deposit Contribution') or (accTo = 'Benevolent Fund') or (accTo = 'Insurance Fund')
                      then begin
                        ...
                        msg := 'You have transfered KES ' + Format(amount) + ' from Account ' + accFrom + ' to ' + accTo +
                         ' .Thank you for Test Sacco Mobile.';
                        SMSMessage(DocNumber, accFrom, objvendor."Mobile Phone No", msg, '');
                    end
                    else begin
                        result := 'ACC2INEXISTENT';
    ...
    procedure AdvanceEligibility(account: Text[50]; period: Integer) Res: Text
    var
    ...
    begin
        ...
        if DateRegistered <> 0D then begin
            ...
            if CalcDate('6M', DateRegistered) > Today then begin
                ...
                Res := '{"success":false,';
                Res += '"description":"You do not Qualify for this loan because you should be a member for last 6 Months",';
                Res += '"amount":0}';
            end;
        end;
            ...
            if LoansRegister.Find('-') then begin
                repeat
                    ...
                    if (LoansRegister."Outstanding Balance" > 0) then begin
                        ...
                        if (LoansRegister."Loan Product Type" = 'M-EPA') then begin
                            amount := 2;
                            //Res := '2::::You do not Qualify for this loan because You have an outstanding M-JPESA Loan::::False';
                            Res := '{"success":false,';
                            Res += '"description":"You do not Qualify for this loan because You have an outstanding M-EPA Loan",';
                            Res += '"amount":0}';
                            EXIT
                        end;
           ...
           */
            ...
            if amount <> 2 then begin
                if amount <> 3 then begin
                    ...
                    FreeShares := GetCharge(Members."Current Shares", 'MOBILIMIT');

                    amount := FreeShares;

                    //==================================================Get maximum loan amount
                    LoanProductsSetup.Reset;
                    LoanProductsSetup.SetRange(LoanProductsSetup.Code, 'M-EPA');
                    if LoanProductsSetup.Find('-') then begin
                ...
                if amount > 0 then begin
                    ...
                    Res := '{"success":false,';
                    Res += '"description":"Your do not qualify for this loan, kindly contact sacco for more info",';
                    Res += '"amount":0}';
                end;
            end;
        end;
    ...
    procedure AdvanceEligibilitySalary(account: Text[50]; period: Integer) Res: Text
    var
        StoDedAmount: Decimal;
        STO: Record "Standing Orders";
        FOSALoanRepayAmount: Decimal;
    ...
    begin
        ...
        IF Vendor.FIND('-') THEN BEGIN
            ...
            IF (Vendor."Salary Processing" = FALSE) THEN BEGIN
                amount := 1;
                Res := '{"success":false,';
                Res += '"description":"Your salary ids not processed through FOSA Account",';
                Res += '"amount":0}';
                EXIT;
            END
            ELSE BEGIN
                ...
                IF LoansRegister.FIND('-') THEN BEGIN
                    REPEAT
                        ...
                        IF (LoansRegister."Outstanding Balance" > 0) THEN BEGIN
                            //CHECK EXCEMPTIONS
                            IF (LoansRegister."Loan Product Type" = 'M-BANKING ADVANCE') OR (LoansRegister."Loan Product Type" = '555') OR (LoansRegister."Loan Product Type" = '551') THEN BEGIN
                                amount := 3;
                                Res := '{"success":false,';
                                Res += '"description":"You have outstanding salary advance of Ksh. ' + format(LoansRegister."Outstanding Balance") + '",';
                                Res += '"amount":0}';
                                EXIT;
                            end;
                            ...
                            IF (LoansRegister."Loans Category" <> LoansRegister."Loans Category"::Perfoming) THEN
                                IF (LoansRegister."Loans Category" <> LoansRegister."Loans Category"::Watch) THEN BEGIN
                                    amount := 4;
                                    Res := '{"success":false,';
                                    Res += '"description":"You do not qualify,You have loan in Arraers ",';
                                    Res += '"amount":0}';
                                    EXIT;
                                END;
                            ...
                            //Check outstanding elaon
                            IF (LoansRegister."Loan Product Type" = 'M-BANKING ADVANCE') THEN BEGIN
                                amount := 2;
                                EXIT;
                            END;
                ...
                IF amount <> 2 THEN BEGIN
                    IF amount <> 3 THEN BEGIN
                        ...
                        STO.SETRANGE(STO."Standing Order Dedution Type", STO."Standing Order Dedution Type"::Salary);
                        ...
                        Vledger.RESET;
                        Vledger.SETASCENDING(Vledger."Entry No.", FALSE);
                        Vledger.SETRANGE(Vledger."Vendor No.", Vendor."No.");
                        Vledger.SETFILTER(Vledger.Reversed, '<>%1', TRUE);
                        Vledger.SETRANGE(Vledger."Journal Batch Name", 'SALARIES');
                        Vledger.SETRANGE(Vledger.Positive, FALSE);
                        Vledger.SETFILTER(Vledger."Posting Date", FORMAT(CALCDATE('-3M', CALCDATE('CM+1D-2M', TODAY))) + '..' + FORMAT(CALCDATE('CM-1M', TODAY)));
                        Vledger.SETFILTER(Vledger."Credit Amount", '<>%1', 0);
                        //  Vledger.SETFILTER(Vledger.Description,'=%1','*SALARY*');
                        IF (Vledger.FIND('-')) THEN BEGIN

                            RCount := Vledger.COUNT;

                            MESSAGE(FORMAT(RCount));
                            TCount := 0;
                            Sal1 := 0;
                            Sal2 := 0;
                            Sal3 := 0;
                            IF RCount > 2 THEN BEGIN
                                REPEAT
                                ...
                                IF (Sal1 < Sal2) AND (Sal1 < Sal3) THEN
                                    SalaryAverage := Sal1
                                ELSE
                                    IF (Sal2 < Sal1) AND (Sal2 < Sal3) THEN
                                        SalaryAverage := Sal2
                                    ELSE
                                        SalaryAverage := Sal3;
                            END
                        ...
                        LoanProductsSetup.RESET;
                        LoanProductsSetup.SETRANGE(LoanProductsSetup.Code, 'M-BANKING ADVANCE');
                        IF LoanProductsSetup.FINDFIRST() THEN BEGIN
                            interestRate := LoanProductsSetup."Interest rate";
                        END;

                        AdvQualificationAmount := SalaryAverage - (RepayedLoanAmt + StoDedAmount) - MonthlyLoanRepayments;
                        interestAMT := (interestRate / 100) * amount;
                        amount := AdvQualificationAmount - interestAMT;
                        IF amount <= 0 THEN BEGIN
                            amount := 5;
                            Res := '{"success":false,';
                            Res += '"description":"You do not qualify for this loan, your salary minus deductions cannot qualify for this loan",';
                            Res += '"amount":0}';
                        END ELSE BEGIN
    ...
    procedure PostAdvanceLoan(docNo: Code[20]; AccountNo: Code[50]; amount: Decimal; Period: Decimal) result: Code[30]
    var
        LoanAcc: Code[30];
        InterestAcc: Code[30];
        InterestAmount: Decimal;
    ...
    begin
        ...
        ELSE BEGIN
            ...
            Vendor.RESET;
            Vendor.SETRANGE(Vendor."Phone No.", AccountNo);
            ...
            IF Vendor.FIND('-') THEN BEGIN
                ...
                IF Members.FIND('-') THEN BEGIN
                    ...
                    IF LoansRegister.FIND('-') THEN BEGIN
                        ...
                        GenJournalLine."Posting Date" := TODAY;
                        GenJournalLine.Description := 'Mobile Loan Disbursment-' + LoansRegister."Client Code" + LoansRegister."Client Name";
                        GenJournalLine.Amount := amount;
                        GenJournalLine.VALIDATE(GenJournalLine.Amount);
                        IF GenJournalLine.Amount <> 0 THEN
                            GenJournalLine.INSERT;
                        ...
                        GenJournalLine."Document No." := LoansRegister."Loan  No.";
                        GenJournalLine."Posting Date" := TODAY;
                        GenJournalLine.Description := 'Mobile Loan Charge-' + LoansRegister."Client Code" + LoansRegister."Client Name";
                        ...
                        GenJournalLine."Posting Date" := TODAY;
                        GenJournalLine."Loan No" := LoansRegister."Loan  No.";
                        GenJournalLine.Description := 'Loan ' + FORMAT(GenJournalLine."Transaction Type"::"Loan Insurance Charged") + LoansRegister."Client Code";
                        GenJournalLine.Amount := InterestAmount;
                        GenJournalLine.VALIDATE(GenJournalLine.Amount);
        ...
        if SurePESATrans.Find('-') then begin
            ...
            */
            ...
            if Members.Find('-') then begin
                ...
                NoSeriesMgt.GetNextNo(SaccoNoSeries."BOSA Loans Nos");
...
*/
    ...
    begin
        ...
        if CloudPESATrans.Find('-') then begin
            ...
            CloudPESACommACC := '30632';// GenLedgerSetup."Mobile Comm Account";
            MPESARecon := 'BANK_0014';// GenLedgerSetup."MPESA Recon Acc";
            MobileChargesACC := '30632';//Charges."GL Account";
