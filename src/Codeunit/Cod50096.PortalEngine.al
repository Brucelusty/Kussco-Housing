    ...
    begin
        ...
        if IdentifierType = 'MSISDN' THEN BEGIN
            ...
            if Memb.FindFirst() then begin
                ...
                // end else begin
                //     IdentifierTwo := DelChr(Identifier, '=', '+');

                //     Memb.Reset();
                //     Memb.SetRange(Memb."Mobile Phone No", UpperCase(Format(IdentifierTwo)));
                //     if Memb.FindFirst() then begin
                //         Vendor.Reset();
                //         Vendor.SetRange(Vendor."BOSA Account No", Memb."No.");
                //         if Vendor.FindFirst() then begin
        ...
        if Found = false then begin
            SetResponseStatus(ResponseJson, 'error', 'Error', 'Member Account not Found');
        end;
                ...
                if SavLedger.FindFirst() then
                    repeat
                        if SavLedger."Debit Amount" <> 0 then
                            ...
                            EntryType := 'Credit';
                        DataJson.Add('entry_number', SavLedger."Entry No.");
                        DataJson.Add('transaction_reference', SavLedger."Document No.");
                        DataJson.Add('posting_date', SavLedger."Posting Date");
                        DataJson.Add('description', SavLedger.Description);
                        DataJson.Add('amount', SavLedger.Amount);
                        DataJson.Add('running_balance', 0);
                        DataJson.Add('entry_type', EntryType);
                    until SavLedger.Next() = 0;
