//************************************************************************
#pragma warning disable AA0005, AA0008, AA0018, AA0021, AA0072, AA0137, AA0201, AA0206, AA0218, AA0228, AL0424, AL0469, AW0006 // ForNAV settings
Table 50368 "Loans Register"
{
    ...
    fields
    {
        ...
        field(2; "Application Date"; Date)
        {
            ...
            trigger OnValidate()
            begin
                if "Application Date" > Today then Error('Application date can not be in the future.');
            end;
        }
        field(3; "Loan Product Type"; Code[50])
        {
            ...
            begin
                AuFactory.FnCheckMembershipRequirements("Client Code", "Loan Product Type");
                ...
                if LoanType.Get("Loan Product Type") then begin
                    ...
                    if (LoanType."Loan Product Expiry Date" = 0D) or (LoanType."Loan Product Expiry Date" > Today) then begin
                        ...
                        if LoanApp.Find('-') then begin
                            repeat
                                ...
                                if LoanApp."Outstanding Balance" > 0 then begin
                                    Message('Member has another Loan of same type. Loan Type name: %1 outstanding Balance %2 ', LoanApp."Loan Product Type", LoanApp."Outstanding Balance");
                                end;
                        ...
                        if LoanType.Get("Loan Product Type") then begin
                            ...
                            If LoanType."Use Scale" = false then begin
                                Cust.get("Client Code");
                                if cust."Member Type" = Cust."member type"::Member then
                                    Interest := LoanType."Interest rate"
                                else
                                    Interest := LoanType."Staff Interest rate";
                            end;
                            "Instalment Period" := LoanType."Instalment Period";
                            "Recommended Amount" := LoanType."Deposits Multiplier" * Savings;
                            ...
                            "Guarantorship Multiplier" := genSetup."Guarantors Multiplier";
        ...
        field(4; "Client Code"; Code[50])
        {
            ...
            var
                ...
                guarantorship: record "Loans Guarantee Details";
                collateral: Record "Loan Collateral Details";
                msg: Text;
                msgII: Text;
                GenShares: record "Sacco General Set-Up";
            begin
                loantopup.reset();
                ...
                guarantorship.reset();
                guarantorship.setrange("loan No", "Loan  No.");
                if guarantorship.find('-') then begin
                    guarantorship.deleteAll();
                end;

                collateral.reset();
                collateral.setrange("Loan No", "Loan  No.");
                if collateral.find('-') then begin
                    collateral.deleteAll();
                end;
...
/*                     if "Loan Product Type" = 'M_OD' then begin
                        cust.CalcFields("Ordinary Savings");
                        if Cust."Ordinary Savings" < 0 then begin
                            msg := 'This member has an overdrawn account equal to Ksh. %1. Do you wish to proceed with reconciliation?';
                            msgII := StrSubstNo(msg, Cust."Ordinary Savings");
                            if Confirm(msgII, false) = true then begin
                                "Requested Amount" := Cust."Ordinary Savings" * -1;
                                "Recommended Amount" := Cust."Ordinary Savings" * -1;
                                "Approved Amount" := Cust."Ordinary Savings" * -1;
                                "Overdrawn Reconciliation" := true;
                            end;
                        end else begin
                            ...
                            // if Cust."E-Mail" = '' then Error('Kindly ensure this member has an e-mail address');
                        end;
                    end else begin
                    ...
                    "Monthly Contribution" := cust."Monthly Contribution" + cust."ESS Contribution";
                end;
                ...
                if LoanApp.Find('-') then begin
                    ...
                    Message('This member has a defaulted A03 Loan: %1, due on: %2, with an outstanding balance of %3', LoanApp."Loan  No.", LoanApp."Expected Date of Completion", LoanApp."Total Outstanding Balance");
                end;
                ...
                MemberLedgerEntry2.SetRange(MemberLedgerEntry2."Customer No.", "Client Code");
                MemberLedgerEntry2.SetFilter(MemberLedgerEntry2."Transaction Type", '%1|%2', MemberLedgerEntry2."transaction type"::Loan,
                MemberLedgerEntry2."transaction type"::Repayment);
                if MemberLedgerEntry2.FindSet then begin
                    MemberLedgerEntry2.CalcSums(MemberLedgerEntry2.Amount);
                    "Total Outstanding Loan BAL" := MemberLedgerEntry2.Amount;
                end;
                ...
                if CustomerRecord.Get("BOSA No") then begin
                    ...
                    //    if CustomerRecord."Shares Retained"<GenSetUp."Retained Shares" then
                    //   error('Member has not attained minimum shares of %1',CustomerRecord."Shares Retained");
                    ...
                    "Account Type" := CustomerRecord."Account Category";
                    ...
                    LoanApps.Reset;
                    LoanApps.SetRange(LoanApps."Client Code", "Client Code");
                    LoanApps.SetFilter(LoanApps."Loans Category", '%1|%2|%3', LoanApps."Loans Category"::Doubtful, LoanApps."Loans Category"::Loss, LoanApps."Loans Category"::Substandard);
                    LoanApps.SetAutoCalcFields(LoanApps."Total Outstanding Balance");
                    LoanApps.SetFilter(LoanApps."Total Outstanding Balance", '>%1', 0);
                    LoanApps.SetRange(LoanApps.Posted, true);
                    if LoanApps.Find('-') then begin
                        repeat
                            LoanTypes.get(LoanApps."Loan Product Type");
                            Message('Member has  defaulted %1 Loan No %2.', LoanTypes."Product Description", LoanApps."Loan  No.");
                        /*                         LOffset.Init();
                                                LOffset."Loan No.":="Loan  No.";
                                                LOffset."Client Code":="Client Code";
                                                LOffset."Loan Top Up":=LoanApps."Loan  No.";
                                                LOffset.Validate(LOffset."Loan Top Up");
                                                LOffset.Insert(); */
                ...
                /*                 AccountType.Reset;
                                AccountType.setrange(AccountType."Is Deposits", true);
                                if AccountType.findfirst then begin
                                    Vend.Reset();
                                    Vend.SetRange(Vend."BOSA Account No", "Client Code");
                                    Vend.SetRange(Vend."Account Type", AccountType.Code);
                                    if Vend.FindFirst() then begin
                                        Vend.CalcFields(Vend.Balance);
                                        "Member Deposits" := Vend.Balance;
                                    end;
                                end; */
                ...
                if LoanApp.Find('-') then
                    Message('Member has a loan which has previously been recovered from gurantors. - %1. Do you wish to continue?', LoanApp."Loan  No.");
            ...
            begin
                if LoanType.Get("Loan Product Type") then begin
                    ...
                    if "Requested Amount" < LoanType."Min. Loan Amount" then begin
                        Error('You Can not request less than the Loan Allowable limit of %1', LoanType."Min. Loan Amount");
                    end;
                end;
                ...
                if LoanApps.Find('-') then begin
                    repeat
                        LoanApps.CalcFields(LoanApps."Outstanding Interest", LoanApps."Outstanding Balance");
                        TotalDamount := TotalDamount + LoanApps."Outstanding Balance" + LoanApps."Outstanding Interest";
                    until LoanApps.Next = 0;
                end;
                if "Requested Amount" < TotalDamount then
                    error('The amount requested should be more than the total loans defaulted of %1.', TotalDamount);
        ...
        field(21; "Source of Funds"; Text[1200])
        {
        ...
        {
            OptionCaption = 'Application,Appraisal,Valuation,Credit Committee,TCC Approved,Rejected,Disbursed,Closed,Approved,Open,Issued,Approval,legal,Canceled,Finance';
            OptionMembers = Application,Appraisal,Valuation,"Credit Committee","TCC Approved",Rejected,Disbursed,Closed,Approved,Open,Issued,Approval,Legal,Canceled,Finance;
        ...
        field(30; Installments; Integer)
        {
            ...
            trigger OnValidate()
            var
                Products: Record "Loan Products Setup";
                GraduatedInterest: Record "Product Graduated Interest";
            begin
                if Posted <> true then begin
                    Products.get("Loan Product Type");
                    if Installments > Products."Default Installements" then
                ...
                if Cust.Get("Client Code") then begin
                    if Cust."Employer Code" <> 'PENSIONERS' then begin
                ...
                if LoanType.Get("Loan Product Type") then begin
                    If LoanType."Use Scale" = true then begin

                        GraduatedInterest.Reset();
                        GraduatedInterest.SetRange(GraduatedInterest."Product Code", "Loan Product Type");
                        if GraduatedInterest.FindSet() then begin
                            repeat
                                if (Installments >= GraduatedInterest."Minimum Installment") and (Installments <= GraduatedInterest."Maximum Installment") then begin

                                    Interest := GraduatedInterest.Percentage;
                                end;
                            until GraduatedInterest.next() = 0;
                        end;
                    end;
                end;
        ...
        field(34; "Loan Disbursement Date"; Date)
        {
            ...
            begin
                ...
                IF "Loan Product Type" <> 'A03' then BEGIN
                ...
                END;
                IF "Loan Product Type" = 'A03' then BEGIN
                    "Repayment Start Date" := "Loan Disbursement Date"; //CalcDate('1M', "Loan Disbursement Date");
                END;
        ...
        field(68; "Sent for Appraisal On"; Time)
        { }
        field(69; "Sent for Approval On"; Time)
        { }
        field(70; "Disbursed Amount"; Decimal)
        {

            FieldClass = FlowField;
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Customer No." = field("Client Code"), "Loan No" = field("Loan  No."), "Transaction Type" = filter(Loan)));
        }
        /*         field(70;"";Time)
                {}
                field(;;)
        ...
        field(53065; "Batch No."; Code[20])
        {
            ...
            begin
                Loan.Reset();
                Loan.SetRange("Batch No.", "Batch No.");
                Loan.SetRange("Overdrawn Reconciliation", false);
                Loan.SetFilter("Loan  No.", '<>%1', "Loan  No.");
                if Loan.Find('-') then begin
                    Loan."Batch No." := '';
                    // Loan.Modify;
                end;
        ...
        field(70102; "Installment Defaulted"; Decimal)
        {
        }
        ...
        field(172300; "Outstanding Balance"; Decimal)
        {
            ...
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Loan No" = field("Loan  No."), "Transaction Type" = filter(Loan | Repayment),
                                                                        "Posting Date" = field("Date filter"), Reversed = filter(false), "Customer No." = field("Client Code")));
            Editable = false;
            FieldClass = FlowField;
        ...
        {
            CalcFormula = max("Detailed Cust. Ledg. Entry"."Posting Date" where("Loan No" = field("Loan  No."), Reversed = filter(false),
                                                                          "Transaction Type" = filter(repayment | "Interest Paid" | "Loan Penalty Paid" | "Loan Insurance Paid")));
            Editable = false;
            FieldClass = FlowField;
        }
        ...
        field(53184; "Appraisal Status"; Option)
        {
            ...
            begin
                if "Appraisal Status" = "appraisal status"::"Management Approved" then begin
                    if "Requested Amount" > 17200000 then
        ...
        {
            /*             CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Loan No" = field("Loan  No."),
                                                                              "Transaction Type" = filter(Apprai),
                                                                              "Posting Date" = field("Date filter")));
                        Editable = false;
                        FieldClass = FlowField; */
        }
        ...
        {
            FieldClass = FlowField;
            CalcFormula = Max("Loan Repayment Schedule"."Monthly Repayment" where("Loan No." = field("Loan  No."),
                                                                                     "Repayment Date" = field("Date filter")));
        }
        field(53193; "Schedule Interest"; Decimal)
        {
            CalcFormula = sum("Loan Repayment Schedule"."Monthly Interest" where("Loan No." = field("Loan  No."), "Repayment Date" = field("Date filter")));
            FieldClass = FlowField;
            Enabled = false;
        }
        ...
        field(53197; "Last Repayment"; Decimal)
        {
            CalcFormula = - max("Detailed Cust. Ledg. Entry".Amount where("Loan No" = field("Loan  No."), "Posting Date" = field("Last Pay Date"),
                                                                          "Transaction Type" = filter(repayment | "Loan Penalty Paid" | "Loan Insurance Paid")));
            Editable = false;
            FieldClass = FlowField;
            Enabled = false;
        }
        ...
        field(172001; "Outstanding Interest"; Decimal)
        {
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Transaction Type" = filter("Interest Due" | "Interest Paid"),
                                                                  ...
                                                                  "Customer No." = field("Client Code"), "Loan No" = field("Loan  No.")));
            Editable = false;
            FieldClass = FlowField;
        }
        ...
        field(68061; "Recovered Guarantor Amount"; Decimal)
        {
        }
        field(68100; "Guarantor Amount"; Decimal)
        {
        }
        ...
        field(69014; "Total Earnings(Salary)"; Decimal)
        {
            ...
            var
                myInt: Integer;
            begin
                "Third basic" := "Basic Pay H" * 1 / 3;
                "Two Thirds Basic" := "Basic Pay H" * 2 / 3;
            end;
        }
        field(69015; "Total Deductions(Salary)"; Decimal)
        {
            ...
            begin
                "Third basic" := "Basic Pay H" * 1 / 3;
                "Two Thirds Basic" := "Basic Pay H" * 2 / 3;
                Gross := ("Total Earnings(Salary)");
                "Net take Home" := Gross - "Total Deductions(Salary)";
            end;
        }
        ...
        {
            OptionCaption = ' ,Checkoff,Standing Order,Salary,Pension,Direct Debits,Mobile,PaytoFosa';
            OptionMembers = " ",Checkoff,"Standing Order",Salary,Pension,"Direct Debits",Mobile,PaytoFosa;

            trigger OnValidate()
            begin
                // if ("Employer Code" = 'POSTAL CORP') or ("Employer Code" = 'KENYA POSTAL') or ("Employer Code" = 'TELKOM') then begin
                //     if "Recovery Mode" = "Recovery Mode"::Checkoff then Error('Checkoff is not available for PCK and Telkom members. Kindly Select another recovery mode.');
                // end;
                if ("Employer Code" = 'PENSIONERS') then begin
                    "Recovery Mode" := "Recovery Mode"::Pension;
                end;
                if ("Employer Code" = 'PENSIONERS') then begin
                    if "Recovery Mode" <> "Recovery Mode"::Pension then Error('Pensioners are only allowed a recovery mode of pension.');
                end
            end;
        }
        ...
        {
            OptionCaption = 'Open,Pending,Approved,Rejected,Canceled';
            OptionMembers = Open,Pending,Approved,Rejected,Canceled;
        }
        ...
        field(69066; "Rejected By"; Code[50])
        {
            Editable = false;
        }
        ...
        field(69073; "Rejection  Remark"; Text[500])
        {
        }
        ...
        field(69076; "Re-Appraissing Remark"; Text[500])
        {
        }
        ...
        field(69086; "Reason for Other Income"; Text[1000])
        {
        }
        ...
        {
            FieldClass = FlowField;
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Customer No." = field("Client Code"), "Loan No" = field("Loan  No."),
                                                                        "Posting Date" = field("Date filter"), Reversed = filter(false),
                                                                        "Transaction Type" = filter("Loan Penalty Charged")));
            Enabled = false;
        }
        ...
        field(69109; "Staff Assement"; Decimal)
        {
            ...
            trigger OnValidate()
            begin
                if "Staff Assement" > 1720000 then
        ...
        field(69111; "Medical Insurance"; Decimal)
        {
            ...
            trigger OnValidate()
            begin
                if "Medical Insurance" > 1720000 then
        ...
        field(69112; "Life Insurance"; Decimal)
        {
            ...
            trigger OnValidate()
            begin
                if "Life Insurance" > 1720000 then
        ...
        field(69113; "Other Liabilities"; Decimal)
        {
            ...
            trigger OnValidate()
            begin
                if "House AllowanceH" > 1720000 then
        ...
        field(69165; "Disbursed By"; Code[250])
        {
        }
        ...
        field(69174; "Loan Debt Collector"; Code[20])
        {
            ...
            TableRelation = "Debt Collectors Table";
            trigger OnValidate()
            var
            ...
            begin
                ...
                debtCollectors.SetRange("Debtors Code", "Loan Debt Collector");
                if debtCollectors.Find('-') then begin
                    "Debt Collector Name" := debtCollectors."Debt Collectors";
                    "Loan Under Debt Collection" := true;
                    "Loan Debt Collector Interest %" := debtCollectors.Rate;
                    "Debt Collection date Assigned" := Today;
                    CalcFields("Total Outstanding Balance");
                    "Loan Bal As At Debt Collection" := "Total Outstanding Balance";
                end;
            end;
        }
        ...
        field(69177; "Debt Collector Name"; Code[1250])
        {
            // TableRelation = "Debt Collectors Table"."Debt Collectors";
            Editable = false;
        }
        ...
        {
            // FieldClass = FlowField;
            // CalcFormula = max("Detailed Cust. Ledg. Entry".Reversed where("Customer No." = field("Client Code"), "Transaction Type" = filter(Loan), "Loan No" = field("Loan  No.")));
        }
        ...
        field(69220; "Loan Deposit Multiplier"; Decimal)
        {
        }
        ...
        field(69247; "Principal In Arrears"; Decimal)
        {
        }
        field(69248; "Recover Share Capital"; Decimal)
        {
        }
        field(69249; "Loanee FOSA Balance"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = - sum("Detailed Vendor Ledg. Entry".Amount where("Vendor No." = field("Account No"), Reversed = filter(false), "Posting Date" = field("Date filter")));
            Enabled = false;
        }
        ...
        {
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Customer No." = field("Client Code"), "Loan No" = field("Loan  No."),
                                                                   "Transaction Type" = filter("Loan Penalty Paid" | "Loan Penalty Charged"),
                                                                   "Posting Date" = field("Date filter")));
            FieldClass = FlowField;
        }
        ...
        field(51516250; "Loan Application Mode"; Option)
        {
            OptionCaption = ' ,System,Mobile,WalkIn,Portal';
            OptionMembers = " ",System,Mobile,WalkIn,Portal;
        }
        ...
        {
            CalcFormula = max("Cust. Ledger Entry"."Posting Date" where("Transaction Type" = filter(repayment),
                                                                          "Loan No" = field("Loan  No.")));
            FieldClass = FlowField;
        }
        ...
        field(51516287; "Main Sector"; Code[20])
        {
            ...
            begin
                ...
                if MainS.FindFirst then begin
                    "Manin Sector Name" := MainS.Description;
                end;
            end;
        }
        ...
        field(172000; PayPayee; Boolean) { DataClassification = ToBeClassified; }
        field(172005; "T-Kash"; Decimal) { DataClassification = ToBeClassified; }
        field(172006; "Sector Description"; Text[1000])
        { DataClassification = ToBeClassified; }
        field(172007; "Interest Charged"; Decimal)
        {
            CalcFormula = sum("Interest Buffer Table"."Interest Amount" where("Loan No" = field("Loan  No.")));
            FieldClass = FlowField;
            // Enabled=false;
        }

        field(172008; "OutstandingLessInt"; Decimal)
        {
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Loan No" = field("Loan  No."), "Transaction Type" = filter(Loan | Repayment), "Posting Date" = field("Date filter"), Description = filter(<> 'InterestCharged')));
            Editable = false;
            FieldClass = FlowField;
            Enabled = false;
        }
        field(172009; "2 thirds Amount"; Decimal)
        {
            DataClassification = ToBeClassified;
        }
        field(172010; "Principle Amount Updated"; Decimal)
        {
            DataClassification = ToBeClassified;
            ...
            begin
                //Message('here outside');
                // Rec.Reset();
                // Rec.SetRange(Rec."Loan  No.","Loan  No.");
                // if Rec.Find() then begin
                //     Rec."Loan Principle Repayment":="Principle Amount Updated";
                //     Rec.Modify(true);
                // end;
                // LoanApps.Reset();
                // LoanApps.SetRange(LoanApps."Loan  No.", "Loan  No.");
                // if LoanApps.Find('-') then begin
                //     LoanApps."Loan Principle Repayment" := "Principle Amount Updated";
                // end;
            end;
        }
        field(172116; "Archive Loan Member"; Boolean) { }
        field(172011; "Archive Loan"; Boolean) { }
        field(172012; "Defaulter Loan"; Boolean)
        {

        }
        field(172013; "Original Loanee"; Text[250])
        {

        }
        field(172014; "Original Loan"; code[40]) { }
        field(172015; "Committee Approval Comments"; Text[2048])
        {

        }
        field(172016; "Committee Approval"; Option)
        {
            OptionMembers = Open,Pending,Approved,Rejected;
            OptionCaption = 'Open,Pending,Approved,Rejected';
        }
        field(172017; "Loan Age"; Integer)
        {

        }
        field(172018; "Sector Specific Name"; Text[2048])
        {

        }
        field(172019; "Manin Sector Name"; Text[2048])
        {

        }
        field(172020; "SubSector Name"; Text[2048])
        {

        }
        field(172021; "Guarantor Remarks-Appraisal"; Text[2048])
        {

        }
        field(172022; "Deposit Remarks-Appraisal"; Text[2048])
        {

        }
        field(172023; "Salary Remarks-Appraisal"; Text[2048])
        {

        }
        field(172024; "Business Remarks-Appraisal"; Text[2048])
        {

        }
        field(172025; "Share Capital Boosting"; Decimal)
        {

        }
        field(172026; "Guarantorship Multiplier"; Decimal)
        {
            DataClassification = ToBeClassified;

        }
        field(172027; "All Statutory Deduction"; Decimal)
        {
            DataClassification = ToBeClassified;

        }
        field(172028; "External loan"; Decimal)
        {
            DataClassification = ToBeClassified;

        }
        field(172029; "All Other Expenses"; Decimal)
        {
            DataClassification = ToBeClassified;

        }
        field(172030; "All Allowances"; Decimal)
        {
            DataClassification = ToBeClassified;

        }
        field(172031; "External loan Commission"; Decimal)
        {
            DataClassification = ToBeClassified;

        }
        field(172032; "External loan Excise"; Decimal)
        {
            DataClassification = ToBeClassified;

        }
        field(172033; "Housing Levy"; Decimal)
        {
            DataClassification = ToBeClassified;

        }
        field(172034; "Total Interest-Scheduled"; Decimal)
        {
            CalcFormula = sum("Loan Repayment Schedule"."Monthly Interest" where("Loan No." = field("Loan  No.")));
            Editable = false;
        ...
        field(172035; "Tax Exempt Deductions"; Decimal)
        {
            DataClassification = ToBeClassified;

        }
        field(172036; "Book Balance"; Decimal)
        {
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Loan No" = field("Loan  No."), "Transaction Type" = filter(Loan | Repayment | "Interest Due" | "Interest Paid"), "Posting Date" = field("Date filter")));
            Editable = false;
            FieldClass = FlowField;
            Enabled = false;
        }
        field(172037; "Exempt Salary Details"; boolean)
        {
            DataClassification = ToBeClassified;

        }
        field(172038; "Post Upfront Int"; boolean)
        {
            DataClassification = ToBeClassified;

        }
        field(172039; "Upfront Interest Amount"; decimal)
        {
            CalcFormula = sum("Loan Repayment Schedule"."Monthly Interest" where("Loan No." = field("Loan  No."), "Loan Category" = filter('DIVIDENDS ADVANCE')));
            FieldClass = FlowField;
            Editable = false;
            Enabled = false;
        ...
        field(172041; "Mode Of Application"; Option)
        {
            OptionMembers = ,"Over The Counter","Mobile",Portal;
            OptionCaption = ',Over The Counter,Mobile,Portal';
            DataClassification = ToBeClassified;
        }
        field(172042; "Exempt Interest Charging"; Boolean)
        {
            DataClassification = ToBeClassified;
        }

        field(172043; "Member Ability"; decimal)
        {
            DataClassification = ToBeClassified;
            Editable = false;

        }
        field(172044; "Third Gross"; decimal)
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }
        field(172045; "Member Gross Salary"; decimal)
        {
            DataClassification = ToBeClassified;
            Editable = false;
        ...
        field(172046; "Third Percentage"; decimal)
        {
            DataClassification = ToBeClassified;
            Editable = false;

        }
        field(172047; "Member Loans Deductions"; decimal)
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }

        field(172048; "Member Offets"; decimal)
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }

        field(172049; "Pension Contribution"; decimal)
        {
            DataClassification = ToBeClassified;

        }
        field(172050; "TwoThirds"; decimal)
        {
            DataClassification = ToBeClassified;

        }

        field(172051; "Is PCK Salary?"; Boolean)
        {
            DataClassification = ToBeClassified;

        }
        field(172052; "No Of Defaulted Loans"; Integer)
        {
        }

        field(172053; "No Of Guarantors"; Integer)
        {
            FieldClass = FlowField;
            CalcFormula = Count("Loans Guarantee Details" WHERE("Loan No" = FIELD("Loan  No.")));
            Enabled = false;
        }
        field(172054; "No Of Pending Guarantors"; Integer)
        {
            FieldClass = FlowField;
            CalcFormula = Count("Loans Guarantee Details" WHERE("Loan No" = FIELD("Loan  No."), "Acceptance Status" = const(Pending)));
            Enabled = false;
        }
        field(172055; "No Of Guarantors Needed"; Integer)
        {
            FieldClass = FlowField;
            CalcFormula = Lookup("Loan Products Setup"."Min No. Of Guarantors" WHERE(Code = FIELD("Loan Product Type")));
            Enabled = false;
        }
        field(172056; "Has Pending Guarantors"; Boolean)
        {
            FieldClass = FlowField;
            CalcFormula = Exist("Loans Guarantee Details" WHERE("Loan No" = FIELD("Loan  No."), "Acceptance Status" = const(Pending)));
            Enabled = false;
        }

        field(177057; WriteOff; Boolean)
        {
        }
        field(172058; "Debtor Collection Status"; Option)
        {
            OptionCaption = ',Debt Cleared,Debtor being Traced,Debtor Deceased,Disputing,Inability to Pay,Invalid Number,Negotiation in Process,Non-Committal,Phone Switched Off,Promised to Pay,Ringing No Response,Sick,Temporarily out of service,Unable to Trace,Wrong Number,No Contact provided';
            OptionMembers = ,"Debt Cleared","Debtor being Traced","Debtor Deceased",Disputing,"Inability to Pay","Invalid Number","Negotiation in Process","Non-Committal","Phone Switched Off","Promised to Pay","Ringing No Response",Sick,"Temporarily out of service","Unable to Trace","Wrong Number","No Contact provided";
        }
        field(172059; "Defaulted Balance"; Decimal)
        {
            //  FieldClass = FlowField;
            //CalcFormula = Sum("Member Ledger Entry".Amount WHERE("Transaction Type" = FILTER("Loan Repayment" | "Interest Due" | "Interest Paid" | "Loan Penalty Charged" | "Loan Penalty Paid"), "Customer No." = FIELD("Client Code"), "Loan No" = FIELD("Loan  No.")));
        }
        field(172060; "Agreed Instalments"; Integer)
        {
        }
        field(172061; "Agreed Amount"; Decimal)
        {
        }
        field(172062; "Payment Date"; Date)
        {
        ...
        field(172063; "Notification Date"; Date)
        {
        }
        field(172064; "Phone No."; Text[30])
        {
        }
        field(173061; "Debt Collector Commission"; Decimal)
        {
        }
        field(173062; "VAT on Commission"; Decimal)
        {
        }
        field(173063; "Total Collection Amount"; Decimal)
        {
        }
        field(173064; "15% on Karibu"; Decimal)
        {
            Editable = false;
        }

        //New Fields from .71
        field(172065; "Offset Loan"; Boolean)
        {

        }
        field(172066; "Total TopUp Commission"; Decimal)
        {

        }
        field(172067; "Top Up Amount"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = Sum("Loan Offset Details"."Total Top Up" WHERE("Loan No." = FIELD("Loan  No."),
                                                                          "Client Code" = FIELD("Client Code")));
        ...
        field(172068; "Reason for rejection"; Code[55])
        {
        }
        field(172069; "Sub-Sector"; Code[10])
        {
        }
        field(172070; "Specific Sector"; Code[10])
        {
        }
        field(172071; "Reason for Loan Re-open"; Code[10])
        {
        }
        field(172072; "Registration Time"; Time)
        {
        }
        field(172073; "Terms of Service"; Option)
        {
            OptionCaption = ',Permanent,Temporary,Contract';
            OptionMembers = ,Permanent,"Temporary",Contract;
        }
        field(172074; Designation; Text[40])
        {
        }
        field(172075; Workstation; Text[40])
        {
        }
        field(172076; "Gross monthly Income"; Decimal)
        {
        }
        field(172077; "Estimated Monthly Expense"; Decimal)
        {
        }
        field(172078; "Estimated Net Monthly Income"; Decimal)
        {
        }
        field(172079; "Staff Loan"; Boolean)
        {
        }
        field(172080; "Other Incomes"; Decimal)
        {
        ...
        field(172081; "Other Income."; Decimal)
        {
        ...
        field(172082; "Third basic"; Decimal)
        {
        }
        field(172083; "Two Thirds Basic"; Decimal)
        {
        }
        field(172084; "Other Deductions"; Decimal)
        {
        }
        field(172085; "Disbursed Time"; Time)
        {
        }
        field(172086; "Reason For Loan Rejection"; Text[40])
        {
        }
        field(172087; "Emp Loan Codes"; Code[15])
        {
        }
        field(172088; "Appraised By"; Code[30])
        {
        }
        field(172089; "Appraised Time"; Time)
        {
        }
        field(51516021; "Express Loan"; Boolean)
        {
            Editable = false;
        }
        field(51516022; "Bridge Shares"; Decimal)
        {
        }
        field(51516023; "interest upfront Amount1"; Decimal)
        {
        }
        field(51516024; "Discount Amount"; Decimal)
        {
        }
        field(51516025; "Vendor No"; Code[20])
        {
        ...
        field(51516126; "Exempted from PAYE"; Boolean)
        {
        }

        field(173065; "Appraised Date"; Date)
        {
        }
        field(173066; "Disbursement Date"; Date)
        {
        }
        field(173067; "Approved Date"; Date)
        {
        }

        field(173070; "Expected Disbursement Time"; Time)
        {
        }
        field(173071; "Approved Time"; Time)
        {
        }
        field(173077; "Mobile Loan"; Boolean)
        {
            FieldClass = FlowField;
            CalcFormula = lookup("Loan Products Setup"."Is Mobile Loan?" WHERE(Code = FIELD("Loan Product Type")));
            Enabled = false;
        }
        field(173088; "Requires Guarantors"; Boolean)
        {
            FieldClass = FlowField;
            CalcFormula = lookup("Loan Products Setup"."Requires Guarantors" WHERE(Code = FIELD("Loan Product Type")));
        }


        field(173078; "Captured By Date"; Date)
        {

        }

        field(173079; "Captured By Time"; Time)
        {

        }

        field(173080; "Appraissed By Date"; Date)
        {
            Editable = false;
        }

        field(173081; "Appraissed By"; Code[60])
        {
            Editable = false;
        }
        field(173082; "Appraissed By Time"; Time)
        {
            Editable = false;
        }
        field(173083; "Approved By Date"; Date)
        {
            Editable = false;
        }

        field(173085; "Disbursed By Date"; Date)
        {
            Editable = false;
        }

        field(173084; "Disbursed By Time"; Time)
        {
            Editable = false;
        }
        field(173086; "Deduct Share Capital Amount?"; Boolean)
        {
            trigger OnValidate()
            var
            ...
            begin
                SaccoGen.get();
                if "Deduct Share Capital Amount?" = true then begin
                    "Share Capital Amount" := SaccoGen."Loan Shares Deduction Amount";
                end;
                if "Deduct Share Capital Amount?" = false then begin
                    "Share Capital Amount" := 0;
                end;
            end;
        }
        field(173087; "Share Capital Amount"; Decimal)
        {
            Editable = false;
        }
        field(173089; "Requires Moblie Guarantors"; Boolean)
        {
            Editable = false;
        }
        field(172090; "Requires Guarantors Mobile"; Boolean)
        {
            FieldClass = FlowField;
            CalcFormula = Lookup("Loan Products Setup"."Requires Guarantors Mobile" WHERE(Code = FIELD("Loan Product Type")));
        }

        field(172091; "Total Outstanding Balance"; Decimal)
        {
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Loan No" = field("Loan  No."), Reversed = filter(false), "Posting Date" = field("Date filter"),
                                                                        "Transaction Type" = filter(Loan | Repayment | "interest Due" | "Interest Paid" | "Loan Penalty Charged" | "Loan Penalty Paid")));
        ...
        field(172092; "Start Date Portal"; Date)
        {
            Editable = false;
        }

        field(172093; "End Date Portal"; Date)
        {
            Editable = false;
        }
        field(172094; "Guaranteed Amount"; Decimal)
        {
            FieldClass = FlowField;
        ...
        field(172096; "Underguaranteed"; Boolean)
        {
            Editable = false;
        }


        field(172097; "Total Offsets"; Decimal)
        {
            CalcFormula = sum("Loan Offset Details"."Outstanding Balance" where("Loan No." = field("Loan  No."), "Client Code" = field("Client Code")));
            Editable = false;
            FieldClass = FlowField;
            Enabled = false;
            trigger OnValidate()
        ...
        field(172098; "Total Principal Repayment"; Decimal)
        {
            CalcFormula = sum("Detailed Cust. Ledg. Entry"."Amount" where("Loan No" = field("Loan  No."), "Customer No." = field("Client Code"), "Transaction Type" = filter(Repayment), Reversed = filter(false)));
            Editable = false;
            FieldClass = FlowField;
            Enabled = false;
        }
        field(172099; "Total Loan Interest Due"; Decimal)
        {
            CalcFormula = sum("Detailed Cust. Ledg. Entry"."Amount" where("Loan No" = field("Loan  No."), "Customer No." = field("Client Code"), "Transaction Type" = filter("Interest Due"), Reversed = filter(false)));
            Editable = false;
            FieldClass = FlowField;
            Enabled = false;
        }
        field(172100; "Principal Paid Total"; Decimal)
        {
        }
        field(172101; "Re-Opened By"; Code[20])
        { }
        field(172102; "Re-Opened On"; Date)
        { }
        field(172103; "Re-Opened On Time"; Time)
        { }
        field(172104; "Re-Appraised By"; Code[20])
        { }
        field(172105; "Re-Appraised On"; Date)
        { }
        field(172106; "Re-Appraised On Time"; Time)
        { }
        field(172107; "ShortTermLoan"; Decimal)
        {
            DataClassification = ToBeClassified;
        }
        field(172108; "LongTermLoan"; Decimal)
        { }
        field(172109; "Write Off Amount"; Decimal)
        { }
        field(172110; "Overdrawn Reconciliation"; Boolean)
        { }
        field(172111; "Special Advance"; Decimal)
        {
            // Editable = false;
        }
        field(172112; "Loan Buy-Off Facilitated"; Boolean)
        {
            Editable = false;
        }
        field(172113; "Special Advance Eligible"; Boolean)
        {
            // Editable = false;
        }
        field(172114; "Classification Date"; Date)
        {
            Editable = false;
        }
        field(172115; "Loans Category AsAt"; Option)
        {
            OptionCaption = 'Perfoming,Watch,Substandard,Doubtful,Loss';
            OptionMembers = Perfoming,Watch,Substandard,Doubtful,Loss;
        }

        field(172117; "Agency EFT"; Decimal)
        {

        }
        field(172118; "Agency EFT Ref"; Code[60])
        {

        }
        field(172119; "Manual Special Advance"; Boolean)
        {

        }
        field(172120; "Marital Status"; Enum "Marital Status")
        {

        }
        field(172121; "Name Of Spouse"; Text[100])
        {

        }
        field(172122; "Spouse Mobile Number"; Code[40])
        {

        }
        field(172123; "Spouse Email Address"; Text[100])
        {

        }
        field(172124; "Other Sources Of Income"; Decimal)
        {

        }
        field(172125; "Valuer Code"; Code[80])
        {
            ...
            begin
                ...
                CollateralValuer.SetRange(CollateralValuer."Valuer Code", "Valuer Code");
                if CollateralValuer.Find('-') then begin
                    "Valuer Name" := CollateralValuer."Valuer Name";
                end;
            end;
        }
        field(172126; "Valuer Name"; Text[100])
        {
            Editable = false;
        }
        field(172127; "Recovery Stage"; Enum "Recovery Stage")
        {
            Editable = false;

        }
        field(172128; "Total Disbursed Tranches"; Decimal)
        {
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = sum("Tranche Disbursement Schedule"."Tranche Amount" where("Loan Number" = field("Loan  No."), Posted = filter(true)));
            Enabled = false;
        }
        field(172129; "Assigned Officer"; Code[60])
        {
            // Editable = false;

        }
        field(172130; "Arrears Bucket"; Enum "Arrears Bucket") { Editable = false; }

        field(172131; "Collateral No."; Code[20])
        {
        ...
        field(172132; "FSV"; Decimal) { Editable = false; }
        field(172133; "Market Value"; Decimal) { Editable = false; }

        field(172134; "Followup Officer"; Code[60])
        {

        }
        field(172135; "Mortgage Officer"; Code[60])
        {
            TableRelation = "User Setup"."User ID" where(Department = filter("Mortgage"));
        }

        field(172136; "Vendor Bank Code"; Code[60])
        {
            TableRelation = "Banks Ver2"."Bank Code";
            trigger OnValidate()
            var
            ...
            begin
                Banks.Reset();
                Banks.SetRange(Banks."Bank Code", "Vendor Bank Code");
                if Banks.FindFirst() then begin
                    "Vendor Bank Name" := Banks."Bank Name";
                    "Vendor Bank Branch Code" := Banks."Branch Code";
                    "Vendor Bank Branch Name" := Banks."Branch Name";
                end;
            end;
        }
        field(172137; "Vendor Bank Name"; Text[100])
        {
            //Editable = false;
        }
        field(172138; "Vendor Bank Branch Code"; Code[60])
        {

        }
        field(172139; "Vendor Bank Branch Name"; Text[100])
        {

        }
        field(172140; "Vendor Bank Account No"; Code[60])
        {

        }
        field(172141; "Property Status"; Option)
        {
            OptionMembers = Commercial,Residential,Agricultural;
            OptionCaption = 'Commercial,Residential,Agricultural';
        }

        field(172142; "Property Location"; Code[100])
        {
            //TableRelation = "Dimension Value".Code;
        }
        field(172143; "Property Location Description"; Text[500])
        {

        }
        field(172144; "Property Description"; Text[500])
        {

        }
        field(172145; "Current Market Value"; Text[500])
        {

        }
        field(172146; "Loan Payee"; Text[500])
        {

        }
        field(172147; "Total Business Income"; Decimal)
        {

        }
        field(172148; "Business Expense"; Decimal)
        {

        }
        field(172149; "Business Net Income"; Decimal)
        {

        }
        field(172150; "Gross Salary"; Decimal)
        {

        }
        field(172151; "Total Salary Deductions"; Decimal)
        {

        }
        field(172152; "Net Salary"; Decimal)
        {

        }
        field(172153; "Valuation Comments"; Text[250])
        {

        }
        field(172154; "Portfolio Officer"; Code[60])
        {
            TableRelation = "User Setup"."User ID" where(Department = filter("Recovery"));
        }
        field(172155; "Address"; Code[120])
        {

        }
        field(172156; "Phone No"; Code[60])
        {

        }

        field(172157; "Email Address"; Text[100])
        {
        }
        field(172158; "Alternative Name"; Text[100])
        {
        }
        field(172159; "Alternative Phone No"; Code[60])
        {

        }

        field(172160; "Alternative Email Address"; Text[100])
        {
        }
        field(172161; "Spouse Name"; Text[100])
        {
        }
        field(172162; "Spouse Phone No"; Code[60])
        {

        }

        field(172163; "Cert of Registration"; Boolean)
        {

        }
        field(172164; "CR12"; Boolean)
        {

        }
        field(172165; "Constitution"; Boolean)
        {

        }
        field(172166; "Board Resolution to Borrow"; Boolean)
        {

        }
        field(172167; "ID & KYC of Directors"; Boolean)
        {

        }
        field(172168; "Tax Compliance Certificate"; Boolean)
        {

        }
        field(172169; "MINUTES of Borrowing"; Boolean)
        {

        }
        field(172170; "Audited Financials"; Boolean)
        {

        }
        field(172171; "Account Type"; enum AccountCategoryEnum)
        {

        }
        field(172172; "Mortgage Officers Comments"; Text[1400])
        {

        }

        field(172173; "Copy Of KRA Pin"; Boolean)
        {

        }
        field(172174; "Copy of Title Deed"; Boolean)
        {

        }
        field(172175; "Consent Letter"; Boolean)
        {

        }
        field(172176; "Bill Of Quantities"; Boolean)
        {

        }
        field(172177; "Bulding Plan"; Boolean)
        {

        }
        field(172178; "Number of Members/Shareholders"; Integer)
        {

        }
        field(172179; "Membership Sector"; Text[1000])
        {
            Caption = 'Membership - Drawn From Which Sector or Location';
        }
        field(172180; "Years In Operation"; Decimal)
        {

        }
        field(172181; "Physical Address"; Text[500])
        {

        }
        field(172182; "Corporate Type"; Option)
        {
            OptionMembers = Sacco,Corporate,"Housing Cooperative","Investment/Chama";
        }
        field(172183; "Corporate Date of Registration"; Date)
        {

        }
        field(172184; "Bankers"; Text[500])
        {

        }


        field(172185; "Findings Summary"; Text[1500])
        {

        }
        field(172186; "Proffession"; Text[200])
        {

        }

        field(172187; "Age"; Text[200])
        {


        }

        field(172188; "Dependants"; Integer)
        {


        }
        field(172189; "Ages Of Dependants"; Text[1200])
        {


        }
        field(172190; "Years In Said Proffession"; Integer)
        {
        }
        field(172191; "Education Level"; Text[1200])
        {


        }
        field(172192; "Current Living Status"; Option)
        {
            OptionMembers = Rented,Owned;

        }
        field(172193; "Status Of Account(6 Months)"; Text[1200])
        {

        }
        field(172194; "Savings Balance"; Decimal)
        {
        ...
        field(172196; "Average Savings Per Month"; Decimal)
        {

        }
        field(172197; "Total Exposure with KHC"; Decimal)
        {

        }
        field(172198; "Account Conduct Rating"; Option)
        {
            OptionMembers = Good,Fair,Poor;
        }
        field(172199; "Nature Of Savings"; Option)
        {
            OptionMembers = "Per Month","Lumpsum";
        }
        field(172200; "Is Waiting Applicable?"; Option)
        {
            OptionMembers = "Yes","No";
        }
        field(172201; "Pended Description"; Text[1200])
        {

        }
        field(172202; "Loan Application Type"; Option)
        {
            OptionMembers = individual,Joint,Corporate,TopUp,"Loan Against Savings";
        }
        field(172203; "Tenure"; Text[1200])
        {

        }
        field(172204; "Rent"; Decimal)
        {

        }
        field(172205; "School Fees"; Decimal)
        {

        }
        field(172206; "Food"; Decimal)
        {

        }
        field(172207; "Transport"; Decimal)
        {

        }
        field(172208; "Total Net Income"; Decimal)
        {
            CalcFormula = Sum("Loan Income"."Net Income"
                      where("Loan No." = field("Loan  No.")));
            FieldClass = FlowField;
        }
        field(172209; "Valuation Instruction"; Text[1200])
        {

        }
        field(172210; "Current Property Owner"; Text[1200])
        {

        }
        field(172211; "Consent From Guarantor"; Option)

        {
            OptionMembers = Yes,"Not Applicable";
        }
        field(172212; "Owner After Loan Uptake"; Text[1200])
        {

        }
        field(172213; "Collateral Type"; Option)
        {
            //Editable = false;
            NotBlank = true;
        ...
        field(172214; "LR No"; Code[1200])
        {
        }
        field(172215; "Distance From Main Road"; Text[1200])
        {
        }
        field(172216; "Expected Security value After Disbursement"; Decimal)
        {
        }
        field(172217; "Urban/Rural?"; Option)
        {
            OptionMembers = Urban,Rural;
        }
        field(172218; "Forced Sale Value"; Decimal)
        {
        }
        field(172219; "Percentage CMV"; Decimal)
        {
        }
        field(172220; "Total FSV Plus Savings"; Decimal)
        {
        }
        field(172221; "Loan Amount(VAV)"; Decimal)
        {
        }
        field(172222; "Reason(VAV)"; text[1200])
        {
        }
        field(172223; "Tenure(VAV)"; text[1200])
        {
        }
        field(172224; "Monthly Repayment(VAV)"; Decimal)
        {
        }
        field(172225; "Loan Amount(VAA)"; Decimal)
        {
        }
        field(172226; "Reason(VAA)"; text[1200])
        {
        }
        field(172227; "Tenure(VAA)"; text[1200])
        {
        }
        field(172228; "Monthly Repayment(VAA)"; Decimal)
        {
        }
        field(172229; "Income Assessment"; Text[1200])
        {
        }
        field(172230; "Collateral Assessment"; Text[1200])
        {
        }
        field(172231; "Equity Release Ratio Observed"; Text[1200])
        {
        }
        field(172232; "TCC Reccomendation"; Text[1200])
        {
        }
        field(172233; "CEO Reccomendation"; Text[1200])
        {
        }
        field(172234; "Credit Committee Decision"; Text[1200])
        {
        }
        field(172235; "Existing Loan Balance"; Decimal)
        {
        ...
        field(172236; "Total BQ Amount"; Decimal)
        {
        }
        field(172237; "Personal Contribution Amount"; Decimal)
        {
        }
        field(172238; "Corporate Tax Pin"; Code[60])
        {
        }
        field(172239; "Corporate Bankers"; Code[60])
        {
        }
        field(172240; "Personal Directors Guarantees"; Boolean)
        {
        }
        field(172241; "Joint & Several Liability"; Boolean)
        {
        }
        field(172242; "Assignment of Rental Income"; Boolean)
        {
        }
        field(172243; "Savings Less Appraissal Fee"; Decimal)
        {
        }
        field(172244; "% Required per Policy"; Decimal)
        {
        }
        field(172245; "Loan Amount Applied"; Decimal)
        {
        }
        field(172246; "Maximum as Per % Policy"; Decimal)
        {
        }
        field(172247; "Sales Agreement"; Boolean)
        {
        }
        field(172248; "Collateral Type(VAv)"; Option)
        {
            //Editable = false;
            NotBlank = true;
            //Caption = 'Security Type';
        ...
        field(172249; "LR No (VAV)"; Code[1200])
        {
        }
        field(172250; "Distance From Main Road(VAV)"; Text[1200])
        {
        }
        field(172251; "Expected value After Disbursement(VAV)"; Decimal)
        {
        }
        field(172252; "Urban/Rural?(VAV)"; Option)
        {
            OptionMembers = Urban,Rural;
        }
        field(172253; "Forced Sale Value(VAV)"; Decimal)
        {
        }
        field(172254; "Percentage CMV(VAV)"; Decimal)
        {
        }
        field(172255; "Total FSV Plus Savings(VAV)"; Decimal)
        {
        }
        field(172256; "Property Location (VAV)"; Code[100])
        {
            //TableRelation = "Dimension Value".Code;
        }
        field(172257; "Property Location Descr(VAV)"; Text[500])
        {

        }
        field(172258; "Property Description(VAV)"; Text[500])
        {

        }
        field(172260; "Current Market Value(VAV)"; Text[500])
        {

        }
                field(172261; "Copy of M-Pesa Statement"; Boolean)
        {
    ...
    keys
    {
        ...
        key(Key4; Source, "Client Code", "Client Name", "Loan Product Type", "Issued Date")
        {
        }
        ...
        key(Key15; "Issued Date", "Loan Disbursement Date", "Loan Product Type")
        {
            SumIndexFields = "Approved Amount";
        }
    ...
    fieldgroups
    {
        fieldgroup(DropDown; "Loan  No.", "Client Code", "Client Name", "Loan Product Type", "Loan Product Type Name")
        {
        }
    }
    ...
    begin
        /*         userSetup.Reset();
                ...
                if userSetup.Find('-') then begin
                    if userSetup."Can Capture Multiple Loans" = false then begin
                        Loan.Reset();
                        Loan.SetRange("Captured By", UserId);
                        Loan.SetRange(Loan."Loan Status", Loan."Loan Status"::Application);
                        if Loan.Find('-') then begin
                            Error('You have a loan still under application. Loan No: %1', Loan."Loan  No.");
                        end;
                    end;
                end; */
        ...
        if Source = Source::BOSA then begin
            if "Loan  No." = '' then begin
                ...
                "Loan  No." := NoSeriesMgt.GetNextNo(SalesSetup."BOSA Loans Nos");
            end;
        end else if Source = Source::FOSA then begin
            if "Loan  No." = '' then begin
                SalesSetup.Get;
                SalesSetup.TestField(SalesSetup."FOSA Loans Nos");
                "Loan  No." := NoSeriesMgt.GetNextNo(SalesSetup."FOSA Loans Nos");
            end;
        end else begin
            if "Loan  No." = '' then begin
                SalesSetup.Get;
        ...
        "Captured By Time" := TIME;
        "Captured By Date" := TODAY;
    ...
    var
        ...
        LOffset: Record "Loan Offset Details";
        ...
        if ObjLoans.FindSet then begin
            repeat
                ...
                if not LoanTopup.FindFirst then begin
                    VarTotalLoanRepayments += ObjLoans."Loan Principle Repayment" + SFactory.FnCalculateLoanInterest(ObjLoans, EndMonth); //+ ObjLoans."Loan Interest Repayment";
    ...
    begin
        ...
        "Gross Pay" := "Basic Pay H" + "All Allowances";
    ...
    procedure InterestBalanceAsAt(AsAt: Date): Decimal
    var
        MembLedger: Record "Detailed Cust. Ledg. Entry";
    begin
        MembLedger.RESET;
    ...
    procedure GetLoanExpectedRepayment(ReturnOption: option Repayment,Principal,Interest; AsAt: Date): Decimal
    var
    ...
    begin
        ...
        CASE ReturnOption OF
            ReturnOption::Repayment:
                EXIT(ROUND(MRep, 1, '>'));
            ReturnOption::Principal:
                EXIT(ROUND(MPrinc, 1, '>'));
            ReturnOption::Interest:
                EXIT(ROUND(MInt, 1, '>'));
        END;

    end;
    // New Procedures from .71
    procedure MarkLoanAsExpress()
    var
        LoanBatch: Record //"51516236"
        "Loan Disburesment-Batching";
        cnfm_express: Label 'Sure to Mark this loan as an express loan?';
    ...
    procedure OffsetCondition()
    var
        Loan: Record "Loans Register";
        LoanOffset: Record "Loan Offset Details";
        err_offset: Label 'Loan %1, must be inserted in the loan offset details';
    begin
        Loan.RESET;
    ...
    procedure getAccountbalance(accountType: Code[20]; MemberNo: Code[20]; LoanNo: Code[20])
    var
    begin
        LoanApp.Reset();
        LoanApp.SetRange("Loan  No.", LoanNo);
        if LoanApp.Find('-') then begin
            //Message('Account: %1, Member: %2.', accountType, MemberNo);
            Vend.Reset();
            Vend.SetAutoCalcFields(Balance);
            Vend.SetRange("BOSA Account No", MemberNo);
            Vend.SetRange("Account Type", accountType);
            if Vend.Find('-') then begin
                // Message('%1, %2.', accountType, MemberNo);
                Vend.CalcFields(Balance);
                if Vend.Balance > 0 then begin
                    //Message('Member %1 has a %2 account with a balance of %3.', MemberNo, Vend."Account Type Name", Vend.Balance);
                    "Member Deposits" := Vend.Balance;
                    LoanApp.Modify();
                end
                else begin
                    Error('The member''s %1 account has a balance of %2.', Vend."Account Type Name", Vend.Balance);
                end;
            end
            else begin
                Error('Member %1 must have an active %2 account.', MemberNo, Vend."Account Type Name")
            end;
        end;
    end;
}
