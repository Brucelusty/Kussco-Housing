table 50063 "Bank Reconciliation Header"
{
    Caption = 'Bank Reconciliation Header';
    ...
    fields
    {
        ...
        {
            FieldClass = FlowField;
            CalcFormula = sum("Bank Account Ledger EntryC".Amount where(Reconciled = filter(true), "Statement No." = field("Statement No."), "Bank Account No." = field("Bank Account No.")));

        }
        field(5001; "Unreconciled"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = sum("Bank Account Ledger EntryC".Amount where(Reconciled = filter(false), "Statement No." = field("Statement No."), "Bank Account No." = field("Bank Account No.")));

        }
        field(5002; "Cleared Cheques and Payments"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = sum("Bank Account Ledger EntryC".Amount where(Reconciled = filter(true), Amount = filter(< 0), "Statement No." = field("Statement No."), "Bank Account No." = field("Bank Account No.")));
        ...
        {
            FieldClass = FlowField;
            CalcFormula = sum("Bank Account Ledger EntryC".Amount where(Reconciled = filter(false), Amount = filter(< 0), "Statement No." = field("Statement No."), "Bank Account No." = field("Bank Account No.")));
