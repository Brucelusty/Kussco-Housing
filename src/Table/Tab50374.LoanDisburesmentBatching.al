...
Table 50374 "Loan Disburesment-Batching"
{
    DrillDownPageId = "Loans Disbursment  List";
    LookupPageId = "Loans Disbursment  List";
    ...
    fields
    {
        ...
        {
            OptionCaption = 'Individual Cheques,Cheque,Transfer to FOSA,FOSA Loans,EFT,RTGS,Agency';
            OptionMembers = "Individual Cheques",Cheque,"Transfer to FOSA","FOSA Loans",EFT,RTGS,//New
            Agency;
            InitValue = "Transfer to FOSA";
            Editable = false;
            trigger OnValidate()
            begin
        ...
        field(32; "Disbursement Type"; Option)
        {
            OptionMembers = ,Normal,Express;
            OptionCaption = ',Normal,Express';
        }
        //New from .71
        field(35; "Approved By"; Code[40])
        {
            Editable = false;
        }

        field(36; Overdraft; Boolean)
        {
        }
        field(37; "Overdraft Reconciliation"; Boolean)
        {
        }
    }
    ...
    begin
        // loanBatches.Reset();
        // loanBatches.SetAutoCalcFields("No of Loans");
        // loanBatches.SetRange("No of Loans", 0);
        // loanBatches.SetRange(Status, loanBatches.Status::Open);
        // if loanBatches.Find('-') then begin
        //     Error('There exists an unused loan batch: %1. Kindly use it instead.', loanBatches."Batch No.");
        // end;

        if "Batch No." = '' then begin
            SalesSetup.Get();
            SalesSetup.TestField(SalesSetup."Loans Batch Nos");
            NoSeriesMgt.GetNextNo(SalesSetup."Loans Batch Nos");
