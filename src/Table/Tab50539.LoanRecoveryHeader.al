...
Table 50539 "Loan Recovery Header"
{
    ...
    fields
    {
        ...
        {
            OptionCaption = ' ,Recover From Loanee Deposits,Attach Defaulted Loans to Guarantors,Attach Defaulted Loans to GuarantorsFOSA,Recover From Guarantors Deposits,Recover From Loanee FOSA';
            OptionMembers = " ","Recover From Loanee Deposits","Attach Defaulted Loans to Guarantors","Attach Defaulted Loans to GuarantorsFOSA","Recover From Guarantors Deposits","Recover From Loanee FOSA";
        ...
        field(20; "Loan to Attach"; Code[20])
        {
            ...
            begin
                ...
                "Loan Liabilities" := 0;
                "Loan Distributed to Guarantors" := 0;
                //   Modify();
                TotalInterestDue := 0;
                TotalThirdParty := 0;
                ...
                if LoanRec.Find('-') then begin
                    LoanRec.CalcFields(LoanRec."Outstanding Balance", LoanRec."Outstanding Interest", "Outstanding Penalty");
                    ...
                    TotalInterestDue := Round(LoanRec."Outstanding Interest" + LoanRec."Outstanding Penalty");
                ...
                "Loan Distributed to Guarantors" := ("Loan Liabilities" + "Total Interest Due Recovered") - ("Deposits Aportioned");
                // Message('onyango %1', "Loan Liabilities");

                LoanGuarantors.Reset();
                LoanGuarantors.SetRange("Loan No", "Loan to Attach");
                if LoanGuarantors.FindSet() then begin
                    LoanGuarantors.CalcSums("Amont Guaranteed");
                    "Total Guaranteed" := LoanGuarantors."Amont Guaranteed";
                end;
        ...
        field(30; "Loan Disbursement Date"; Date)
        {
            ...
            begin
                ...
                if DAY <= 23 then begin
                    "Repayment Start Date" := CalcDate('CM', Today);
                end else begin
                    "Repayment Start Date" := CalcDate('CM', CalcDate('CM+1M', Today));
                end;
                "Expected Date of Completion" := CalcDate(Format(Installments) + 'M', Today);
            end;
        }
        ...
        field(42; "Recover Single Loan"; Boolean)
        {
            trigger OnValidate()
            begin

                if "Current Shares" - "Total Outstanding Loans" < 0 then begin
                    "Recover Single Loan" := true;
                end;

                if "Recover Single Loan" = false then begin
                    "Loan to Attach" := '';
                end;
            end;
        }
        field(43; "Attach Closure Fee"; Boolean)
        {
        }
        field(44; "Total Guaranteed"; Decimal)
        {
        }
        field(45; "Amount to be Recovered"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = sum("Loan Member Loans"."Guarantor Amount Apportioned" where("Document No" = field("Document No"), "Loan No." = field("Loan to Attach")));
        }
        field(46; "Guarantors"; Integer)
        {
            FieldClass = FlowField;
            CalcFormula = count("Loans Guarantee Details" where("Loan No" = field("Loan to Attach")));
        }
    }
    ...
    begin
        ...
        if "Document No" = '' then begin
            ...
            NoSeriesMgt.GetNextNo(SalesSetup."Change Request No");
        end;
    ...
    begin
        ...
        if ObjLoans.Find('-') then begin
            repeat
                ...
                if ObjLoans."Total Outstanding Balance" > 0 then begin
                    RecAmount := ObjLoans."Total Outstanding Balance" + RecAmount;
                end;
            until ObjLoans.Next = 0;
        end;
