...
Table 50431 "Transactions"
{
    ...
    fields
    {
        ...
        {
            TableRelation = if ("Member No" = filter(<> '')) Vendor."No." where("BOSA Account No" = field("Member No"), Status = filter(<> Closed | Deceased), Blocked = filter(<> Payment | All))
            else
            if ("Member No" = filter('')) Vendor."No." where(Status = filter(<> Closed | Deceased), Blocked = filter(<> All | Payment));
            ...
            begin
                ...
                "Book Balance" := Account.Balance - "Minimum Account Balance";
        ...
        field(4; "Transaction Type"; Code[20])
        {
            ...
            TableRelation = if ("Savings Product" = filter('103|102|101')) "Transaction Types" where("Account Type" = field("Savings Product"))
            else
            if ("Account No" = filter(<> '')) "Transaction Types" where("Legible For All" = filter(true))
            else
            ...
            begin
                ...
                if TransactionTypes.FindSet then begin
                    ...
                    // "Account Types" := TransactionTypes."Account Type";
                    ...
                    if Banks.Find('-') then begin
                        ...
                        "EOD Done?" := Banks."End of Day Done";
                    end;
                    //END;
        ...
        field(5; Amount; Decimal)
        {
            ...
            begin
                ...
                if ObjAccount.Get("Account No") then begin
                    if ObjAccount."Account Type" = '406' then begin
                        ...
                        if Type = 'Withdrawal' then begin
                            "Book Balance After" := AvailableBal - Amount;
                ...
                ChargeAmount := 0;
                if ("Type _Transactions" = "Type _Transactions"::"Withdrawal") and "Use Graduated Charges" = true then begin
                    ...
                    if Vend.get("Account No") then begin
                        if vend.GetAvailableBalance() < (Amount + ChargeAmount) then begin
                            Error('The member''s available balance is insufficient.');
                        end
                    end;
                end;
        ...
        field(15; "Cheque No"; Code[20])
        {
            ...
            //TableRelation="Cheques Register"."Cheque No.";
            trigger OnValidate()
            begin
                ...
                if StrLen("Cheque No") > 6 then Error('Cheque Number must be equal to 6 digits.');
                //end;
        ...
        {
            TableRelation = Customer."No." where(ISNormalMember = const(true), "Membership Status" = filter(Active | Dormant));
            ...
            begin
                ...
                if Cust.Find('-') then begin
                    ...
                    if Cust."ID No." = '' then Error('Kindly ensure this member has an ID number.');
                    if Cust.Pin = '' then Error('Kindly ensure this member has a KRA pin.');
        ...
        {
            OptionCaption = 'Cash,Deposit Slip,Cheque,MPESA';
            OptionMembers = Cash,"Deposit Slip",Cheque,MPESA;
        ...
        {
            TableRelation = if ("Transaction Type" = filter('ORDCHECK')) "Bank Account"."No." where("Cheque Clearing Bank" = filter(true))
            else
            if ("Transaction Type" = filter('CHQW')) "Bank Account"."No." where("Cheque Clearing Bank" = filter(true));
                ...
                if Confirm('Are you sure you want to bounce this cheque', false) = true then begin
                    ...
                    if ChequeTypes.Get("Cheque Type") then begin
                        ...
                        if "Branch Transaction" = true then
                            ...
                            GenJournalLine.Description := TransactionTypes.Description + '-' + 'Bounced';
                        //Project Accounts
                        if Acc.Get("Account No") then begin
                            // if Acc."Account Category" = Acc."account category"::Project then
                            //     GenJournalLine.Description := TransactionTypes.Description + '-' + "Branch Refference"
                        end;
                        //Project Accounts
                        GenJournalLine.Validate(GenJournalLine."Currency Code");
        ...
        {
            OptionCaption = 'Cash Deposit,Withdrawal,Cheque Deposit,ATM Cash Deposit,ATM Cheque Deposit,ATM Withdrawal,Batch Deposit,BOSA Withdrawal,Bankers Cheque,Encashment,Transfer,Inhouse Cheque Withdrawal,Deposit Slip,Cheque Withdrawal,ABC Deposit,MPESA Withdrawal';
            OptionMembers = "Cash Deposit",Withdrawal,"Cheque Deposit","ATM Cash Deposit","ATM Cheque Deposit","ATM Withdrawal","Batch Deposit","BOSA Withdrawal","Bankers Cheque",Encashment,Transfer,"Inhouse Cheque Withdrawal","Deposit Slip","Cheque Withdrawal","ABC Deposit","MPESA Withdrawal";
        }
        ...
        field(69236; "Cheque Document Number"; Decimal)
        {
        }
        field(69237; "EOD Done?"; Boolean)
        {
            Editable = false;
        }
        field(69238; "Book Balance After"; Decimal)
        {
            Editable = false;
        }
        field(69239; "MPESABank Account"; Code[20])
        {
            TableRelation = "Bank Account"."No.";
        }
        field(69240; "Destination Allocated Amount"; Decimal)
        {
        ...
        field(69241; "Cheque Destination Type"; Option)
        {
        ...
        field(69242; "Clearing Days."; integer)
        {
            Editable = false;

        }
        field(69243; "Amount Posted"; Decimal)
        {
            CalcFormula = Sum("Bank Account Ledger Entry".Amount WHERE("Document No." = field(No)));
            Editable = false;
            FieldClass = FlowField;

        }
        field(69244; "Withdrawal Fee"; Decimal)
        {
            ...
            var
                myInt: Integer;
            begin
                "Balance CF" := Account.Balance - "Minimum Account Balance" - "Withdrawal Fee";
            end;


        }
        field(69245; "Bank Used To Post"; code[20])
        {
        ...
        field(69246; "Balance CF"; Decimal)
        {
            Editable = false;
            FieldClass = Normal;
        }
        field(69247; "Transaction ID"; Code[20])
        {

        }
        field(69248; "Withdrawal Fee Excise"; Decimal)
        {

        }

        field(69249; "Account Types"; Code[100])
        {
            Editable = false;
        }
        field(69250; Types; Enum FOSATransactionTypesEnum)
        {
            FieldClass = FlowField;
            CalcFormula = lookup("Transaction Types".Type WHERE(Code = field("Transaction Type")));

        }
        field(69251; "Till Balance"; Decimal)
        {
            Editable = false;

        }
        field(69252; "Charges."; Decimal)
        {
            Editable = false;

        }
        field(69253; "Bank Branch Name"; Text[100])
        {

        }
        field(69254; "Cheque Status"; Option)
        {
    ...
    begin
        ...
        if No = '' then begin
            ...
            NoSeriesMgt.GetNextNo(NoSetup."Transaction Nos.");
        end;
        ...
        Banks.Reset();
        Banks.SetRange(Banks."Cashier ID", UserId);
        Banks.SetRange(Banks."Account Type", Banks."Account Type"::Cashier);
        if Banks.FindFirst() then begin
            Banks.CalcFields(Banks.Balance);
            "Till Balance" := Banks.Balance;
        end;
