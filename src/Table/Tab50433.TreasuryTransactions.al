...
Table 50433 "Treasury Transactions"
{
    ...
    fields
    {
        ...
        field(3; "Transaction Type"; Option)
        {
            ...
            begin
                if "Transaction Type" = "transaction type"::"End of Day Return to Treasury" then BEGIN
                    ...
                    BankAcc.SetRange(BankAcc."Account Type", BankAcc."Account Type"::Cashier);
                    if BankAcc.Find('-') then begin
                        //Message('here%1',BankAcc."No.");
                        BankAcc.CalcFields(BankAcc.Balance);
                        "From Account" := BankAcc."No.";
                        "From Teller" := BankAcc."No.";
                        "From Account User" := BankAcc.CashierID;
                        "Source Account Balance" := BankAcc.Balance;
                        "Actual Teller Till Balance" := BankAcc.Balance;
                        //Modify;
                    end;
                ...
                if "Transaction Type" = "transaction type"::"Teller to Intra-Day" then begin
                    Description := 'TELLER TO BULK CASH';
                end;

                if "Transaction Type" = "transaction type"::"Intra-Day to Treasury" then begin
                    Description := 'BULK CASH TO TELLER';
                end;

                if "Transaction Type" = "transaction type"::"Treasury to Intra-Day" then begin
                    Description := 'TELLER TO CASH HELD OVERNIGHT';
                end;
                if "Transaction Type" = "transaction type"::"Intra-Day to Teller" then begin
        ...
        field(5; "To Account"; Code[20])
        {
            ...
            if ("Transaction Type" = filter("Issue To Teller" | "Inter Teller Transfers" | "Intra-Day to Teller")) "Bank Account"."No." where("Account Type" = filter(Cashier | "Petty Cash"))
            else
        ...
        field(7; Amount; Decimal)
        {
            ...
            begin
                ...
                if ObjBanks.Find('-') then begin
                    ObjBanks.CalcFields(ObjBanks.Balance);
                    "Excess Account" := ObjBanks."Teller Excess Account";
                    "Shortage Account" := ObjBanks."Teller Shortage Account";
                    ...
                    if Rec."Transaction Type" = rec."Transaction Type"::"End of Day Return to Treasury" then begin
                        if AvailableBal <> Amount then begin
                            rec.TestField("Shortage Account");
                            rec.TestField("Excess Account");
                            "Excess/Shortage Amount" := Amount - AvailableBal;
                            ;
                        end
                    end;
                end;
        ...
        field(55; "To Teller"; Code[20])
        {
            ...
            begin
                ...
                if Teller.Get("To Teller") then begin
                    ...
                    if Teller."Account Type" = Teller."Account Type"::"Petty Cash" then begin
                        "Is Petty Cash" := true;
                        "From Account" := fundsGen."Petty Cash Reimbursement Acc";
                    end;
                end;
            end;
        }
        ...
        field(59; "Custodian 1 Name_Issue"; Code[200])
        {
        }
        field(60; "Custodian 2 Confirm Issue"; Boolean)
        {
        }
        field(61; "Custodian 2 Name_Issue"; Code[200])
        {
        }
        field(62; "Custodian 1 Confirm Receipt"; Boolean)
        {
        }
        field(63; "Custodian 1 Name_Receipt"; Code[200])
        {
        }
        field(64; "Custodian 2 Confirm Receipt"; Boolean)
        {
        }
        field(65; "Custodian 2 Name_Receipt"; Code[200])
        {
        }
        field(66; "Excess Account"; Code[200])
        {
            // Editable = false;
        }
        field(67; "Shortage Account"; Code[200])
        {
            // Editable = false;
        }
        field(68; requested; Boolean)
        {
        }
        field(69; "Requested Time"; Time)
        {
        }
        field(70; "Requested Date"; Date)
        {
        }
        field(71; "Amount to request"; Decimal)
        {
            trigger OnValidate()
            begin
                IF "Amount to request" < 0 THEN Error('You cannot request Zero');
                Amount := "Amount to request";
            end;
        }
        field(72; Requester; Code[30])
        {
        }
        field(73; "Is Petty Cash"; Boolean)
        {
        }

        field(74; "Ignore Trans"; Boolean)
        {
        }
    ...
    begin
        if No = '' then begin
            NoSetup.Get();
            if "Is Petty Cash" then begin
                NoSetup.TestField("Petty Cash Requisition Nos");
                NoSeriesMgt.GetNextNo(NoSetup."Petty Cash Requisition Nos");
            end else begin
                NoSetup.TestField("Treasury Transactions No");
                NoSeriesMgt.GetNextNo(NoSetup."Treasury Transactions No");
            end;
        end;
