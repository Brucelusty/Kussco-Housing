...
Table 50447 "EFT/RTGS Header"
{
    ...
    fields
    {
        field(1; No; Code[20])
        {
            ...
            begin
                if No <> xRec.No then begin
                    if "Expected Amount" >= 1000000 then begin
                        NoSetup.Get();
                        NoSeriesMgt.TestManual(NoSetup."RTGS Header Nos.");
                        "No. Series" := '';
                    end else begin
                        NoSetup.Get();
        ...
        field(25; "Expected Amount"; Decimal)
        {
        }
        field(26; "Loan Buy-Off"; Boolean)
        {
        }
        field(27; "Reason for EFT/RTGS"; Text[1800])
        {
        }
        field(28; "Reason for EFT/RTGS Failure"; Text[1800])
        {
        }
        field(29; Failed; Boolean)
        {
        }
    }
    ...
    begin
        if No = '' then begin
            if "Expected Amount" >= 1000000 then begin
                NoSetup.Get();
                NoSetup.TestField(NoSetup."RTGS Header Nos.");
                NoSeriesMgt.GetNextNo(NoSetup."RTGS Header Nos.");
            end else begin
                NoSetup.Get();
                NoSetup.TestField(NoSetup."EFT Header Nos.");
                NoSeriesMgt.GetNextNo(NoSetup."EFT Header Nos.");
            end;
        end;
