...
Table 50384 "Receipt Allocation"
{
    ...
    fields
    {
        ...
        field(2; "Member No"; Code[20])
        {
            NotBlank = true;
            TableRelation =
            if ("Account Type" = const(Customer)) Customer where(ISNormalMember=const(true),"No."=field(MemberNo))
            else
            if ("Account Type" = const(Vendor)) Vendor where("Creditor Type" = filter("FOSA Account"),"BOSA Account No"=field(MemberNo));
            ...
            begin
                ...
                begin
                    ...
                    "Destination Acount Name":=Cust.Name
                end;
                if "STO Account Type"="STO Account Type"::"FOSA Account" then 
                begin
                    Ven.Reset();
                    Ven.SetRange(Ven."No.","Member No");
                    if Ven.FindFirst() then begin 
                    "Destination Acount Name":=Ven.Name;
                    AccountType.Reset();
                    AccountType.SetRange(AccountType.Code,Ven."Account Type");
        ...
        field(4; "Loan No."; Code[20])
        {
            TableRelation = "Loans Register"."Loan  No." where("Client Code" = field("Member No"),
                                                                "Loan Status" = filter(<>Application & <>Closed & <>Rejected & <>Open));
        ...
        field(5; Amount; Decimal)
        {
            ...
            trigger OnValidate()
            begin
                if (("Transaction Type" = "transaction type"::"Interest Paid") or ("Transaction Type" = "transaction type"::Loan) or ("Transaction Type" = "Transaction Type"::Repayment)) then begin
                    if "Loan No." = '' then
        ...
        field(12; "Portal Deduction Type"; Code[20])
        {
        }
        field(172000; "Loan Insurance"; Decimal)
        {
            BlankZero = true;
        ...
        field(172001; "Applied Amount"; Decimal)
        {
            CalcFormula = lookup("Loans Register"."Approved Amount" where("Loan  No." = field("Loan No.")));
            FieldClass = FlowField;
        }
        field(172002; Insurance; Decimal)
        {
        }
        field(172003; "Un Allocated Amount"; Decimal)
        {
        }
        ...
        {
            OptionCaption = ',Member BOSA Savings,Member FOSA Savings,,,,,';
        ...
        field(51516160; "Account No"; Code[100])
        {
            ...
            if ("Account Type" = const(Customer)) Customer where(ISNormalMember=const(true))
            else
        ...
        field(172100; "Entry No1"; Integer)
        {
            AutoIncrement = true;
        }
        field(172101; "Transaction Type Defined"; Code[100])
        {
            TableRelation = "Defined Transaction Type"."Defined Transaction Type";


        }
        field(172102; "STO Account Type"; Option)
        {
           
            OptionCaption = ' ,Member,Savings Account,G/L Account';
            OptionMembers = " ",Member,"FOSA Account","G/L Account";
            trigger OnValidate()
            var
                myInt: Integer;
            begin
                if "STO Account Type"="STO Account Type"::Member then "Account Type":="Account Type"::Customer
                else if "STO Account Type"="STO Account Type"::"FOSA Account" then "Account Type":="Account Type"::Vendor
                else if "STO Account Type"="STO Account Type"::"G/L Account" then "Account Type":="Account Type"::"G/L Account";            
            end;
        }
        field(172103; "Destination Acount Name"; text[2048])
        {
            Editable=false;
        }
        field(172104; "STO Transaction Type"; Option)
        {
           
            OptionCaption = ' ,Share Capital,FOSA Account,Principal Repayment,Interest Repayment,Deposit Contribution,BBF Fund';
            OptionMembers = " ","Share Capital","FOSA Account","Principal Repayment","Interest Repayment","Deposit Contribution","BBF Fund";
            trigger OnValidate()
            var
                myInt: Integer;
            begin
                if "STO Transaction Type"="STO Transaction Type"::"Share Capital" then "Transaction Type":="Transaction Type"::"Share Capital"
                else if "STO Transaction Type"="STO Transaction Type"::"FOSA Account" then "Transaction Type":="Transaction Type"::" "
                else if "STO Transaction Type"="STO Transaction Type"::"Principal Repayment" then "Transaction Type":="Transaction Type"::Repayment
                else if "STO Transaction Type"="STO Transaction Type"::"Interest Repayment" then "Transaction Type":="Transaction Type"::"Interest Paid"
                else if "STO Transaction Type"="STO Transaction Type"::"Deposit Contribution" then "Transaction Type":="Transaction Type"::"Deposit Contribution"
                else if "STO Transaction Type"="STO Transaction Type"::"BBF Fund" then "Transaction Type":="Transaction Type"::" ";
                
                
            end;
        }
        field(172105; "Account Type Name"; text[2048])
        {
            Editable=false;
        }
                field(172106; "MemberNo"; Code[60])
        {
            Editable=false;
        }
    }
    ...
    keys
    {
        key(Key1; "Document No",  "Entry No1","MemberNo")
        {
            Clustered = true;
        } 

/*         key(Key1; "Document No", "Transaction Type", "Account Type", "Account No", "Member No", Amount, "Entry No1", Description)
        {
            Clustered = true;
        } */
        key(Key2; "Loan No.")
