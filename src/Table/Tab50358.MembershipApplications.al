...
Table 50358 "Membership Applications"
{
    ...
    fields
    {
        ...
        field(68995; "Non MOU"; Boolean)
        {
            trigger OnValidate()
            begin
                if "Non MOU" = true then begin
                    "Employer Code" := '';
                    "Employer Name" := '';
                    "Employer Address" := '';
                    "Payroll No" := '';
                end else begin
                    "Business Employer Code" := '';
        ...
        field(68996; "Business Employer Address"; Code[20])
        {
        }
        field(68998; "Business Employer Name"; Code[100])
        { }
        field(68999; "Business Employer Code"; Code[150])
        {
            TableRelation = "Non MOU Employers";

            trigger OnValidate()
            var
                Employers: Record "Non MOU Employers";
            begin
                /*Employer.GET("Employer Code");
                "Employer Name":=Employer.Description;*/
                Employers.Reset;
                Employers.SetRange(Employers."Employer No.", "Employer Code");
                if "Employment Info" = "Employment Info"::"Self-Employed" then begin
                    if Employers.Find('-') then begin

                        "Business Employer Code" := Employers."Employer No.";
                        "Business Employer Name" := Employers."Employer Name";
                        "Business Employer Address" := Employers."Employer Address";
                        // "Member Payment Type" := Employers."Payment Type";
                    end;
                end;
            end;
        }
        ...
        field(68002; "Membership Approval Status"; Option)
        {
            OptionCaption = 'Open,Pending Approval,Approved,Rejected';
            OptionMembers = Open,"Pending Approval",Approved,Rejected;
        }
        field(68003; "Employer Code"; Code[150])
        {
            TableRelation = if ("Employment Info" = const(Employed)) "Employers Register"."Employer Code" where("Self-Employed" = filter(false))
            else if ("Employment Info" = const("Self-Employed")) "Employers Register"."Employer Code" where("Self-Employed" = filter(true));
            ...
            begin
                ...
                "Employer Name":=Employer.Description;*/
                Employers.Reset;
                Employers.SetRange(Employers."Employer Code", "Employer Code");
                // if "Employment Info" = "Employment Info"::employed then begin

                // end;
                if Employers.Find('-') then begin
                    "Employer Code" := Employers."Employer Code";
                    "Employer Name" := Employers."Employer Name";
                    "Employer Address" := Employers."Employer Address";
                    // "Member Payment Type" := Employers."Payment Type";
                end;
                if "Employer Code" = 'BUSINESS COMP' then begin
                    SalesSetup.Get();
                    SalesSetup.TestField(SalesSetup."Business Comp Payroll Nos");
                    NoSeriesMgt.GetNextNo(SalesSetup."Business Comp Payroll Nos"
                  );
                    "Employment Info" := "Employment Info"::"Self-Employed";
                end else
                    if "Employer Code" = 'PRIME TECH' then begin
                        SalesSetup.Get();
                        SalesSetup.TestField(SalesSetup."Prime Tech Payroll Nos");
                        NoSeriesMgt.GetNextNo(SalesSetup."Prime Tech Payroll Nos"
                       );
                    end else
                        if "Employer Code" = 'RSL' then begin
                            SalesSetup.Get();
                            SalesSetup.TestField(SalesSetup."Reat Security Payroll Nos");
                            NoSeriesMgt.GetNextNo(SalesSetup."Reat Security Payroll Nos"
                           );
                        end;
            end;
        }
                ...
                if "Account Category" = "account category"::Individual then begin
                    if "Date of Birth" <> 0D then begin
                        if GenSetUp.Get() then begin
                            ...
                            GenSetUp.TestField("Retirement Age");
        ...
        field(68005; "E-Mail (Personal)"; Text[50])
        {
            ...
            trigger OnValidate()
            var
                Customer: Codeunit "Mail Management";
            begin
                Customer.CheckValidEmailAddress("E-Mail (Personal)");
            end;
        }
        ...
        field(68012; "ID No."; Code[30])
        {
            ...
            begin
                Cust.Reset();
                Cust.SetRange("ID No.", "ID No.");
                if Cust.Find('-') then begin
                    Error('A member already exists with the same ID No. %1.', "ID No.");
                end;
                ...
                if (StrLen("ID No.") < 5) or (StrLen("ID No.") > 10) then
                    Error('ID No Can not be less than 5 Characters or more than 10 characters');
        ...
        field(68013; "Mobile Phone No"; Code[50])
        {
            ...
            begin
                ...
                if (StrLen("Mobile Phone No") <> 12) then
                    Error('Mobile No. Can not be less than 12 Characters');

                if "Mobile Phone No" <> '' then begin
                    if "Account Category" = "Account Category"::Individual then begin
                        Cust.Reset;
                        Cust.SetRange(Cust."Mobile Phone No", "Mobile Phone No");
                        Cust.SetRange(Cust."Membership Status", Cust."Membership Status"::Active);
                        if Cust.Find('-') then
                            Error('This mobile phone number is already in use by an active member. Kindly use another number.');
                    end;
                end;
                ...
                if "Mobile Phone No" <> '' then begin
                    ...
                    if ObjMemberApplication.Find('-') = true then
                        Error('The mobile phone number %1 is already in use. Kindly use another number', ObjMemberApplication."Mobile Phone No");
                end;
            end;
        }
        field(68234; "Group Mobile Phone No"; Code[50])
        {
            InitValue = '254';
            trigger OnValidate()
            var
            ...
            begin
                ...
                if (StrLen("Group Mobile Phone No") <> 12) then
                    Error('Mobile No. Can not be less than 12 Characters');
            end;
        }
        ...
        field(68022; "Office Telephone No."; Code[50])
        {
            ...
            begin
                ...
                if (StrLen("Office Telephone No.") <> 12) then
                    Error('Mobile No. Can not be less than 12 Characters');
            end;
        }
        ...
        field(68027; "Village/Residence"; Text[2048])
        {
        }
        field(68028; "File Movement Remarks"; Text[2048])
        {
            Enabled = false;
        }
        ...
        field(68033; Occupation; Text[2048])
        {
        }
        field(68034; Designation; Code[2048])
        {
            // TableRelation = Designation.Designation;
        }
        // field(68035; "Terms of Employment"; Option)
        // {
        //     OptionCaption = 'Permanent & Pensionable,Contract';
        //     OptionMembers = " ","Permanent & Pensionable",Contract;
        // }
        ...
        field(68041; "Approved By"; Code[40])
        {
        }
        field(68042; "Sent for Approval By"; Code[50])
        {
        }
        ...
        field(68045; County; Code[30])
        {
            TableRelation = Counties."County Code";
            trigger OnValidate()
            begin
                countyCode.Reset();
                countyCode.SetRange("County Code", County);
                if countyCode.Find('-') then begin
                    CountyName := countyCode."County Name";
                    Region := countyCode.Region;
                end;
            end;
        }
        field(69280; CountyName; Text[250])
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }
        field(69231; "Sub-county"; Text[250])
        {
            TableRelation = "Sub-County" where("County Code" = field(County));
        }
        field(69282; "Workstation Region"; Code[20])
        {
        }
        field(69283; "Workstation County"; Code[30])
        {
            TableRelation = Counties."County Code";
            trigger OnValidate()
            begin
                ...
                countyCode.SetRange("County Code", County);
                if countyCode.Find('-') then begin
                    "Workstation County Name" := countyCode."County Name";
                    "Workstation Region" := countyCode.Region;
                end;
            end;
        }
        field(69284; "Workstation County Name"; Text[250])
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }
        ...
        field(68051; "Recruited By"; Code[40])
        {
        ...
        field(68055; "Employer Name"; Code[100])
        {
        }
        field(68056; Title; Enum "TitleEnum")
        {
        ...
        field(68064; "Created By"; Text[40])
        {
        }
        ...
        field(68070; "Account Category"; Enum AccountCategoryEnum)
        {
            /*             OptionCaption = 'Individual,Joint,Corporate';
                        OptionMembers = Individual,Joint,Corporate; */
        }
        ...
        field(68084; "Date of Birth2"; Date)
        {
            ...
            trigger OnValidate()
            begin
                if "Date of Birth2" > Today then
                    Error('Date of birth cannot be greater than today');


                if "Date of Birth2" <> 0D then begin
                    if GenSetUp.Get() then begin
                        if CalcDate(GenSetUp."Min. Member Age", "Date of Birth2") > Today then
                            Error('Applicant bellow the mininmum membership age of %1', GenSetUp."Min. Member Age");
                        GenSetUp.TestField("Retirement Age");
                        "Retirement Date" := CalcDate(GenSetUp."Retirement Age", "Date of Birth2");
                    end;
                end;
            end;
        }
            ...
            begin
                ...
                */
                ...
                if CustMember.FindSet then begin
                    ...
                    "Group Mobile Phone No" := CustMember."Group Mobile Phone No";
        ...
        field(69132; "Captured By"; Code[40])
        {
        }
        ...
        field(69271; "Customer Service Rep."; Code[20])
        {
            TableRelation = "HR Employees"."No.";
            trigger
            OnValidate()
        ...
        field(69272; "Customer Service Rep. Name"; Text[200])
        {
        ...
        field(69180; "Business Name"; Code[800])
        {
            TableRelation = "BSS Businesses"."Business Name";
        }
        ...
        field(69223; "Reffered By Member No"; Code[100])
        {
            DataClassification = ToBeClassified;
            TableRelation = if ("How Did you Know us?" = filter('Member|Staff')) Customer where(ISNormalMember = const(true));
            trigger OnValidate()
            var
        ...
        field(69224; "Reffered By Member Name"; Text[100])
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }

        field(69225; "NHIF No"; Code[100])
        {
            DataClassification = ToBeClassified;
        }

        field(69226; "Religion."; Enum MemberReligionEnum)
        {
            // DataClassification = ToBeClassified;
        ...
        field(69228; "Member Type"; Option)
        {
            OptionMembers = Member,Staff,Board,"Station Representative";
            OptionCaption = 'Member,Staff,Director,';

        }

        field(69229; "Exempted From Tax"; Boolean)
        {

        }

        field(69230; "How did you know about us?"; Text[1200])
        {

        }
        field(69232; "Area"; Code[100])
        {

        }
        field(69233; "Work E-Mail"; Code[100])
        {

        }

        field(69234; "Nearest Landmark"; Text[100])
        {

        }

        field(69235; "Mode Of Remmittance"; Option)
        {
        ...
        field(69236; "Religion"; Enum MemberReligionEnum)
        {
            DataClassification = ToBeClassified;
        ...
        field(69237; "Station Representative"; Text[100])
        {
            DataClassification = ToBeClassified;

        }

        field(69238; "Retirement Date"; Date)
        {
            DataClassification = ToBeClassified;
            Editable = false;
        ...
        field(69239; "Receive SMS Notification"; Boolean)
        {
            DataClassification = ToBeClassified;
        ...
        field(69240; "Buying Center"; Text[1000])
        {
            DataClassification = ToBeClassified;
            TableRelation = "Workstation Buying Centers"."Buying Centre" where(Factory = field(Workstation));
        }
        field(69241; "Workstation"; Code[100])
        {
            DataClassification = ToBeClassified;
            TableRelation = WorkStations.Code where(EmployerCode = field("Employer Code"), Region = field("Workstation Region"));
        }
        field(69242; "New/Rejoining"; Option)
        {
            OptionCaption = 'New,Rejoining';
            OptionMembers = New,Rejoining;
        }
        field(69243; "How Did you Know us?"; Enum "Lead Source")
        {

        }
        field(69244; "Registration Fee"; Decimal)
        {

        }
        field(69245; "Registration Fee Comm %"; Decimal)
        {

        }
        field(69246; "Registration Fee Comm Amount"; Decimal)
        {

        }
        field(69247; "Why Exempt from Tax?"; Text[2048])
        {

        }
        field(69248; "Date Format"; Text[2048])
        {

        }
        field(69249; "Employment Start Date"; Date)
        {
            trigger OnValidate()
            var
                myInt: Integer;
            begin
                "Employment End Date" := CalcDate("Employment Period", "Employment Start Date")
            end;
        }
        field(69250; "Employment End Date"; Date)
        {

        }

        field(69251; "Employment Period"; DateFormula)
        {
            trigger OnValidate()
            var
                myInt: Integer;
        ...
        field(69252; "Tax Exemption Start Date"; Date)
        {
            trigger OnValidate()
            var
                myInt: Integer;
            begin
                "Tax Exemption End Date" := CalcDate("Tax Exemption Period", "Tax Exemption Start Date")
            end;
        }
        field(69253; "Tax Exemption End Date"; Date)
        {

        }

        field(69254; "Tax Exemption Period"; DateFormula)
        {
            trigger OnValidate()
            var
                myInt: Integer;
        ...
        field(69255; "ID Front"; MediaSet)
        {

        }
        field(69256; "ID Back"; MediaSet)
        {

        }
        field(69257; "Mode Of Application"; Option)
        {
            DataClassification = ToBeClassified;
            OptionCaption = ',Walk-In,Point Of Sale,Online';
            OptionMembers = ,"Walk-In","Point Of Sale",Online;
            InitValue = "Walk-In";
        }
        field(69258; "Rejoining Fee"; Decimal)
        {
        }
        field(69259; "How Many"; Integer)
        {
        }
        field(69260; "Mode of Payment"; Option)
        {
            OptionCaption = 'Cash,Paybill,Checkoff,Salary';
            OptionMembers = Cash,Paybill,Checkoff,Salary;

            trigger OnValidate()
            begin
                if "Mode of Payment" = "Mode of Payment"::Cash then begin

                end else if "Mode of Payment" = "Mode of Payment"::Paybill then begin

                end else if "Mode of Payment" = "Mode of Payment"::Checkoff then begin

                end else if "Mode of Payment" = "Mode of Payment"::Salary then begin

                end;
            end;
        }
        field(69261; "Transaction Reference Number"; Code[50])
        {
        }
        field(69262; "Main Sector"; Code[20])
        {
            DataClassification = ToBeClassified;
            Editable = false;
        ...
        field(69263; "Sub Sector"; Code[20])
        {
            //DataClassification = ToBeClassified;
            Editable = false;
        ...
        field(69264; "Sector Specific"; Code[20])
        {
            DataClassification = ToBeClassified;
        ...
        field(69265; "Sector Specific Name"; Text[2048])
        {

        }
        field(69266; "Main Sector Name"; Text[2048])
        {

        }
        field(69267; "SubSector Name"; Text[2048])
        {

        }
        field(69268; "Sales Person"; Code[100])
        {
            TableRelation = "HR Employees"."No.";
        }
        field(69269; "Registration Status"; option)
        {
            OptionCaption = 'Paid,NotPaid,Underpaid';
            OptionMembers = Paid,NotPaid,Underpaid;
        }
        field(69270; "Membership Operational Status"; option)
        {
            OptionCaption = 'Active, Dormant, Closed, Deceased, Awaiting Withdrawal, Withdrawal';
            optionMembers = Active,Dormant,Closed,Deceased,"Awaiting Withdrawal",Withdrawal;
        }
        field(69275; "Membership Class Tier"; Option)
        {
            OptionCaption = ' ,Class A, Class B, Class C, Class D, Class F-A, Class F-B, Class F-C';
            OptionMembers = " ","Class A","Class B","Class C","Class D","Class FA","Class FB","Class FC";
        }
        field(69276; BBF; Decimal)
        {

        }
        field(69277; ShareCapital; Decimal)
        {

        }
        field(69278; Salary; Decimal)
        {

        }
        field(69279; "Erroneous Application"; Boolean)
        {
        }
        field(69285; "Member Region"; code[30])
        {
            TableRelation = "Dimension Value".Code where("Global Dimension No." = filter(2));
        }
        field(69286; "Lead No"; code[30])
        {

        }
        field(69287; "Referred By"; Option)
        {
            OptionCaption = ' ,Member,Self,Staff';
            OptionMembers = " ","Member",Self,Staff;
        }
        field(69288; "Downline ID"; Code[60])
        {
            DataClassification = ToBeClassified;
            TableRelation = if ("How Did you Know us?" = filter('Member|Staff')) Customer where(ISNormalMember = const(true));
        }
        field(69289; "MLM Level"; Code[20])
        {
            TableRelation = "MLM Level Setup"."Level Code";

        }
        field(69290; "Upline ID"; Code[60])
        {
            DataClassification = ToBeClassified;
            TableRelation = if ("How Did you Know us?" = filter('Member|Staff')) Customer where(ISNormalMember = const(true));
        }
        field(69291; "Marketing Campaign ID"; Code[40])
        {
            Caption = 'Marketing Campaign ID';
            ToolTip = 'Specifies Marketing Campaign ID';
            TableRelation = "Marketing Campaign"."Campaign ID";
        }
        field(69292; "Marketing Event ID"; Code[40])
        {
    ...
    begin
       ...
       ERROR('Status must be pending');*/
        if ("Membership Approval Status" = "Membership Approval Status"::Approved) or ("Membership Approval Status" = "Membership Approval Status"::"Pending Approval") or ("Membership Approval Status" = "Membership Approval Status"::Rejected) then begin
    ...
    begin
        ...
        if "No." = '' then begin
            ...
            /*             if "Account Category" = "Account Category"::Corporate then begin
                            ...
                            "No." := NoSeriesMgt.GetNextNo(SalesSetup."Member Application Nos");
        ...
        "Recruited By" := UserId;
        "Receive SMS Notification" := true;
        //"Monthly Contribution":=GenSetUp."Min. Contribution";
        "Registration Fee" := GenSetUp."BOSA Registration Fee Amount";
        "Registration Fee Comm %" := GenSetUp."BOSA RegistrationFee (%)";
        "Captured By" := UserId;
        "Customer Posting Group" := 'MEMBER';
        "Global Dimension 1 Code" := '';
        if UserSetup.Get(UserId) then begin
            "Global Dimension 2 Code" := UserSetup."Branch Code";
        end;
        ...
        if ObjAccountTypes.FindSet() then begin
            repeat
                //Message('Here%1App%2',ObjAccountTypes.Code,"No.");
                ObjSelectProduct.Init;
                ObjSelectProduct."Membership Applicaton No" := "No.";
                ObjSelectProduct.Product := ObjAccountTypes.Code;
                ObjSelectProduct."Account Category" := Format("Account Category");
                ObjSelectProduct.Validate(ObjSelectProduct.Product);
