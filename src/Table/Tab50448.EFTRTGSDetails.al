...
Table 50448 "EFT/RTGS Details"
{
    ...
    fields
    {
        ...
        field(2; "Account No"; Code[20])
        {
            ...
            begin
                if Accounts.Get("Account No") then begin
                    //Block Payments
                    if (Accounts.Blocked = Accounts.Blocked::Payment) or (Accounts.Blocked = Accounts.Blocked::All) then Error('This account has been blocked from Transacting.');
                ...
                EFTHeader.Reset();
                EFTHeader.SetRange(No, "Header No");
                if EFTHeader.Find('-') then begin
                    "Transaction Description" := EFTHeader."Transaction Description";
                end;
        ...
        field(7; Amount; Decimal)
        {
            ...
            trigger OnValidate()
            var
                eftCharges: Record "EFT - RTGS Charges";
            begin

                eftCharges.Reset();
                eftCharges.SetFilter("Lower Limit", '<=%1', Amount);
                eftCharges.SetFilter("Upper Limit", '>=%1', Amount);
                if eftCharges.Find('-') then begin
                    Charges := eftCharges."Charge Amount";
                end;

                AvailableBal := 0;

                if Accounts.Get("Account No") then begin
                    AvailableBal := Accounts.GetAvailableBalance();
                    "Total Amount" := Amount + Charges;
                    if (Amount + Charges) > AvailableBal then Error('EFT/RTGS Amount+Charges of %1, is more than the member''s account balance %2. <br>Account No. %3', "Total Amount", AvailableBal, "Account No");
                end;
            end;
        }
    ...
    begin
        if No = '' then begin
            ...
            NoSeriesMgt.GetNextNo(NoSetup."EFT Details Nos.");
        end;
