...
Table 50204 "HR Leave Application"
{
    ...
    fields
    {
        ...
        field(4; "Days Applied"; Decimal)
        {
            ...
            begin
                ...
                "Expected Leave Bal" := "Leave Balance Per Category" - "Approved days";
            end;
        }
        field(5; "Start Date"; Date)
        {
            ...
            begin
                ...
                HRLeaveApp.Setfilter(HrLeaveApp.Status, '%1', HrLeaveApp.Status::Posted);
                HRLeaveApp.SetFilter(HRLeaveApp."Start Date", '%1..%2', "Start Date", "End Date");
                if HrLeaveApp.Find('-') then begin
                    Error('You cant apply for another leave while a previous one is still ongoing.');
                end;
        ...
        field(8; "Expected Leave Bal"; Decimal)
        {
            Editable = false;
        }
        field(9; "Portal Application"; Boolean)
        {
            Editable = false;
        }
        field(12; Status; Option)
        {
            ...
            begin
                ...
                //     if not HRLeaveEntries.Insert then HRLeaveEntries.Modify;
                // end;
            end;
        }
        ...
        field(3921; "E-mail Address"; Text[60])
        {
            ...
            begin
                MailManagement.CheckValidEmailAddresses("E-mail Address");
            end;
        }
        ...
        field(3936; "Cell Phone Number"; Text[50])
        {
            ...
            var
                myInt: Integer;
            begin
                if (StrLen("Cell Phone Number") < 10) OR (StrLen("Cell Phone Number") > 13) then
        ...
        field(3949; Reliever; Code[50])
        {
            ...
            begin
                ...
                supervisorApproval."Leave No." := "Application Code";
                supervisorApproval."Staff No" := "Employee No";
                supervisorApproval."Leave Type" := "Leave Type";
                supervisorApproval."Staff Name" := "Employee Name";
                supervisorApproval."Leave Account Balance" := "Leave Balance Per Category";
                supervisorApproval."Day Applied" := "Application Date";
                supervisorApproval."Days Applied" := "Days Applied";
                supervisorApproval."Approved Days" := "Days Applied";
        ...
        field(3986; "Leave Balance Per Category"; Decimal)
        {
            Editable = false;
        }
        field(3987; "Pending Tasks"; Text[2048])
        {
            DataClassification = ToBeClassified;
        }
        field(3988; "Supervisor Name"; Text[2048])
        {
            DataClassification = ToBeClassified;
        }
        field(3989; "Is Manager"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(3990; "Handed Over Report"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(3991; Posted; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(3992; "Date Posted"; Date)
        {
            DataClassification = ToBeClassified;
        }
        field(3993; "Time Posted"; Time)
        {
            DataClassification = ToBeClassified;
        }
    }
    ...
    begin
        ...
        if HREmp.Find('-') then begin
            ...
            if "Application Code" = '' then begin
                ...
                "Application Code":=NoSeriesMgt.GetNextNo(HRSetup."Leave Application Nos.");
            end;

            HRLeaveApp.Reset();
            HRLeaveApp.SetRange("Employee No", HREmp."No.");
            HRLeaveApp.SetFilter(Status, '%1|%2', HRLeaveApp.Status::New, HRLeaveApp.Status::"HOD Approval");
            if HRLeaveApp.Find('-') then begin
                Error('You have an existing leave application, no: %1. Kindly use it before opening a new one.', HRLeaveApp."Application Code");
            end;
            ...
            if HRSetup.Get then begin
                HRSetup.TestField(HRSetup."Probation Period");
                //  if empMonths < HRSetup."Probation Period" then Error(Text002, HRSetup."Probation Period");Kitui
            end;
            ...
            "Department Code" := HREmp."Responsibility Center Name";
            "E-mail Address" := HREmp."Company E-Mail";
            Gender := HREmp.Gender;
            "Is Manager" := HREmp.Supervisor;
            if HREmp.Supervisor = true then begin
                Status := Status::"HOD Approval";
            end;
    ...
    procedure FnValidateInsert(Emp: Code[999])
    var
        Workflowintegration: codeunit WorkflowIntegration;
    begin
        HREmp.Reset;
        HREmp.SetRange("No.", Emp);
        if HREmp.Find('-') then begin
            // HREmp.TestField(HREmp."Date Of Join");
            rec."Responsibility Center" := HREmp."Responsibility Center";
            ...
            Error('UserID' + ' ' + '[' + HREmp."User ID" + ']' + ' has not been assigned to any employee. Please consult the HR officer for assistance')
        end;
    end;

    procedure CancelApprovalRequest(app: Code[250]; IsManager: boolean)
    var
        Workflowintegration: codeunit WorkflowIntegration;
    ...
    begin
        if IsManager = false then begin
            supApproval.Reset();
            supApproval.SetRange("Leave No.", app);
            ...
            hrleaveApp.Reset();
            hrleaveApp.SetRange("Application Code", app);
            if hrleaveApp.FindFirst() then begin
                if Workflowintegration.CheckLeaveApplicationApprovalsWorkflowEnabled(hrleaveApp) then begin
                    Workflowintegration.OnCancelLeaveApplicationApprovalRequest(hrleaveApp);
                end;
                ;
            end;
        end;
    ...
    begin
        ...
        currentYear := Date2DMY("Start Date", 3);
        HRLeavePeriods.Reset();
        HRLeavePeriods.SetRange("Period Code", Format(currentYear));
        ...
        if HRSetup.Find('-') then begin
            ...
            LeaveGjline."Leave Period" := periodCode;
            // LeaveGjline."Leave Period" := HRLeavePeriods."Period Code";
            ...
            LeaveGjline.Description := 'Leave Taken from ' + Format("Start Date") + ' to ' + Format("End Date") + '.';
