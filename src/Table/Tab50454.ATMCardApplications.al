...
Table 50454 "ATM Card Applications"
{
    ...
    fields
    {
        ...
        {
            TableRelation = Vendor."No." where("Account Type" = filter('103'));
        ...
        {
            OptionCaption = 'New,Replacement,Renewal,Pin Replacement,Card Reissue';
            OptionMembers = New,Replacement,Renewal,"Pin Replacement","Card Reissue";
            trigger OnValidate()
            begin
                IF ("Request Type" = "Request Type"::New) AND ("Previous Card No" <> '') then
                    Error('The member has an existing card no.');
                IF ("Request Type" = "Request Type"::Replacement) AND ("Previous Card No" = '') then
                    Error('The member does not have an existing card no.');
                IF ("Request Type" = "Request Type"::Renewal) AND ("Previous Card No" = '') then
                    Error('The member does not have an existing card no.');
                IF ("Request Type" = "Request Type"::"Pin Replacement") AND ("Previous Card No" = '') then
                    Error('The member does not have an existing card no.');
            end;
        }
        ...
        field(62; "Reason for Reissue"; Text[300])
        {
        }
        field(63; "Pin Replacement Issued"; Boolean)
        {
        }
        field(64; "Pin Replacement Issued On"; Date)
        {
        }
        field(65; "Pin Replacement Issued By"; Code[50])
        {
        }
        field(66; "Legacy ATMs"; Boolean)
        {
        }
        ...
        field(68036; Posted; boolean)
        {
        }
        field(68037; "Current Account Balance"; Decimal)
        {
            Editable = false;
            Caption = 'Ordinary Savings';
        }
        field(68038; "Previous Card No"; Code[60])
        {
            Editable = false;

        }
        field(68039; "Issued To ID No"; Code[60])
        {
            //  Editable = false;

        }
        field(68040; "Issued to Phone Number"; Code[60])
        {
            //  Editable = false;
            InitValue = '254';
            trigger OnValidate()
        ...
        field(68041; "Mode Of Dispatch"; Option)
        {
            OptionMembers = Posta,G4S,Fargo;
            // ;

        }
        field(68042; "Location"; Code[60])
        {
            //  Editable = false;

        }
        field(68043; "Address"; Code[60])
        {
            //  Editable = false;

        }
        field(68044; "Tracking Code"; Code[60])
        {
            //  Editable = false;
    ...
    begin
        if "No." = '' then begin
            ...
            NoSeriesMgt.GetNextNo(NoSetup."ATM Applications");
        end;
