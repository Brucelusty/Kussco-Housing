...
Table 50373 "Loan Offset Details"
{
    ...
    fields
    {
        ...
        {
            TableRelation = "Loans Register"."Loan  No." where("Client Code" = field("Client Code"),Posted = const(true),"Outstanding Balance" = filter(<> 0));
            trigger OnValidate()
            var
            ...
            begin
                if Confirm('Are you Sure you Want to Offset this loan?', true) = true then
                    ...
                    if loans.FindFirst() THEN BEGIN
                        ...
                        IF LoansTypes.Get(loans."Loan Product Type") then begin
                            "Refinancing Fee" := (LoansTypes."Refinancing %Charge > 1YR" / 100) * LOANS."Total Outstanding Balance";
                            "Interest On TopUp" := (LoansTypes."Interest On Topup" / 100) * LOANS."Total Outstanding Balance";//
                        end;
                    END;
        ...
        field(5; "Principle Top Up"; Decimal)
        {
            ...
            begin
                ...
                NewLoan.Get("Loan No.");
                if NewLoan."Loan Product Type" <> 'DEFAULT' then begin

                    //Message('Loan%1Boosting%2',NewLoan."Loan  No.",Loantypes."Loan Boosting Commission %");
                    Loantypes.Reset;
        ...
        field(6; "Interest Top Up"; Decimal)
        {
            ...
            begin
                ...
                Commision := ROUND(("Principle Top Up") * (Loantypes."Loan Boosting Commission %" / 100), 1, '>');
        ...
        field(30; "Refinancing Fee"; Decimal)
        {
        }
        field(31; "Expected Principle Amount"; Decimal)
        {
            trigger OnValidate()
            begin
                "Principle Top Up" := "Principle Top Up" - "Expected Principle Amount";
            end;
        }
        field(32; "Expected Repayment"; Decimal)
        {
        ...
        field(33; "Mark Adviced"; Boolean)
        {

        }
        field(34; "Interest On TopUp"; Decimal)
        {
        }
        //New from .71
        field(35;"Interest Charged";Decimal)
        {
        }
        field(36; "Loan Product"; Text[250])
        {}
    }
    ...
    trigger OnDelete() begin
        ...
        if Loans.Find('-') then begin
            if Loans."Loan Status" <> Loans."Loan Status"::Application then Error('An offset can only be deleted at loan registration.');
        end;
    end;
    ...
    begin
        ...
        if ObjLoans.Get("Loan Top Up") then begin
            ObjLoans.CalcFields(ObjLoans."Total Outstanding Balance");
            LoanBal := ObjLoans."Total Outstanding Balance";
        end;
        ...
        ObjLoans.SetRange(ObjLoans."Loan  No.", "Loan Top Up");
        if ObjLoans.FindSet then begin
            ObjLoans.CalcFields(ObjLoans."Outstanding Balance", ObjLoans."Total Outstanding Balance");
        ...
        if Loans.Find('-') then begin
            if CurrLoan.Get("Loan No.") then begin
                Loans.CalcFields(Loans."Outstanding Balance", Loans."Interest Due", Loans."Outstanding Interest", "Total Outstanding Balance");
                "Loan Type" := Loans."Loan Product Type";
                "Loan Product" := Loans."Loan Product Type Name";
                Loantypes.Reset;
                Loantypes.SetRange(Loantypes.Code, Loans."Loan Product Type");// "Loan Type");
                if Loantypes.Find('-') then begin
                    if CurrLoan."Loan Product Type" <> 'DEFAULT' then begin
                        Commision := (Loantypes."Loan Boosting Commission %" / 100) * (Loans."Total Outstanding Balance" - "Expected Principle Amount");
                        "Interest On TopUp" := (LoanTypes."Interest On Topup" / 100) * LOANS."Total Outstanding Balance";
                    end;
                end;
                //Message('%1-%2', Loantypes."Loan Boosting Commission %", Commision);

                if (CurrLoan."Loan Product Type" = 'TL1') or (CurrLoan."Loan Product Type" = 'SPL') then
                    Commision := (Loantypes."Topup1_Super Plus offset Comm%" / 100) * (Loans."Outstanding Balance" - "Expected Principle Amount");
            end;
            // Message('%1-%2', Loantypes."Loan Boosting Commission %", Commision);
            ...
            "Principle Top Up" := Loans."Outstanding Balance"-"Expected Repayment";
            ...
            "Total Top Up" := "Principle Top Up" + "Interest Top Up" + Commision + "Tax On Comission" + VarInsuranceAmount + "Penalty Charged"+"Interest On TopUp";
