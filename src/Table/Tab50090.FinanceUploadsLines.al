...
Table 50090 "Finance Uploads Lines"
{
    ...
    fields
    {
        ...
        field(4; "Debit Account Type"; enum "Gen. Journal Account Type")
        {
            ...
            // OptionMembers = "G/L Account",Customer,Debtor,"Bank Account","Fixed Asset","IC Partner",Employee,Member,Investor;
        }
        field(5; "Debit Account No"; Code[30])
        {
            TableRelation = if ("Debit Account Type" = filter(Debtor)) Vendor."No."
            else
        ...
        {
            TableRelation = if ("Credit Account Type" = filter(Debtor)) Vendor."No."
            else
