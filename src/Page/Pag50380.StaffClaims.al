            ...
            group("&Functions")
            {
                ...
                action("Post Payment and Print")
                {
                    ...
                    begin
                        ...
                        ObjStaffClaimHeader.SetRange(ObjStaffClaimHeader."No.", Rec."No.");
                ...
                action("Upload Document")
                {
                    ...
                    begin
 /*                        if Rec.Upload('Upload file', 'C:\', 'Text file(*.txt)|*.txt|PDF file(*.pdf)|*.pdf|ALL file(*)|*', 'Doc.txt', vartest) then begin
                            /*  Links.INIT;
                              Links."Link ID":=0;
                            //"No."  Links."Record ID":=;
                ...
                action(Download)
                {
                    ...
                    var
                        vartest: Variant;
                    begin
                        //Rec.Download('Upload file', 'C:\', 'Text file(*.txt)|*.txt|PDF file(*.pdf)|*.pdf|ALL file(*)|*', 'Doc.txt', vartest)
                    end;
                }
                action("Create Payment Voucher")
                {
                    ...
                    begin
                        ...
                        if not Confirm('Are you sure you want to create a Payment Voucher for %1', false, Rec."No.") then
                            ...
                            PVHeadEr."Payment Mode" := Rec."Pay Mode";
                            PVHeadEr."Bank Account" := Rec."Paying Bank Account";
                            ...
                            PVHeadEr."Global Dimension 2 Code" := Rec."Shortcut Dimension 2 Code";
                            ...
                            PVHeadEr."Payment Description" := CopyStr(Rec.Purpose, 1, 50);
                            PVHeadEr."No." := Rec."No.";
    ...
    begin
        ...
        GenJournalLine."account type"::Vendor, Rec."Paying Bank Account", Today, Rec."Total Net Amount" * -1, 'FOSA', '',
        Rec.Purpose + '' + Rec.Payee, Rec."External Document No", GenJournalLine."application source"::" ", Rec."Shortcut Dimension 2 Code");
        //------------------------------------1.2. Debit Control A/C---------------------------------------------------------------------------------------------
        LineNo := LineNo + 10000;
        SFactory.FnCreateGnlJournalLineBranch(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
        GenJournalLine."account type"::"Bank Account", ObjGensetup."Internal PV Control Account", Today, Rec."Total Net Amount", 'FOSA', '',
        Rec.Purpose + '' + Rec.Payee, '', GenJournalLine."application source"::" ", Rec."Shortcut Dimension 2 Code");




        ObjStaffClaimLine.Reset;
        ObjStaffClaimLine.SetRange(ObjStaffClaimLine.No, Rec."No.");
        if ObjStaffClaimLine.FindSet then begin
            repeat
                ...
                ObjStaffClaimLine.Purpose + '' + Rec.Payee, ObjStaffClaimLine."Claim Receipt No", GenJournalLine."application source"::" ", ObjStaffClaimLine."Shortcut Dimension 2 Code");
                ...
                ObjStaffClaimLine.Purpose + '' + Rec.Payee, ObjStaffClaimLine."Claim Receipt No", GenJournalLine."application source"::" ", Rec."Shortcut Dimension 2 Code");
    ...
    begin
        PostEnabled := true;
        if (Rec.Cashier = UserId) or (Rec.Status <> Rec.Status::Approved) then
            PostEnabled := false;
    end;
}
