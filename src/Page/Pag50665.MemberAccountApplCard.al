...
Page 50665 "Member Account Appl. Card"
{
    ApplicationArea = All;
    ...
    layout
    {
        area(content)
        {
            group("General Info")
            {
                ...
                field("Account Type"; Rec."Account Type")
                {
                    ...
                    begin
                        ...
                        if Rec."Account Type" = '005' then begin
                            FixedAccVisible := true;
                        end;
                        if Rec."Account Type" = '004' then begin
            ...
            group(ChildrenAcc)
            {
                Caption = 'junior Savers Account Details';
        ...
        area(factboxes)
        {
            part(Control10000025; "Vendor Child Picture-App")
            {
                Caption = 'Child Picture';
                // Visible = ChildrenAccVisible;
                Enabled = ChildrenAccVisible;
                        ...
                        if AccoutTypes.Get(Rec."Account Type") then begin
                            ...
                            if ObjAccountTypes.Find('-') then begin
                                ...
                                if Rec."Account Type" = '109' then begin
                                    VarAcctNo := ObjAccountTypes.Branch + '-' + Rec."BOSA Account No" + '-' + Format(9000 + VarNoofMemberAccounts);
                                end;
                            ...
                            if Vendor.Get(AcctNo) then Error('The Account No. %1 already exists.', AcctNo);
                ...
                action(CreateII)
                {
                    ...
                    begin
                        ...
                        if Confirm('Are you sure you want to create this account?', true) = false then
                            exit;
                        SaccoNoSeries.Get();
                        if Rec."Account Type" = '0041' then begin
                            SaccoNoSeries.TestField("Junior Account Nos");
                            VarAcctNo := NoSeries.GetNextNo(SaccoNoSeries."Junior Account Nos")
                        end else
                            if Rec."Account Type" = '002' then begin
                                SaccoNoSeries.TestField(SaccoNoSeries."Debenture Nos");
                                VarAcctNo := NoSeries.GetNextNo(SaccoNoSeries."Debenture Nos");
                            end;
                        ...
                        ObjAccounts."Child Picture" := Rec."Child Photo";
                        ObjAccounts."Child Name" := rec."Name of the Child";
                        ObjAccounts."Child Birth certificate" := Rec."Birth Certificate-Child";
                        ObjAccounts."Child DOB" := rec."Date of Birth-Child";
                        //    end;
                        ...
                        if Rec."Account Category" = Rec."account category"::Corporate then begin
                            ObjAccounts."Account Category" := ObjAccounts."account category"::Joint;
                        ...
                        Message('You have successfully created a Junior Savings, A/C No=%2', Rec."Account Type", VarAcctNo);
                        ...
                        if Confirm('Do you want to notify this member through sms?') then begin
                            // SFactory.FnSendSMS('ACCOUNTAPP', 'Dear ' + Rec.Name + 'Your ' + Rec."Account Type Name" + 'has been created. Your account number is ' +
                            // VarAcctNo, Rec."BOSA Account No", Rec."Phone No.");

                            if Cust.Get(rec."BOSA Account No") then begin
                                if rec."Account Type" = '111' then begin
                                    fosaMsg := 'Dear ' + Rec.Name + '. Your ' + rec."Account Type Name" + ' has been created successfully, account no.: ' + VarAcctNo + '. Your allocated business store number is ' + storeNo + '.';
                                    smsManagement.SendSmsWithID(Source::NEW_ACCOUNT, cust."Mobile Phone No", fosaMsg, rec."BOSA Account No", cust."FOSA Account No.", true, 240, true, 'CBS', CreateGuid(), 'CBS');
                                end else begin
                                    fosaMsg := 'Dear ' + Rec.Name + '. Your ' + rec."Account Type Name" + ' has been created successfully, account no.: ' + VarAcctNo + '.';
                                    smsManagement.SendSmsWithID(Source::NEW_ACCOUNT, cust."Mobile Phone No", fosaMsg, rec."BOSA Account No", cust."FOSA Account No.", true, 240, true, 'CBS', CreateGuid(), 'CBS');
                                end;
                            end;
                        end;
                ...
                action("Send Approval Request")
                {
                    ...
                    begin
                        if Confirm('Are you sure you want to send Approval request for this record?', true) = false then
                            exit;
                        if Rec."Account Type" = '004' then begin
                            Rec.TestField("Birth Certificate-Child");
                            Rec.TestField("Name of the Child");
                            Rec.TestField("Date of Birth-Child");
                        end;
