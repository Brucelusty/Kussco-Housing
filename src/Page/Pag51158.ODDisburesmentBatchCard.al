...
page 51158 "OD Disburesment Batch Card"
{
    ApplicationArea = All;
    DeleteAllowed = false;
    PageType = Card;
    ...
    SourceTableView = where(Posted = filter(false), "Disbursement Type" = const(Normal), Source = filter(FOSA));
            ...
            group(LoansB)
            {
                ...
                action(Post)
                {
                    ...
                    begin
                        ...
                        //     Error('The user who prepared this batch should be the one posting it.');
                        // end;
                        ...
                        //Try implementing a try function for the cases of errors that lead to double posting
                        GenJournalLine.Reset;
                        GenJournalLine.SetRange("Journal Template Name", 'PAYMENTS');
                        GenJournalLine.SetRange("Journal Batch Name", 'LOANS');
                        if GenJournalLine.Find('-') then begin
                        ...
                        if LoanApps.Find('-') then begin
                            repeat
                                if LoanApps."Overdrawn Reconciliation" = false then begin
                                    Members.Reset;
    ...
    SFactorys: Codeunit "SURESTEP FactoryMobile";
        ...
        if rec."Mode Of Disbursement" = rec."mode of disbursement"::"Transfer to FOSA" then begin
            ...
            if LoanApps.Find('-') then begin
                repeat
                    ...
                    if LoanApps."Loan Product Type" <> 'M_OD' then
                        Error('This facility is restricted to Overdraft Loans.');
        ...
        IF LoanApps.GET(VarLoanNo) THEN BEGIN
            ...
            if LoanApps."Overdrawn Reconciliation" = false then begin
                ...
                IF LoanApps."Loan Offset Amount" > 0 THEN BEGIN
                    ...
                    END;
                END;
                LoanType.Get(LoanApps."Loan Product Type");
                //-----------------------------------------Accrue Interest Disburesment--------------------------------------------------------------------------------------------
                LineNo := LineNo + 10000;
                SFactorys.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::"G/L Account", LoanType."Loan Interest Account", Today, (LoanType."Interest rate" * LoanApps."Approved Amount" / 100) * -1, '', DOCUMENT_NO,
                'Overdraft Loan interest Upfront' + ' ' + LoanApps."Client Code", '');

                LineNo := LineNo + 10000;
                SFactorys.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::"Interest Due",
                GenJournalLine."Account Type"::Customer, LoanApps."Client Code", Today, (LoanType."Interest rate" * LoanApps."Approved Amount" / 100), '', DOCUMENT_NO,
                'Overdraft Loan ' + ' ' + LoanApps."Client Code", LoanApps."Loan  No.");


                LineNo := LineNo + 10000;
                SFactorys.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::Vendor, LoanApps."Account No", Today, (LoanType."Interest rate" * LoanApps."Approved Amount" / 100), '', DOCUMENT_NO,
                'Overdraft Loan interest Repayment' + ' ' + LoanApps."Client Code", '');


                LineNo := LineNo + 10000;
                SFactorys.FnCreateGnlJournalLinePaybill(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::"Interest Paid",
                GenJournalLine."Account Type"::Customer, LoanApps."Client Code", Today, (LoanType."Interest rate" * LoanApps."Approved Amount" / 100) * -1, '', DOCUMENT_NO,
                'Overdraft Loan interest Repayment' + ' ' + LoanApps."Client Code", LoanApps."Loan  No.");
                ...
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::Loan,
                GenJournalLine."Account Type"::Customer, LoanApps."Client Code", Rec."Posting Date", VarAmounttoDisburse, FORMAT(LoanApps.Source), LoanApps."Loan  No.",
                Rec."Description/Remarks"+' - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                //--------------------------------(Debit Member Loan Account)---------------------------------------------

                //------------------------------------2. CREDIT MEMBER FOSA A/C---------------------------------------------------------------------------------------------
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::Vendor, LoanApps."Account No", Rec."Posting Date", VarAmounttoDisburse * -1, 'FOSA', LoanApps."Loan  No.",
                Rec."Description/Remarks"+' - ' + LoanApps."Loan Product Type Name" + ' - ' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                //----------------------------------(Credit Member Fosa Account)------------------------------------------------
            end;
