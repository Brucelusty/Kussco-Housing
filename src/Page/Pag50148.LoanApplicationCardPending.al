...
Page 50148 "Loan Application Card(Pending)"
{
    ApplicationArea = All;
    DeleteAllowed = false;
    Editable = true;
 ...
 Visible=false;
            ...
            group("Salary Details.")
            {
                ...
                group("Details")
                {
                    ...
                    field("SaccoDeductions";Rec."Sacco Deductions")
                    {
                        Caption = 'STO Deductions';
                    }
                    field("Member Loans Deductions";Rec."Member Loans Deductions")
                    {
                        Caption = 'Loan Deductions';
                    }
                    field("Total DeductionsH";Rec."Total DeductionsH")
                    {
                    ...
                    field("OtherIncome";Rec."Other Income")
                    {
                    }
            ...
            {
                Caption = 'Loan Collateral Details';
                ...
                action("Loan Application Form")
                {
                    ...
                    begin
                        ...
                        if LoanApp.Find('-') then begin
                            Report.run(172896, true, false, LoanApp);
                        end;
                    end;
                }
...
/*                         ApprovalEntries.Reset();
                        ApprovalEntries.SetRange("Document No.", Rec."Loan  No.");
 ...
 */
                        ...
                        LoanApp.SetRange(LoanApp."Loan  No.", Rec."Loan  No.");
                        if LoanApp.Find('-') then begin
                            LoanApp."Re-Opened By" := UserId;
                            LoanApp."Re-Opened On" := Today;
                            LoanApp."Re-Opened On Time" := Time;
                ...
                action("Return to Appraisal")
                {
                    ...
                    PromotedCategory = Process;
                    PromotedOnly = true;
                    Enabled = Rec."Loan Status" <> Rec."Loan Status"::Appraisal;
                    // Visible = false;

                    trigger OnAction()
                    begin
                        if Confirm('Confirm sending this Loan back to appraisal?', true, false) = true then begin
...
/*                             ApprovalEntries.Reset();
                            ...
                            rec."Re-Appraised By" := UserId;
                            rec."Re-Appraised On" := Today;
                            rec."Re-Appraised On Time" := Time;
                ...
                action("Loan Repayment Schedule")
                {
                    ...
                    trigger OnAction()
                    begin
                        if Rec."Loan Disbursement Date" = 0D then begin
            ...
            group(Approvals)
            {
                ...
                action("Send Approval Request")
                {
                    ...
                    begin
                        ...
                        if Rec."Approved Amount" = 0 then Error('Loan Must Be Upraised before sending Approval');
    ...
    begin
        UpdateControl();
        FnVisibility();
        docApprover := FnAllowApproverEdit();
    ...
    begin
        ...
        if Rec."Income Type" = Rec."income type"::Payslip then
            PayslipDetailsVisible := true;

        if Rec."Income Type" = Rec."income type"::"Bank Statement" then
            BankStatementDetailsVisible := true;

        if Rec."Income Type" = Rec."income type"::Business then
            BankStatementDetailsVisible := true;
    ...
    local procedure FnAllowApproverEdit() approver:Boolean
    var
        approvals: Record "Approval Entry";
    begin
        approvals.Reset();
        approvals.SetRange("Document No.", rec."Loan  No.");
        if approvals.Find('-') then begin
            if approvals."Approver ID" = UserId then begin
                approver := true;
            end else approver := false;
        end;
    end;
