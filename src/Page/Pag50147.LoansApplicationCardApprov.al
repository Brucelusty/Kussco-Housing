...
Page 50147 "Loans Application Card(Approv)"
{
    ApplicationArea = All;
    DelayedInsert = false;
    DeleteAllowed = false;
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Member Credit Score"; Rec."Member Credit Score")
                {
                    Caption = 'Credit Score';
                    Editable = fieldsEditable;
                    Visible = false;
                }
                field("1 3rd of Basic"; Rec."1 3rd of Basic")
                {
            ...
            group("Disbursement Bank Details")
            ...
            group("Rejection Details")
            {
                ...
                Visible = false;
                field("Re-Appraissing Remark"; Rec."Re-Appraissing Remark")
                {
                }
            ...
            {
                Caption = 'Loans Collateral Details';
                SubPageLink = "Loan No" = field("Loan  No.");
            }
        }
        area(factboxes)
        ...
        area(navigation)
        {
            group(Loan)
            {
                ...
                action("Return to Appraisal")
                {
                    ...
                    trigger OnAction()
                    begin
                        if Rec."Re-Appraissing Remark" = '' then Error('Kindly ensure the reason for reappraising this loan is highlighted.');

                        if Confirm('Confirm Re-Open Loan?', true, false) = true then begin
                ...
                action("Disburse Loan")
                {
                    ...
                    begin
                        ...
                        if ObjCollateral.FindSet = true then begin
                            if ObjCollateral."Collateral Register Doc" = '' then begin
                          ...
                          Format(rec."Repayment Start Date", 0, '<Day,2> <Month Text,3> <Year4>') + '\\Proceed?', false) = true then begin
                            ...
                            if ObjLoans.FindSet then begin
                                ...
                                (ObjLoans."Mode of Disbursement" = ObjLoans."mode of disbursement"::EFT) then begin
                                    ...
                                    if ObjLoansII.FindFirst() then begin
                                        ...
                                        if (Rec."Disburesment Type" = Rec."disburesment type"::" ") or (Rec."Disburesment Type" = Rec."disburesment type"::"Full/Single disbursement") then begin
                                            ...
                                            ObjLoansII."Tranch Amount Disbursed" := Rec."Amount to Disburse on Tranch 1";
    ...
    begin
        ...
        if LoanApps.Get(Rec."Loan  No.") then begin
            ...
            if (Rec."Disburesment Type" = Rec."disburesment type"::"Full/Single disbursement") or (Rec."Disburesment Type" = Rec."disburesment type"::" ") then begin
                ...
                VarAmounttoDisburse := Rec."Amount to Disburse on Tranch 1";
    ...
    begin
        ...
        if LoanApps.Get(Rec."Loan  No.") then begin
            ...
            if (Rec."Disburesment Type" = Rec."disburesment type"::"Full/Single disbursement") or (Rec."Disburesment Type" = Rec."disburesment type"::" ") then begin
                ...
                TrancheSchedule.Posted:=true;
                TrancheSchedule."Posted By":=UserId;
                TrancheSchedule."Posting Date":=Today;
                TrancheSchedule.modify;
                end;
                if VarAmounttoDisburse <= 0 then
                    Error('You have specified Disbursement Mode as Tranche/Multiple Disbursement, Amount to Disburse on Tranche 1 must be greater than zero.')
            end;
            ...
            if LoanType.Get(LoanApps."Loan Product Type") then begin
                if LoanType."Accrue Total Insurance&Interes" = true then begin
                    ...
                    if PCharges.Find('-') then begin
                        ...
                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Loan Insurance Charged",
                        GenJournalLine."account type"::Customer, LoanApps."Client Code", Rec."Posting Date", PChargeAmount * LoanType."No of Installment", '', BLoan,
                        'Insurance Charged' + '_' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."application source"::" ");
            ...
            if LoanApps."Share Boosting Comission" > 0 then begin
                ...
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::"G/L Account", GenSetUp."Boosting Fees Account", Rec."Posting Date", LoanApps."Share Boosting Comission" * -1, '', BLoan,
                'Deposits Boosting Fee', BLoan, GenJournalLine."application source"::" ");
            ...
            LineNo := LineNo + 10000;
            SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
            GenJournalLine."account type"::"Bank Account", Rec."Paying Bank Account No", Rec."Posting Date", VarAmounttoDisburse * -1, '', LoanApps."Cheque No.",
            'Loan Disbursement - ' + LoanApps."Loan Product Type Name" + ' - ' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."application source"::" ");
