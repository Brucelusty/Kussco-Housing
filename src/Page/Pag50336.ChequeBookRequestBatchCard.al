...
Page 50336 "Cheque Book Request Batch Card"
{
    ApplicationArea = All;
    PageType = Card;
        ...
        area(creation)
        {
            action(OrderChequeBook)
            {
                ...
                begin
                    ...
                    ObjChqBookRequestLines.SetRange(ObjChqBookRequestLines."Batch No.", Rec."Batch No.");
                    ...
                    if ObjChqBookRequestLines.FindSet then begin
                      //  CardTxtFile.Create(ObjGensetup."Cheque Book Request Path" + 'Cheque Book Application ' + Format(WorkDate, 0, '<Day,2> <Month Text,3> <Year4>') + '.txt');
                        ...
                        repeat
                            TxtStream.WriteText('AU SACCO-' + ObjChqBookRequestLines."Account Name" + ',' + ObjChqBookRequestLines."Cheque Book No");
                            TxtStream.WriteText();
    ...
    begin
        EnabledAction := false;
        if Rec.Requested = false then begin
            EnabledAction := true;
        end;
    end;
