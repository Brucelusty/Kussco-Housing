...
Page 50679 "Teller & Treasury Trans Card"
{
    ApplicationArea = All;
    DeleteAllowed = false;
    PageType = Card;
            ...
            action("Issue/Return")
            {
                ...
                begin
                    ...
                    (Rec."Transaction Type" = Rec."transaction type"::"Treasury to Intra-Day") then begin
                        if Rec.Issued = Rec.Issued::Yes then Error('The money has already been issued.');

                        if Rec."Is Petty Cash" = false then begin
                            TellerTill.Reset;
                            TellerTill.SetRange(TellerTill."No.", Rec."From Account");
                            if TellerTill.Find('-') then begin
                                if UpperCase(UserId) <> TellerTill.CashierID then Error('You do not have permission to transact on this teller till/Account.');
                            end;
