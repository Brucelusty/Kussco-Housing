...
Page 50879 "Posted Cashier Transactions V1"
{
    ApplicationArea = All;
    DeleteAllowed = false;
    Editable = false;
                    ...
                    begin
                        ...
                        if Trans.Find('-') then begin
                            if Rec."Type _Transactions" = Rec."type _transactions"::"Cash Deposit" then
                                Report.run(173038, true, true, Trans);

                            if Rec."Type _Transactions" = Rec."type _transactions"::Withdrawal then
                                Report.run(173039, true, true, Trans);
                            
                            if Rec."Type _Transactions" = Rec."type _transactions"::"MPESA Withdrawal" then
                                Report.run(175071, true, true, Trans);

                            if Rec."Type _Transactions" = Rec."type _transactions"::"Cheque Deposit" then
                                Report.run(173037, true, true, Trans);
    ...
    begin
        ...
        if ObjTellerTill.Find('-') then begin
            ...
            if (TransactionTypes.Type = TransactionTypes.Type::"Cash Withdrawal") or (TransactionTypes.Type = TransactionTypes.Type::Encashment) then begin
    ...
    begin
        ...
        ObjReceiptAllocations.SetFilter(ObjReceiptAllocations."Transaction Type", '<>%1', ObjReceiptAllocations."transaction type"::" ");
        ...
        if ObjReceiptAllocations.Find('-') then begin
            repeat
                ...
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLineBalanced(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, VarReceivableTransactionType, GenJournalLine."account type"::Customer, ObjReceiptAllocations."Member No",
