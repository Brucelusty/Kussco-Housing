...
Page 50193 "Cash Payment Header"
{
    ApplicationArea = All;
    ...
    layout
    {
        area(content)
        {
            group(Control1)
            {
                ...
                field("Staff Payment"; Rec."Staff Payment")
                {
                    //Editable = false;
                }
        ...
        area(processing)
        {
            group("&Functions")
            {
                ...
                action("Post Payment")
                {
                    ...
                    trigger OnAction()
                    begin
                        if rec.Status = rec.Status::Posted then Error('This Payment has already been posted');

                        //Post PV Entries
                        if BankAcc.Get(Rec."Bank Account") then begin
                            if UserId <> BankAcc.CashierID then Error('Only the teller can post this petty cash.');

                            CheckPVRequiredItems(Rec);
                            PostPV(Rec);

                            // Report.Run(172002, false, true, Rec);
                            BankAcc.CalcFields(Balance);
                            if (BankAcc.Balance - Rec.Amount) <= BankAcc."Minimum Teller Withholding" then Message('Kindly start preparing a reimbursement of this petty cash till');
                        end;
                    end;
                }
                action("Send A&pproval Request")
                {
                    ...
                    begin
                        ...
                        rec.TestField("Payment Purpose");
                        rec.TestField(Payee);
                        // rec.TestField("Responsibility Center");

                        if rec.Amount <= 0 then Error('The amount should not be equal to or less than zero');

                        if BankAcc.Get(Rec."Bank Account") then begin
                            BankAcc.CalcFields(Balance);
                            if Rec.Amount > BankAcc."Max Withdrawal Limit" then Error('The maximum petty cash payment amount is Ksh. ' + Format(BankAcc."Max Withdrawal Limit") + '.');

                            if (BankAcc.Balance - Rec.Amount) <= BankAcc."Minimum Teller Withholding" then begin
                                if rec."User ID" = rec.Cashier then Message('Kindly start preparing a reimbursement of the petty cash till');

                                departments.Reset();
                                departments.SetRange("Is Finance", true);
                                if departments.FindSet() then begin
                                    repeat
                                        employees.Reset();
                                        employees.SetRange("Responsibility Center", departments.Code);
                                        if employees.FindSet() then begin
                                            repeat
                                                if employees.Supervisor then begin
                                                    Recipients.Add(employees."Company E-Mail");
                                                end else
                                                    CCList.Add(employees."Company E-Mail");
                                            until employees.Next() = 0;
                                        end;
                                    until departments.Next() = 0;
                                end;

                                EmailSubject := 'REQUEST FOR PETTY CASH REIMBURSEMENT';
                                EmailBody := 'Dear <strong>FINANCE</strong>, <br><br>Kindly not that the petty cash level for till <strong>%1</strong> is below the threshold. Thus, commence the process of petty cash reimbursment.';
                                EmailBody += '<br><br><hr> <strong> Be advised, this is a system generated email.</strong> </hr>';
                                FinalEmailBody := StrSubstNo(EmailBody, BankAcc."Name");

                                EmailMessage.Create(Recipients, EmailSubject, FinalEmailBody, true, CCList, BccList);
                                EmailSend.Send(EmailMessage, Enum::"Email Scenario"::"Purchase Order");
                            end;
                        end;
                ...
                action(Approvals)
                {
                    ...
                    begin
                        ApprovalEntries.SetRecordFilters(Database::"Payment Header.", approvalDoc::PettyCash, rec."No.");
                        ApprovalEntries.Run;
                    end;
                }
            }
            ...
            action("View Schedule")
            {
                ...
                begin
                    ...
                    if Payments.Find('-') then begin
                        Report.run(172002, true, true, Payments);
                    end;
                end;
            }
        }
    ...
    begin
        ...
        if (rec.Posted = false) then begin
            if (Rec.Cashier = UserId) or (Rec.Status = Rec.Status::Approved) then begin
                EnablePost := true;
            end;
        end;
        if rec."Staff Payment" then staff := true;
    end;
    ...
    begin
        ...
        EnablePost := false;
        if (rec.Posted = false) and (Rec.Status = Rec.Status::Approved) then EnablePost := true;
    ...
    begin
        ...
        GenJnlLine.Description := rec.Payee + Payments."Manual No";    //COPYSTR('Pay To:' + Payments.Payee,1,50);    //COPYSTR("Payment Description",1,50);
    ...
    var
        ...
        pettyCash: Record "Petty Cash Transaction Types";
        // pettyCash: Record "Receipts and Payment Types";
        ...
        if PayLine.Find('-') then begin
            repeat
                ...
                if PayLine."Pay Mode" = PayLine."pay mode"::Cash then begin
                    CashierLinks.Reset;
                    CashierLinks.SetRange(CashierLinks.UserIDS, UserId);
                end;
            ...
            if StaffClaim.Get(rec."No.") then begin
            ...
            if AdvanceHeader.Get(rec."No.") then begin
            ...
            if PayReqHeader.Get(rec."No.") then begin
                PayReqHeader."Date Posted" := Today;
                PayReqHeader."Time Posted" := Time;
                PayReqHeader."Posted By" := UserId;
                PayReqHeader.Status := PayReqHeader.Status::Posted;
                PayReqHeader.Posted := true;
                PayReqHeader.Modify;
            end;
        end else begin
            if Confirm('Do you wish to proceed with posting this petty cash?', true) = false then exit;
            ...
            if not genBatches.Find('-') then begin
                ...
                genBatches.Description := 'Petty Cash Payments Journal';
                genBatches.Insert();
            end;

            pettyCash.Reset();
            if pettyCash.Get(Payment."Payment Purpose") then begin
                destAcc := pettyCash."Destination Account";
            end;

            // pettyCash.Reset();
            // if pettyCash.Get(Payment."Payment Purpose") then begin
            //     destAcc := pettyCash."G/L Account";
            // end;
            ...
            Payment."Bank Account", Payment."Payment Release Date", (Payment.Amount) * -1, 'FOSA', 'Petty Cash to: ' + Payment.Payee + '' + Payment."Payment Description", '');
            ...
            destAcc, Payment."Payment Release Date", (Payment.Amount), 'FOSA', 'Petty Cash to: ' + Payment.Payee + '' + Payment."Payment Description", '');
