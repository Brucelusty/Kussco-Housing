...
page 50190 "Payment Voucher"
{
    ...
    DeleteAllowed = false;
    PageType = Card;
    PromotedActionCategories = 'New,Process,Report,Approve,Release,Posting,Prepare,Order,Request Approval,Print/Send,Navigate,Attachments';
    //PromotedActionCategories = 'New,Process,Reports,ApprovalRequest,Budgetary Control,Cancellation,Category7_caption,Category8_caption,Category9_caption,Category10_caption';
        ...
        area(processing)
        {
            group("&Functions")
            {
                ...
                action(Post)
                {
                    ...
                    trigger OnAction()
                    begin
                        if Rec.Posted = true then Error('This PV has already been posted.');
                        IF UserSetup.GET(USERID) THEN BEGIN
                            IF NOT UserSetup."Post Pv" THEN
                                Error(Text004)
                            ELSE
                                IF (rec."Pay Mode" = rec."Pay Mode"::Cheque) or (rec."Pay Mode" = rec."Pay Mode"::EFT) or (rec."Pay Mode" = rec."Pay Mode"::RTGS) THEN begin
                            ...
                            if Confirm('This PV is set to be posted on ' + Format(Rec."Payment Release Date") + '. Do you wish to continue with posting this payment voucher?', true) = false then exit;
                            //post pv
                ...
                action("Send Approval Request")
                {
                    ...
                    begin
                        ...
                        */
                        ...
                        // IF (rec."Pay Mode" = rec."Pay Mode"::Cheque) or (rec."Pay Mode" = rec."Pay Mode"::EFT) THEN begin
                        //     Rec.TESTFIELD("Cheque No.");
                        // end;
                        rec.TESTFIELD(Payee);
                ...
                action(Approval)
                {
                    ...
                    begin
                        ...
                        ApprovalEntries.SetRecordFilters(DATABASE::"Payments Header", approvalDoc::"Payment Voucher", Rec."No.");
                        ApprovalEntries.RUN;
                    end;
                }
    ...
    begin
        ...
        rec."Payment Type" := rec."Payment Type"::Bank;
    ...
    begin
        ...
        // rec."Shortcut Dimension 2 Code" := 'FINANCE';
        //rec."Global Dimension 1 Code" := 'HQ';
    ...
    begin
        ...
        END;
        IF rec."Paying Type" = rec."Paying Type"::GLAccount THEN BEGIN
            GenJnlLine."Account Type" := GenJnlLine."Account Type"::"G/L Account";
            GenJnlLine."Account No." := Payments."Paying Bank Account";
        END;
        IF rec."Paying Type" = rec."Paying Type"::Customer THEN BEGIN
            GenJnlLine."Account Type" := GenJnlLine."Account Type"::Customer;
            GenJnlLine."Account No." := Payments."Paying Bank Account";
        END;
        IF rec."Paying Type" = rec."Paying Type"::FOSAAccount THEN BEGIN
        ...
        GenJnlLine.Description := COPYSTR('Pay To:' + Payments.Payee, 1, 100);
        GenJnlLine.VALIDATE(GenJnlLine.Description);
        //Message('message bank%1', GenJnlLine.Amount);
    ...
    begin
        ...
        JBatch := 'PAYMENTS';//Temp."Payment Journal Batch";
    ...
    begin
        ...
        IF PayLine.FIND('-') THEN BEGIN
            ...
            REPEAT
                ...
                GenJnlLine.VALIDATE(GenJnlLine."Account No.");
                GenJnlLine."External Document No." := Payments."Cheque No.";
                GenJnlLine.Description := COPYSTR(PayLine.Remarks, 1, 150);
                GenJnlLine."Currency Code" := Payments."Currency Code";
                GenJnlLine.VALIDATE("Currency Code");
                GenJnlLine."Currency Factor" := Payments."Currency Factor";
                GenJnlLine.VALIDATE("Currency Factor");
                if Payment."Paying Type" = Payment."Paying Type"::Bank then begin
                ...
                //Message('gl%1-name %2', GenJnlLine.Amount, GenJnlLine."Account No.");
                GenJnlLine.VALIDATE(GenJnlLine.Amount);
    ...
    begin
        ...
        if rec.Status = rec.Status::Pending then begin
            ...
            canCreate := true;
            postDateEdit := true;
        end else if (rec.Status = rec.Status::Posted) and (Rec.Posted = true) then begin
            canSendApproval := false;
            canCreate := false;
            postDateEdit := false;
        end
