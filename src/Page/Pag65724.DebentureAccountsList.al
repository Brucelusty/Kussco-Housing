...
Page 65724 "Debenture Accounts List"
{
    ApplicationArea = All;
    ...
    UsageCategory = Lists;
    SourceTable = Vendor;
    SourceTableView = where("Creditor Type" = filter("FOSA Account"),"Account Type Name"=filter('Debenture Account'));
    ...
    begin
        ...
        userSetup.SetRange("Is Manager", true);
        if UserSetup.Find('-') then begin
            staffView := true;
        end else Error('The user: %1, does not have permissions to view all member savings accounts.', UserId);
    end;
    ...
    begin
        MinimumBalance := 1000;
        if (Rec.Balance = 0) or (Rec."Balance Filterable" = 0) then
            CoveragePercentStyle := 'Strong'
        else
            if (Rec.Balance < MinimumBalance) or (Rec."Balance Filterable" < MinimumBalance) then
