...
Page 51113 "Credit Committee List"
{
    ApplicationArea = All;
    CardPageID = "Loan Committee Card";
    DeleteAllowed = false;
    Editable = false;
    InsertAllowed = false;
    PageType = List;
    ...
    SourceTableView = where(Posted = filter(false),
                            "Approval Status" = filter(Open), "Loan Status" = filter("Credit Committee"), "Archive Loan" = filter(false));
    ...
    layout
    {
        area(content)
        {
            repeater(Control1000000000)
            {
                ...
                field("Account No"; Rec."Account No") { Caption = 'Member Number'; ShowMandatory = true; }
