...
Page 50116 "BOSA Receipt Card"
{
    ApplicationArea = All;
    DeleteAllowed = false;
    Caption = 'Member Receipt Card';
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            ...
            {
                SubPageLink = "Document No" = field("Transaction No."),"MemberNo"=field("Account No.");
            }
        }
    }
        ...
        area(navigation)
        {
            action(SendApproval)
            {
                ...
                begin
                    ...
                    If rec."Approval Status" <> Rec."Approval Status"::Open then Error('The record should be open to send an approval request.');

                    if workflowInt.CheckBOSAReceiptApprovalsWorkflowEnabled(Rec) then workflowInt.OnSendBOSAReceiptForApproval(Rec);
                end;
            }
            action(CancelApproval)
            {
                ...
                trigger OnAction()
                begin
                    If rec."Approval Status" <> Rec."Approval Status"::"Pending Approval" then Error('This record is not pending any approval.');

                    if workflowInt.CheckBOSAReceiptApprovalsWorkflowEnabled(Rec) then workflowInt.OnCancelBOSAReceiptApprovalRequest(Rec);
                end;
            }
            action(Approvals)
            {
                ...
                trigger OnAction()
                begin
                    approvalEntries.SetRecordFilters(Database::"Receipts & Payments", approvalDoc::BOSAReceipt, Rec."Transaction No.");
                    approvalEntries.Run;
                end;
            }
            group(Suggest)
        ...
        area(processing)
        {
            action(Post)
            {
                ...
                begin
                    ...
                    if rec.Posted then Error('This receipt is already posted');
                    ...
                    if (rec."Account Type" = rec."account type"::Customer) or (rec."Account Type" = rec."account type"::Member) then begin
                        ...
                        if ReceiptAllocations.Find('-') then begin
                            repeat
                                ...
                                GenJournalLine."External Document No." := rec."Cheque No.";
                                GenJournalLine."Posting Date" := rec."Cheque Date";
                                if ReceiptAllocations."STO Account Type" = ReceiptAllocations."STO Account Type"::"FOSA Account" then begin
                                ...
                                GenJournalLine.Validate(GenJournalLine.Amount);
                                GenJournalLine.Description := CopyStr(rec.Name +  '-' + rec."Cheque No."
                                , 1, 50);
                                ...
                                if ReceiptAllocations."Transaction Type" = ReceiptAllocations."Transaction Type"::Repayment then begin
                                    LoansR.Reset();
                                    LoansR.SetRange(LoansR."Loan  No.", ReceiptAllocations."Loan No.");
                                    LoansR.SetAutoCalcFields(LoansR."Outstanding Balance");
                                    if LoansR.FindFirst() then begin
                                        if ReceiptAllocations.Amount = LoansR."Outstanding Balance" then begin
                                            LoanTypes.Get(LoansR."Loan Product Type");
                                            DataSheet.Init;
                                            DataSheet."PF/Staff No" := LoansR."Staff No";
                                            DataSheet."Type of Deduction" := LoanTypes."Product Description";
                                            DataSheet."Remark/LoanNO" := LoansR."Loan  No.";
                                            DataSheet.Name := LoansR."Client Name";
                                            DataSheet."ID NO." := LoansR."ID NO";
                                            DataSheet."Amount ON" := 0;
                                            DataSheet."Amount OFF" := LoansR."Outstanding Balance";
                                            DataSheet."REF." := '2026';
                                            DataSheet."New Balance" := 0;
                                            DataSheet.Date := Loans."Issued Date";
                                            DataSheet.Employer := '';
                                            DataSheet."Repayment Method" := LoansR."Repayment Method";
                                ...
                                if ReceiptAllocations."Transaction Type" = ReceiptAllocations."Transaction Type"::Repayment then begin
                                    ...
                                    if LoansR.FindFirst() then begin
                                        if ReceiptAllocations.Amount < LoansR."Outstanding Balance" then begin
                                            ...
                                            DataSheet."Amount OFF" := LoansR."Outstanding Balance";
                                            DataSheet."REF." := '2026';
                                            DataSheet."New Balance" := LoansR."Outstanding Balance" - ReceiptAllocations.Amount;
                                            DataSheet.Date := Loans."Issued Date";
                                            DataSheet.Employer := '';
                                            DataSheet."Repayment Method" := LoansR."Repayment Method";
                                ...
                                if ReceiptAllocations."Transaction Type" = ReceiptAllocations."Transaction Type"::Repayment then begin
                                    ...
                                    LoansR.SetAutoCalcFields(LoansR."Outstanding Balance");
                                    if LoansR.FindFirst() then begin
                                        if ReceiptAllocations.Amount > LoansR."Outstanding Balance" then
                                            Error('The amount being repaid is greater than the loan balance in loan no %1', LoansR."Loan  No.");
                                    end;
                                end;
                    ...
                    Audit.FnInsertAuditRecords(UserId, 'BOSA Receipt', Rec.Amount, Rec."Transaction No.", today, time, '', Rec."Transaction No.", Rec."Account No.", '');
