...
Page 50112 "Loan Application BOSA(New)"
{
    ApplicationArea = All;
    ...
    SourceTableView = where(Posted = filter(false)
                            , "Approval Status" = filter(Open), "Loan Status" = filter(Application), "Archive Loan" = filter(false));
    ...
    layout
    {
        area(content)
        {
            repeater(Control1000000000)
            {
                ...
                field("Account No"; Rec."Account No") { Caption = 'FOSA Account'; ShowMandatory = true;Visible = false; }
