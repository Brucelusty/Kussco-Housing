...
Page 50834 "Teller & Treasury Trans Card P"
{
    ApplicationArea = All;
    DeleteAllowed = false;
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            ...
            part(Control1102755000; "Treasury CoinageP")
            {
    ...
    actions
    {
        area(processing)
        {
            action(CustodianConfirmIssue)
            {
                ...
                begin
                    ...
                    if ObjCustodians.Find('-') = true then begin
                        if (Rec."Custodian 1 Name_Issue" = '') and (Rec."Custodian 2 Name_Issue" <> UserId) then begin
                            Rec."Custodian 1 Confirm Issue" := true;
                            Rec."Custodian 1 Name_Issue" := UserId
                        end else
            ...
            action("Mark Transaction")
            {
                Caption = 'Mark Transaction';
                Image = Interaction;
                Promoted = true;
                PromotedCategory = Process;
                ...
                trigger OnAction()
                begin
                    Rec."Ignore Trans" := true;
                    Rec.Modify();
                end;
            }

            action("UnMark Transaction")
            {
                Caption = 'UnMark Transaction';
                Image = Interaction;
                Promoted = true;
                PromotedCategory = Process;
            ...
            action(CustodianConfirmReceipt)
            {
                ...
                Promoted = true;
                PromotedCategory = Process;
                //Visible = VarTreasuryIntrDayReceiptVisible;//Kitui
                Visible = false;
                trigger OnAction()
                begin
                    ObjCustodians.Reset;
            ...
            action("Issue/Return")
            {
                ...
                begin
                    ...
                    if Rec."Total Cash on Treasury Coinage" <> Rec.Amount then
                        Error('Amount to issue should be equal to the total coinage amount.');
                    if (Rec."Transaction Type" = Rec."transaction type"::"End of Day Return to Treasury") then
                        Rec.TestField("From Account User", UserId);
                    ...
                    if (Rec."Transaction Type" = Rec."transaction type"::"Issue To Teller") or (Rec."Transaction Type" = Rec."transaction type"::"Treasury to Intra-Day") or
                      ...
                      (Rec."Transaction Type" = Rec."transaction type"::"Intra-Day to Treasury") then begin
                        if (Rec."Custodian 1 Confirm Issue" <> true) or (Rec."Custodian 2 Confirm Issue" <> true) then begin
                            // Error('Both Custodians have to Confirm issue before you can do %1 ', Rec."Transaction Type");Kitui
                        end;
                    end;
                    ...
                    if (Rec."Transaction Type" = Rec."transaction type"::"Return To Bank") or (Rec."Transaction Type" = Rec."transaction type"::"Issue From Bank") then begin
                    ...
                    (Rec."Transaction Type" = Rec."transaction type"::"Intra-Day to Treasury") or
                    ...
                    (Rec."Transaction Type" = Rec."transaction type"::"Treasury to Intra-Day")
                     then begin
                        ...
                        Message('Are you sure you want to..From..to..of Amount..Shortage %1%2%3%4%5', Rec."Transaction Type", Rec."From Account Name", Rec."To Account Name", Rec.Amount, Rec."Excess/Shortage Amount");
                        if Confirm('Are you sure you want to Proceed', false) = true then begin
            ...
            action(Receive)
            {
                ...
                begin
                    ...
                    if (Rec."Transaction Type" = Rec."transaction type"::"Return To Treasury") or (Rec."Transaction Type" = Rec."transaction type"::"Teller to Intra-Day") or
                    ...
                    /* 
                                        if Rec."Transaction Type" = Rec."transaction type"::"Issue To Teller" then begin
                                            TellerTill.Reset;
                                            ...
                                            if TellerTill.Find('-') then begin
                                                ...
                                                TellerTill.TestField(TellerTill."End of Day Done", true);
                    ...
                    (Rec."Transaction Type" = Rec."transaction type"::"Inter Teller Transfers") or
                     (Rec."Transaction Type" = Rec."transaction type"::"Teller to Intra-Day") or
                     (Rec."Transaction Type" = Rec."transaction type"::"Treasury to Intra-Day") then begin
                    ...
                    if Confirm('Are you sure you want to make this receipt?', false) = true then begin
                        ...
                        /*                         if Rec."Excess/Shortage Amount" > 0 then begin
                                                    ...
                                                    GenJournalLine."Account No." := Rec."Excess Account";
                                                    GenJournalLine."External Document No." := Rec."Cheque No.";
                                                    GenJournalLine."Posting Date" := Today;
                                                    GenJournalLine.Validate(GenJournalLine."Account No.");
                                                    GenJournalLine.Description := Rec.No + ' Excess Incurred on Day Closure';
                                                    GenJournalLine."Currency Code" := Rec."Currency Code";
                                                    GenJournalLine.Validate(GenJournalLine."Currency Code");
                                                    GenJournalLine.Amount := -Rec."Excess/Shortage Amount";
                                                    GenJournalLine.Validate(GenJournalLine.Amount);
                                                    GenJournalLine."Bal. Account Type" := GenJournalLine."bal. account type"::"Bank Account";
                                                    GenJournalLine."Bal. Account No." := Rec."From Account";
                                                    GenJournalLine.Validate(GenJournalLine."Bal. Account No.");
                                                    if GenJournalLine.Amount <> 0 then
                                                        GenJournalLine.Insert;
                                                end;
                                                ...
                                                if Rec."Excess/Shortage Amount" < 0 then begin
                                                    ...
                                                    GenJournalLine."Line No." := LineNo;
                                                    GenJournalLine."Account Type" := GenJournalLine."account type"::"G/L Account";
                                                    GenJournalLine."Account No." := Rec."Shortage Account";
                                                    GenJournalLine."External Document No." := Rec."Cheque No.";
                                                    GenJournalLine."Posting Date" := Today;
                                                    GenJournalLine.Validate(GenJournalLine."Account No.");
                                                    GenJournalLine.Description := Rec.No + ' Shortage Incurred on Day Closure';
                                                    GenJournalLine."Currency Code" := Rec."Currency Code";
                                                    GenJournalLine.Validate(GenJournalLine."Currency Code");
                                                    GenJournalLine.Amount := -1 * Rec."Excess/Shortage Amount";
                                                    GenJournalLine.Validate(GenJournalLine.Amount);
                                                    GenJournalLine."Bal. Account Type" := GenJournalLine."bal. account type"::"Bank Account";
                                                    GenJournalLine."Bal. Account No." := Rec."From Account";
                                                    GenJournalLine.Validate(GenJournalLine."Bal. Account No.");
                    ...
                    if Rec."Transaction Type" = Rec."transaction type"::"End of Day Return to Treasury" then begin
                        ...
                        if TellerTill.Find('-') then begin
                            ...
                            TellerTill."End of Day Done" := true;
                            TellerTill."End of Day Done On" := today;
                            TellerTill.Modify();
            ...
            action("EOD Report")
            {
                ...
                Enabled = rec."Transaction Type" = rec."Transaction Type"::"Return To Treasury";
            ...
            action("Teller-Treasury Report")
            {
                Caption = 'Teller - Treasury Report';
                Image = Print;
                Promoted = true;
                PromotedCategory = "Report";
                Enabled = rec."Transaction Type" <> rec."Transaction Type"::"Return To Treasury";
                ...
                begin
                    ...
                    Trans.SetRange(Trans.No, Rec.No);
                    if Trans.Find('-') then
                        Report.run(50076, true, true, Trans)
                end;
            }
            action(SENDMAIL)
            {
