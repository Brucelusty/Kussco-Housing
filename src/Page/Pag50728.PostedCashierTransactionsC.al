...
Page 50728 "Posted Cashier Transactions C"
{
    ApplicationArea = All;
    DeleteAllowed = false;
    Editable = false;
    ...
    begin
        ...
        if ChequeTypes.Get(Rec."Cheque Type") then begin
            ...
            Trans.SetRange(Trans.No, Rec.No);
            if Trans.Find('-') then
                Report.run(172294, false, true, Trans);
    ...
    begin
                ...
                repeat
                    ...
                    if Rec."Account Type" = 'DIVIDEND' then begin
                        if (Rec.Amount >= 300) and (Rec.Amount < 1000) then
                            ...
                            if (Rec.Amount >= 1001) and (Rec.Amount < 2500) then
                                ChargeAmount := 200
                            else
                                if (Rec.Amount >= 2501) and (Rec.Amount < 5000) then
                                    ChargeAmount := 250
                                else
                                    if (Rec.Amount >= 172001) and (Rec.Amount < 100000) then
                                        ChargeAmount := 300
                                    else
                                        if (Rec.Amount >= 10001) and (Rec.Amount < 200000) then
                                            ChargeAmount := 350
                                        else
                                            if Rec.Amount > 200000 then
                                                ChargeAmount := 400
                    end; //ELSE
                         //ChargeAmount:=TransactionCharges."Charge Amount";
        ...
        if Trans.Find('-') then begin
            if Rec.Type = 'Cash Deposit' then
                Report.run(172292, false, true, Trans)
            /*ELSE IF Type = 'BOSA Receipt' THEN
            Report.run(172292,FALSE,TRUE,Trans)*/
            else
                if Rec.Type = 'Withdrawal' then
                    Report.run(172293, false, true, Trans)
        end;
