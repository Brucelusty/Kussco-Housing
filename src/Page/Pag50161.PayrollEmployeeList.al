...
Page 50161 "Payroll Employee List."
{
    ApplicationArea = All;
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                field("Pays Housing Levy"; Rec."Pays Housing Levy")
                {
                }
                field("Salary Advance Amount"; Rec."Salary Advance Amount")
                {
                }
                field("Helb Loan Amount"; Rec."Helb Loan Amount")
                {
                }
                field("Voluntary Deposit Contribution"; Rec."Voluntary Deposit Contribution")
                {
                }
                field("Pays Voluntary Pension"; Rec."Pays Voluntary Pension")
                {
                }
                field("Voluntary Pension Amount"; Rec."Voluntary Pension Amount")
                {
                }
                field("Earns Leave Allowance"; Rec."Earns Leave Allowance")
                {
                }
                field("Earns Acting Allowance"; Rec."Earns Acting Allowance")
                {
                }
                field("House Allowance Amount"; Rec."House Allowance Amount")
                {
                }
                field("Salary Arrears Amount"; Rec."Salary Arrears Amount")
                {
                }
                field("EOY Amount"; Rec."EOY Amount")
                {
                }
                field(MileageAllowance; Rec.MileageAllowance)
                {
                }
                field("Other Taxable Benefits Amount"; Rec."Other Taxable Benefits Amount")
                {
                }
                field("Other Non Taxable Benefits"; Rec."Other Non Taxable Benefits")
                {
                }
        ...
        area(creation)
        {
            ...
            action("View PaySlip")
            {
                ...
                begin
                    ...
                    if PayrollEmp.FindFirst then begin
                        Report.Run(172009, true, false, PayrollEmp);
                    end;
                end;
            }
            ...
            action("Send Message")
            {
                ...
                begin
                    ...
                    SalaryMonth := FORMAT(SelectedPeriod, 0, '<Month Text>');
                    SalaryYear := Date2DMY(SelectedPeriod, 3);
                    ...
                    if PayrollEmp.FindFirst() then begin
                        repeat
                            Cust.Reset;
                            Cust.Setrange(Cust."No.", PayrollEmp."Payroll No");
                            If Cust.Find('-') then begin
                                if (Cust."Mobile Phone No" <> '') then begin
                                    MessageX := '';
                                    MessageX := 'Dear ' + BName.NameStyle(Cust."No.") + ', Your salary for the month of ' + SalaryMonth + ' ' + Format(SalaryYear) + ' has been posted to your FOSA account.';
                                    smsManagement.SendSmsWithID(Source::SALARY_PROCESSING, Cust."Mobile Phone No", MessageX, Cust."No.", Cust."FOSA Account No.", false, 200, false, 'CBS', CreateGuid(), 'CBS');
                                end;
                            end;
                        until PayrollEmp.Next() = 0;
                        Message('Staff Have Been Notified Successfully.');
                    end;
                end;
            }
        }
