...
Page 50155 "Posted Loan Recovery Header"
{
    ApplicationArea = All;
    DeleteAllowed = false;
    Editable = false;
            ...
            group(General)
            {
                ...
                field("Recovery Type"; Rec."Recovery Type")
                {
                    ...
                    begin
                        ...
                        if Rec."Recovery Type" = Rec."recovery type"::"Recover From Loanee Deposits" then
                            LoanDueVisible := true;
                    end;
                }
                ...
                group("Share Capital Sel")
                {
                    ...
                    //     Editable = false;
                    // }
                    field("Settlement Account"; Rec."Settlement Account")
                    {
                        Editable = false;
                        Visible = false;
                    }
        ...
        area(creation)
        {
            group("Function")
            {
                ...
                action("Post Transaction")
                {
                    ...
                    begin
                        ObjGLEntry.Reset;
                        ObjGLEntry.SetRange(ObjGLEntry."Document No.", Rec."Document No");
                        ...
                        if Rec."Recovery Type" = Rec."recovery type"::"Attach Defaulted Loans to Guarantors" then begin
                            LineNo := 0;
                            FnRunInterest(Rec."Total Interest Due Recovered");
                            FnRunPrincipleThirdparty(Rec."Total Thirdparty Loans");
                            FnRecoverMobileLoanPrincipal(Rec."Mobile Loan");
                            FnRunPrinciple(Rec."Deposits Aportioned");
                        ...
                        if (Rec."Recovery Type" = Rec."recovery type"::"Attach Defaulted Loans to Guarantors") then begin
                            ...
                            if LoanGuarantors.FindSet then begin
                                ...
                                repeat
                                    ...
                                    //  ELSE
                                    LoanDetails."Defaulter Loan" := ROUND(FnGetDefaultorLoanAmount(Rec."Loan Distributed to Guarantors", LoanGuarantors."Amont Guaranteed", LoanGuarantors."Total Loans Guaranteed", GCount), 0.05, '>');

                                    ObjSaccoNoSeries.Get();
                                    //Create Loan
                                    ObjNoSeries.Reset;
                                    ObjNoSeries.SetRange(ObjNoSeries."Series Code", ObjSaccoNoSeries."BOSA Loans Nos");
                                    if ObjNoSeries.FindSet then begin
                                        ...
                                        ObjLoansRec."Client Code" := Rec."Member No";
                                        ObjLoansRec."Client Name" := Rec."Member Name";
                                        ObjLoansRec."Requested Amount" := ROUND(FnGetDefaultorLoanAmount(Rec."Loan Distributed to Guarantors", LoanGuarantors."Amont Guaranteed", LoanGuarantors."Total Loans Guaranteed", GCount), 0.05, '>');
                                        ObjLoansRec."Approved Amount" := ROUND(FnGetDefaultorLoanAmount(Rec."Loan Distributed to Guarantors", LoanGuarantors."Amont Guaranteed", LoanGuarantors."Total Loans Guaranteed", GCount), 0.05, '>');
                ...
                action("Apportion Liability")
                {
                    ...
                    trigger OnAction()
                    begin
                        VarTotalLoanLiabilities := Rec."Loan Distributed to Guarantors";//"Loan Liabilities"+"Outstanding Insurance"+"Total Interest Due Recovered"+"Outstanding Penalty";
                        ...
                        if Rec."Guarantor Allocation Type" = Rec."guarantor allocation type"::"Equally Liable" then begin
                            ...
                            ObjLoanGuar.Reset;
                            ObjLoanGuar.SetRange(ObjLoanGuar."Document No", Rec."Document No");
                            ...
                            ObjLoanGuarantorsII.SetRange(ObjLoanGuarantorsII."Document No", Rec."Document No");
                            ...
                            while (VarRemainingLiability > 0) and (VarCountRemainingGuarantors > 0) do begin
                                ...
                                ObjLoanGuarantorsIII.SetRange(ObjLoanGuarantorsIII."Document No", Rec."Document No");
                                ...
                                ObjLoanGuarantorsV.SetRange(ObjLoanGuarantorsV."Document No", Rec."Document No");
    ...
    begin
        if LoansRec.Get(Rec."Loan to Attach") then begin
            ...
            'Defaulted Loan Recovered-' + Rec."Loan to Attach", Rec."Loan to Attach", GenJournalLine."application source"::" ");//Maximum No of Parameters(13) Exceeded
    ...
    begin
        begin
            if LoansRec.Get(Rec."Loan to Attach") then begin
                ...
                Format(GenJournalLine."transaction type"::Repayment), Rec."Loan to Attach", GenJournalLine."application source"::" ");
                //--------------------RECOVER-----------------------------
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
    ...
    begin
        ...
        if LoanDetails.FindSet then begin
            repeat
                ...
                GenJournalLine."account type"::Customer, LoanDetails."Guarantor Number", Rec."Loan Disbursement Date", LoanDetails."Defaulter Loan", Format(LoansRec.Source::BOSA), Rec."Loan to Attach",
                'Defaulter Recovery-' + Rec."Loan to Attach", LoanDetails."Defaulter Loan No", GenJournalLine."application source"::" ");//DLoan
        ...
        if LoansRec.Get(Rec."Loan to Attach") then begin
            ...
            GenJournalLine."Recoverd Loan" := Rec."Loan to Attach";
    ...
    //                         GenJournalLine."account type"::Customer, LoanApp."Client Code", "Loan Disbursement Date", 'Loan Insurance:_' + "Document No", GenJournalLine."bal. account type"::"G/L Account",
    ...
    //                         GenJournalLine."account type"::Customer, LoanApp."Client Code", "Loan Disbursement Date", AmountToDeduct * -1, Format(LoanApp.Source), EXTERNAL_DOC_NO,
    ...
    //     LineNo := LineNo + 10000;
    //     SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
    //     GenJournalLine."account type"::Customer, "Member No", "Loan Disbursement Date", VarAmounttoDeduct, 'BOSA', EXTERNAL_DOC_NO,
    ...
    //                                 LineNo := LineNo + 10000;
    //                                 SFactory.FnCreateGnlJournalLineBalanced(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Loan Insurance Charged",
    //                                 GenJournalLine."account type"::Customer, VarMemberNo, VarPostingDate, 'Loan Insurance:_' + "Document No", GenJournalLine."bal. account type"::"G/L Account",
