...
Page 50355 "User Approval Signature"
{
    ApplicationArea = All;
    PageType = CardPart;
...
/*                     ExportPath := TemporaryPath + Rec."User ID" + Format(Rec.Signature.MediaId);
                    Rec.Signature.ExportFile(ExportPath);
                    FileManagement.GetServerDirectoryFilesList(TempNameValueBuffer, Rec.TemporaryPath);
                    TempNameValueBuffer.SetFilter(Name, StrSubstNo('%1*', ExportPath));
                    TempNameValueBuffer.FindFirst;
                    ToFile := StrSubstNo('%1 %2.jpg', Rec.Signature, ConvertStr(Rec."User ID", '"/\', '___'));
