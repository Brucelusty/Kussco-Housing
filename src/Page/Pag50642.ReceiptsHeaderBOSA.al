...
Page 50642 "Receipts Header-BOSA"
{
    ApplicationArea = All;
    DeleteAllowed = false;
    Editable = true;
        ...
        area(processing)
        {
            action(Post)
            {
                ...
                begin
                    ...
                    if (Rec."Account Type" =Rec. "account type"::Member) or (Rec."Account Type" = Rec."account type"::Customer) then begin
                    ...
                    if (Rec."Account Type" <> Rec."account type"::Member) and (Rec."Account Type" <> Rec."account type"::Customer) and (Rec."Account Type" <> Rec."account type"::Vendor) then begin
                        LineNo := LineNo + 10000;
                    ...
                    if (Rec."Account Type" = Rec."account type"::Member) or (Rec."Account Type" =Rec. "account type"::Customer) or (Rec."Account Type" =Rec. "account type"::Vendor) then begin
                            ...
                            repeat
                                ...
                                if ReceiptAllocations."Transaction Type" = ReceiptAllocations."transaction type"::Repayment then begin
                                    ...
                                    if Rec."Account Type" = Rec."account type"::Member then begin
                                        ...
                                        GenJournalLine."Account No." := ReceiptAllocations."Member No";
                                        GenJournalLine.Validate(GenJournalLine."Account No.");
                                        if (Rec."Receipt Mode" = Rec."receipt mode"::Mpesa) and (ReceiptAllocations."Transaction Type" = ReceiptAllocations."transaction type"::" ") then begin
                                            GenJournalLine.Amount := -Rec.Amount;
                                            GenJournalLine."Account Type" := GenJournalLine."account type"::"G/L Account";
                                            GenJournalLine."Account No." := GenSetup."FOSA MPESA COmm A/C";
                                            GenJournalLine.Description := Format(ReceiptAllocations."Transaction Type") + '-' + Rec.Remarks;
                        ...
                        if/* ("Mode of Payment"<>"Mode of Payment"::"Standing order") AND */
                          //("Mode of Payment"<>"Mode of Payment"::"Direct Debit") AND
                           (Rec."Receipt Mode" <> Rec."receipt mode"::Mpesa) then begin
                            ...
                            BOSARcpt.SetRange(BOSARcpt."Transaction No.", Rec."Transaction No.");
                            if BOSARcpt.Find('-') then
                                Report.run(172387, false, true, BOSARcpt);
            ...
            action("Reprint Frecipt")
            {
                ...
                begin
                    ...
                    BOSARcpt.SetRange(BOSARcpt."Transaction No.", Rec."Transaction No.");
                    if BOSARcpt.Find('-') then
                        Report.run(172387, true, true, BOSARcpt)
                end;
            }
        }
    }
