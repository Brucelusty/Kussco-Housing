...
Page 50873 "Posted Finance UpLoads Header"
{
    ApplicationArea = All;
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            ...
            part("172000";"Finance UpLoads Lines")
            {
                Caption = 'Salary Processing Lines';
                Enabled = false;
                //   SubPageLink = Field62003=field(No);
            }
        }
    }
            ...
            group(ActionGroup1102755021)
            {
                ...
                action("Process Salaries")
                {
                    ...
                    begin
                        ...
                        if Rec."Scheduled Amount" > Rec."Balancing Account Balance" then
                    ...
                    begin
                        ...
                        if salarybuffer.Find('-') then begin
                            ...
                            repeat
                                ...
                                if Rec."Account Balancing Type" = Rec."account balancing type"::"LampSum Balancing" then
    ...
    begin
        ...
        GenJournalLine."account type"::Vendor, ObjSalaryLines."Account No.", Rec."Posting date", ObjSalaryLines.Amount * -1, 'FOSA', EXTERNAL_DOC_NO, ObjSalaryLines."Credit Narration", '', GenJournalLine."application source"::" ");
        exit(RunningBalance);
    end;
    ...
    begin
        ...
        if Charges.Get('SALARYP') then begin
            ...
            LineNo := LineNo + 10000;
            SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
            GenJournalLine."account type"::"G/L Account", ObjGenSetup."Excise Duty Account", Rec."Posting date", SalProcessingFee * -0.1, 'FOSA', EXTERNAL_DOC_NO,
            Rec."Transaction Description", '', GenJournalLine."application source"::" ");
            //--------------RECOVER------------------
            LineNo := LineNo + 10000;
            SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
            GenJournalLine."account type"::Vendor, ObjSalaryLines."Account No.", Rec."Posting date", SalProcessingFee * 0.1, 'FOSA', EXTERNAL_DOC_NO,
                ...
                repeat
                    ...
                    if (SFactory.FnGetInterestDueFiltered(LoanApp, Datefilter) - Abs(LoanApp."Outstanding Interest")) > 0 then begin
                        if RunningBalance > 0 then begin
                            ...
                            GenJournalLine."account type"::Member, ObjRcptBuffer."Account No.", Rec."Posting date", AmountToDeduct * -1, 'FOSA', EXTERNAL_DOC_NO,
                        ...
                        if ObjLoanProducts."Recovery Method" = ObjLoanProducts."recovery method"::"Salary " then begin
                            ...
                            if (SFactory.FnGetInterestDueFiltered(LoanApp, Datefilter) - Abs(LoanApp."Outstanding Interest")) > 0 then begin
                                if RunningBalance > 0 then begin
                                    ...
                                    LineNo := LineNo + 10000;
                                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Interest Paid",
                                    GenJournalLine."account type"::Member, ObjRcptBuffer."Account No.", Rec."Posting date", AmountToDeduct * -1, Format(LoanApp.Source), EXTERNAL_DOC_NO,
                                    Format(GenJournalLine."transaction type"::"Interest Paid"), LoanApp."Loan  No.", GenJournalLine."application source"::" ");
    ...
    begin
        ...
        salarybuffer.SetRange("Salary Header No.", Rec.No);
        if salarybuffer.Find('-') then begin
            Window.Open(Format(Rec."Account Balancing Type") + ': @1@@@@@@@@@@@@@@@' + 'Record:#2###############');
            TotalCount := salarybuffer.Count;
            repeat
                Percentage := (ROUND(Counter / TotalCount * 10000, 1));
                Counter := Counter + 1;
