...
Page 50271 "HR Leave Applications History"
{
    ApplicationArea = All;
    UsageCategory = Lists;
    ...
    // SourceTableView = where(Status = const(Approved), Posted = filter(false));
    ...
    begin
        ...
        HRLeaveApp.Reset();
        HRLeaveApp.SetRange("Is Manager", false);
        ...
        if HRLeaveApp.FindSet() then begin
            repeat
                ...
                leaveSup.SetRange("Approval Status", leaveSup."Approval Status"::Approved);
    ...
    begin
        ...
        repeat
            if Rec.DetermineIfIncludesNonWorking(Rec."Leave Type") = false then begin
                ...
                if Rec.DetermineIfIsNonWorking(fReturnDate, HRLeaveTypes) then
    ...
    begin
        ...
        if fEndDate <> 0D then begin
            ...
            while (ReturnDateLoop) do begin
                if Rec.DetermineIfIsNonWorking(fEndDate, HRLeaveTypes) then
                    fEndDate := CalcDate('-1D', fEndDate)
                else
