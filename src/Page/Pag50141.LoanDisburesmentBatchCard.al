...
Page 50141 "Loan Disburesment Batch Card"
{
    ApplicationArea = All;
    DeleteAllowed = false;
    PageType = Card;
    ...
    SourceTableView = where(Posted = filter(false), "Disbursement Type" = const(Normal));
        ...
        area(content)
        {
            ...
            field("Mode Of Disbursement"; Rec."Mode Of Disbursement")
            {
                ...
                begin
                    ...
                    "Cheque No.":="Batch No.";
                    MODIFY;  */
                    if Rec."Mode Of Disbursement" <> Rec."mode of disbursement"::"Transfer to FOSA" then
                        Rec."Cheque No." := Rec."Batch No.";
                    rec.Modify;
            ...
            group(LoansB)
            {
                ...
                action("Loans Payment Voucher")
                {
                    ...
                    begin
                        ...
                        Loans.Reset;
                        Loans.SetRange(Loans."Batch No.", Rec."Batch No.");
                        if Loans.Find('-') then begin
                            Report.run(172953, true, false, Loans)
                        end;
                    end;
                }
                    ...
                    begin
                        ...
                        if LoansBatch.Find('-') then begin
                            if LoansBatch."Batch Type" = LoansBatch."batch type"::"Personal Loans" then
                                Report.Run(172485, true, false, LoansBatch)
                            else
                ...
                action("Send A&pproval Request")
                {
                    ...
                    begin
                        ...
                        if Confirm('Kindly confirm the posting date of this batch.', true) = false then exit;
                        ...
                        Audit.FnInsertAuditRecords(UserId, 'Send Batch For Approval', Rec."Total Loan Amount", Rec."Batch No.", today, time, '', Rec."Batch No.", '', '');
                    end;
                }
                ...
                action(Post)
                {
                    ...
                    begin
                        ...
                        if rec.Status <> rec.Status::Approved then
                            Error('Loan Batch must be Approved');
                        ...
                        if LoanApps.find('-') then begin
                            Error('The loan has already been posted under Batch No.: %1 - Loan No.: %2.', Rec."Batch No.", LoanApps."Loan  No.");
                        end;

                        LoanApps.Reset;
                        LoanApps.SetRange("Batch No.", Rec."Batch No.");
                        LoanApps.SetRange("System Created", false);
                        LoanApps.SetFilter("Approval Status", '<>%1', loanApps."Approval Status"::Approved);
                        if LoanApps.find('-') then begin
                            Error('The loan has not been approved Loan No%2', LoanApps."Loan  No.");
                        end;

                        if Confirm('Do you wish to proceed with posting this loan batch?', true) = false then exit;
                        ...
                        LoanApps.Reset;
                        LoanApps.SetRange(LoanApps."Batch No.", Rec."Batch No.");
                        LoanApps.SetRange("Loan Status", LoanApps."Loan Status"::Approved);
                        LoanApps.SetRange(LoanApps."System Created", false);
                        LoanApps.SetRange(LoanApps.Posted, false);
                        if LoanApps.Find('-') then begin
                            repeat
                                if SFactory.FnCheckIfDocNoAlreadyPosted(LoanApps."Loan  No.") then Error('The Loan: %1, has already been posted. Kindly investigate the ledger entries in case a reversal is required.', LoanApps."Loan  No.");
                            ...
                            // Members.Reset;
                            // Members.Setrange(Members."No.", LoanApps."Client Code");
                            // Members.Setfilter(Members."Mobile Phone No", '<>%1', '');
                            // if Members.FindFirst() then begin
                            //     MessageX := 'Dear ' + NameStyle(Members."No.") + '. Your ' + LoanApps."Loan Product Type Name" + ' Loan of Ksh ' + format(LoanApps."Approved Amount") + ' has been disbursed to your FOSA Account.';
                            //     // smsManagement.SendSmsResponse(Members."Mobile Phone No", MessageX);
                            //     smsManagement.SendSmsWithID(Source::LOAN_POSTED, Members."Mobile Phone No", MessageX, Members."FOSA Account No.", Members."FOSA Account No.", TRUE, 200, TRUE, 'CBS', CreateGuid(), 'CBS');
                            // end;
                        ...
                        Audit.FnInsertAuditRecords(UserId, 'Post Loan Batch', Rec."Total Loan Amount", Rec."Batch No.", today, time, '', Rec."Batch No.", '', '');
                        LoanApps.Reset;
                        LoanApps.SetRange(LoanApps."Batch No.", Rec."Batch No.");
                        LoanApps.SetRange(LoanApps."System Created", false);
                        if LoanApps.Find('-') then begin
                            repeat
                                ...
                                //validate loans
                                A03Recov.FnRecoverMobiAdvance(LoanApps."Client Code");
                                ...
                                LoanApps."Disbursed By Date" := Today;
                                LoanApps."Disbursed By Time" := Time;
                                ...
                                LoanApps.Validate(LoanApps."Loan Disbursement Date");
                            ...
                            // if Loans.Find('-') then begin
                            //     Report.run(172953, false, true, Loans)
                            // end;
                ...
                action(MarkAsPosted)
                {
                    ...
                    begin
                        ...
                        if LoanApps.Find('-') then begin
                            ...
                            Rec."Posted By" := UserId;
                            Rec."Posting Date" := Today;
                            Message('Loans Marked As Disbursed Successfully.');
                            Rec.Modify();
                        end;
    ...
    begin
        LoanBatch.Reset();
        LoanBatch.SetRange(LoanBatch."Prepared By", UserId);
        LoanBatch.SetRange(LoanBatch.Status, LoanBatch.Status::Open);
        if LoanBatch.FindSet() then begin
            TransCount := LoanBatch.count;
            if TransCount > 2 then
                error('Kindly utilize the unused transaction cards first.');
        end;
    end;
    ...
    var
        ...
        A03Recov: Codeunit "Recover Default Mobile Advance";
    ...
    begin
        ...
        if rec."Mode Of Disbursement" = rec."mode of disbursement"::"Transfer to FOSA" then begin
            LoanApps.Reset;
            LoanApps.SetRange(LoanApps."Batch No.", rec."Batch No.");
            LoanApps.SetRange(LoanApps."System Created", false);
            if LoanApps.Find('-') then begin
                repeat
                    ...
                    DataSheet."REF." := '2026';
                    DataSheet."Batch No." := rec."Batch No.";
    ...
    begin
        ...
        IF LoanApps.GET(VarLoanNo) THEN BEGIN
            ...
            GenJournalLine."Account Type"::Customer, LoanApps."Client Code", rec."Posting Date", VarAmounttoDisburse, FORMAT(LoanApps.Source), LoanApps."Loan  No.",
            ...
            GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", VarAmounttoDisburse * -1, 'BOSA', LoanApps."Loan  No.",
            ...
            //Message('START');
            if LoanApps."Agency EFT" > 0 then begin
                //------------------------------Credit Excise Duty Account-----------------------------------------------
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::Customer, 'CUST-001', rec."Posting Date", LoanApps."Agency EFT" * -1, 'BOSA', BLoan,
                'Agency EFT Ref:' + LoanApps."Agency EFT Ref", BLoan, GenJournalLine."Application Source"::" ");

                //----------------------Debit FOSA A/C Excise Duty on Bridging Fee-----------------------------------------------------
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", LoanApps."Agency EFT", 'BOSA', BLoan,
                'Agency EFT Ref:' + LoanApps."Agency EFT Ref", BLoan, GenJournalLine."Application Source"::" ");
            end;

            if LoanApps."Employer Code" = 'PENSIONERS' then begin
                Vend.Reset();
                Vend.SetRange("BOSA Account No", LoanApps."Client Code");
                Vend.SetRange("Account Type", '102');
                if Vend.Find('-') then begin
                    vend.CalcFields(Balance);
                    if vend.Balance <= 300000 then begin
                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", (VarAmounttoDisburse * 0.05), 'BOSA', LoanApps."Loan  No.",
                        '5% Pensioners Deposit Contribution - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");

                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, vend."No.", rec."Posting Date", ((VarAmounttoDisburse * -1) * 0.05), 'BOSA', LoanApps."Loan  No.",
                        '5% Pensioners Deposit Contribution - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                    end;
                end;
            end;

            if LoanApps."Loan Product Type" = 'A11' then begin
                if loanProduct.Get(LoanApps."Loan Product Type") then begin
                    AmountShares := 0;
                    AmountSharesPerc := 0;
                    TotalShares := 0;

                    FOSAAccount.Reset();
                    FOSAAccount.SetRange(FOSAAccount."BOSA Account No", LoanApps."Client Code");
                    FOSAAccount.SetFilter(FOSAAccount."Account Type", '101');
                    IF FOSAAccount.FindFirst() then BEGIN
                        AmountShares := LoanApps."Approved Amount" * (loanProduct."% Share Capitalization" / 100);
                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, FOSAAccount."No.", rec."Posting Date", AmountShares * -1, FORMAT(LoanApps.Source), LoanApps."Loan  No.",
                        'Share Capital Percentage On Karibu Loan - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                        //--------------------------------(CREDIT Member SHARECAP Account)---------------------------------------------

                        //------------------------------------2. DEBIT FOSA A/C---------------------------------------------------------------------------------------------
                        //Message('Sharecapital %1', LoanApps."Share Capital Boosting");
                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", AmountShares, 'BOSA', LoanApps."Loan  No.",
                        'Share Capital Percentage On Karibu Loan - ' + LoanApps."Loan Product Type Name" + ' - ' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                    end;
                    FOSAAccount.Reset();
                    FOSAAccount.SetRange(FOSAAccount."BOSA Account No", LoanApps."Client Code");
                    FOSAAccount.SetFilter(FOSAAccount."Account Type", '101');
                    IF FOSAAccount.FindFirst() then BEGIN
                        AmountSharesPerc := loanProduct."Share Capitalization";
                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, FOSAAccount."No.", rec."Posting Date", AmountSharesPerc * -1, FORMAT(LoanApps.Source), LoanApps."Loan  No.",
                        'Share Capital Amount On Karibu Loan - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                        //--------------------------------(CREDIT Member SHARECAP Account)---------------------------------------------
                        ...
                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", AmountSharesPerc, 'BOSA', LoanApps."Loan  No.",
                        'Share Capital Amount On Karibu Loan - ' + LoanApps."Loan Product Type Name" + ' - ' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                    end;
                    TotalShares := AmountShares + AmountSharesPerc;
                end;
            end;

            if LoanApps."Loan Product Type" <> 'A11' then begin
                if (LoanApps."Recover Share Capital") - totalShares > 0 then begin
                    FOSAAccount.Reset();
                    FOSAAccount.SetRange(FOSAAccount."BOSA Account No", LoanApps."Client Code");
                    FOSAAccount.SetFilter(FOSAAccount."Account Type", '101');
                    IF FOSAAccount.FindFirst() then BEGIN
                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, FOSAAccount."No.", rec."Posting Date", ((LoanApps."Recover Share Capital") - TotalShares) * -1, FORMAT(LoanApps.Source), LoanApps."Loan  No.",
                        'Share Capital Recovery - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                        //--------------------------------(CREDIT Member SHARECAP Account)---------------------------------------------

                        //------------------------------------2. DEBIT FOSA A/C---------------------------------------------------------------------------------------------
                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", ((LoanApps."Recover Share Capital") - TotalShares), ' ', LoanApps."Loan  No.",
                        'Share Capital Recovery - ' + LoanApps."Loan Product Type Name" + ' - ' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                    end;
                end;
            end;

            // //------------------------------------1. CREDIT MEMBER DEPOSIT A/C---------------------------------------------------------------------------------------------
            // LineNo := LineNo + 10000;
            ...
            // GenJournalLine."Account Type"::Customer, LoanApps."Client Code", rec."Posting Date", LoanApps."Boosted Amount" * -1, FORMAT(LoanApps.Source), LoanApps."Loan  No.",
            // 'Deposit Boosting - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
            // //--------------------------------(CREDIT Member DEPOSIT Account)---------------------------------------------

            // //------------------------------------2. DEBIT FOSA A/C---------------------------------------------------------------------------------------------
            // LineNo := LineNo + 10000;
            // SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
            // GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", LoanApps."Boosted Amount", ' ', LoanApps."Loan  No.",
            // 'Deposit Boosting -  ' + LoanApps."Loan Product Type Name" + ' - ' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
            // //----------------------------------(Debit Member Fosa Account)------------------------------------------------


            //------------------------------------ . Deposit Boosting on VUKA LOAN . (20%)/ According to Setup. ---------------------------------------------------------------------------------------------

            if LoanApps."Loan Product Type" = 'A20' then begin
                if loanProduct.Get(LoanApps."Loan Product Type") then begin
                    AmountShares := 0;
                    AmountSharesPerc := 0;
                    TotalShares := 0;
                    ...
                    FOSAAccount.Reset();
                    FOSAAccount.SetRange(FOSAAccount."BOSA Account No", LoanApps."Client Code");
                    FOSAAccount.SetFilter(FOSAAccount."Account Type", '102');
                    IF FOSAAccount.FindFirst() then BEGIN
                        AmountShares := LoanApps."Approved Amount" * (loanProduct."% Share Capitalization" / 100);
                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, FOSAAccount."No.", rec."Posting Date", AmountShares * -1, FORMAT(LoanApps.Source), LoanApps."Loan  No.",
                        'Deposits Percentage On VUKA Loan - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                        //--------------------------------(CREDIT Member SHARECAP Account)---------------------------------------------
                        ...
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", AmountShares, 'BOSA', LoanApps."Loan  No.",
                        'Deposits Percentage On VUKA Loan - ' + LoanApps."Loan Product Type Name" + ' - ' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                    end;
                    FOSAAccount.Reset();
                    FOSAAccount.SetRange(FOSAAccount."BOSA Account No", LoanApps."Client Code");
                    FOSAAccount.SetFilter(FOSAAccount."Account Type", '102');
                    IF FOSAAccount.FindFirst() then BEGIN
                        ...
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, FOSAAccount."No.", rec."Posting Date", AmountSharesPerc * -1, FORMAT(LoanApps.Source), LoanApps."Loan  No.",
                        'Deposits Amount On VUKA Loan - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                        //--------------------------------(CREDIT Member SHARECAP Account)---------------------------------------------
                        ...
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", AmountSharesPerc, 'BOSA', LoanApps."Loan  No.",
                        'Deposits Amount On VUKA Loan - ' + LoanApps."Loan Product Type Name" + ' - ' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                    end;
                    TotalShares := AmountShares + AmountSharesPerc;
                end;
            end;
            ...
            //------------------------------------1. CREDIT MEMBER SHARECAPITAL A/C---------------------------------------------------------------------------------------------
            // FOSAAccount.Reset();
            // FOSAAccount.SetRange(FOSAAccount."BOSA Account No", LoanApps."Client Code");
            // FOSAAccount.SetFilter(FOSAAccount."Account Type", '101');
            // IF FOSAAccount.FindFirst() then BEGIN
            //     LineNo := LineNo + 10000;
            //     SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
            //     GenJournalLine."Account Type"::Vendor, FOSAAccount."No.", rec."Posting Date", LoanApps."Share Capital Amount" * -1, FORMAT(LoanApps.Source), LoanApps."Loan  No.",
            //     'ShareCapital Deducted - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
            //     //--------------------------------(CREDIT Member SHARECAP Account)---------------------------------------------
            ...
            //     LineNo := LineNo + 10000;
            //     SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
            //     GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", LoanApps."Share Capital Amount", ' ', LoanApps."Loan  No.",
            //     'ShareCapital Deducted - ' + LoanApps."Loan Product Type Name" + ' - ' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
            // end;

            //----------------------------------(Debit Member Fosa Account)------------------------------------------------
            // Error('STOP');

            //------------------------------------1. CREDIT EXTERNAL GL A/C---------------------------------------------------------------------------------------------
            if LoanApps."External loan" > 0 then begin
                GenSetUp.TESTFIELD(GenSetUp."External Loan Account");
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::"G/L Account", GenSetUp."External Loan Account", rec."Posting Date", LoanApps."External loan" * -1, FORMAT(LoanApps.Source), LoanApps."Loan  No.",
                'Extenal Loan Payment - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                //--------------------------------(CREDIT Member SHARECAP Account)---------------------------------------------
                ...
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", LoanApps."External loan", 'BOSA', LoanApps."Loan  No.",
                'Extenal Loan Payment - ' + LoanApps."Loan Product Type Name" + ' - ' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");

                //----------------------------------(Debit Member Fosa Account)------------------------------------------------

                GenSetUp.TESTFIELD(GenSetUp."External Loan Account");
                GenSetUp.TESTFIELD(GenSetUp."Penalties on Loans");
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::"G/L Account", GenSetUp."Penalties on Loans", rec."Posting Date", ((LoanApps."External loan" * -1) * (GenSetUp."External Loan Comm (%)")) / 100, FORMAT(LoanApps.Source), LoanApps."Loan  No.",
                'Extenal Loan Commission - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                //--------------------------------(CREDIT Member SHARECAP Account)---------------------------------------------
                ...
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", ((LoanApps."External loan") * (GenSetUp."External Loan Comm (%)")) / 100, 'BOSA', LoanApps."Loan  No.",
                'Extenal Loan Commission - ' + LoanApps."Loan Product Type Name" + ' - ' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                //----------------------------------(Debit Member Fosa Account)------------------------------------------------
                GenSetUp.TESTFIELD(GenSetUp."External Loan Account");
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::"G/L Account", GenSetUp."Excise Duty Account", rec."Posting Date", ((LoanApps."External loan" * -1) * (GenSetUp."External Loan Comm (%)") / 100) * GenSetUp."Excise Duty(%)" / 100, FORMAT(LoanApps.Source), LoanApps."Loan  No.",
                'Extenal Loan Excise - ' + LoanApps."Loan Product Type Name", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                //--------------------------------(CREDIT Member SHARECAP Account)---------------------------------------------
                ...
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", ((LoanApps."External loan") * (GenSetUp."External Loan Comm (%)") / 100) * GenSetUp."Excise Duty(%)" / 100, 'BOSA', LoanApps."Loan  No.",
                'Extenal Loan Excise - ' + LoanApps."Loan Product Type Name" + ' - ' + LoanApps."Loan  No.", LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                //----------------------------------(Debit Member Fosa Account)------------------------------------------------
                // Error('STOP');
            end;
            ...
            IF PCharges.FIND('-') THEN BEGIN
                REPEAT
                    ...
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                    GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", PChargeAmount, 'BOSA', LoanApps."Loan  No.",
                    ...
                    IF SFactory.FnChargeExcise(PCharges.Code) THEN BEGIN
                        ...
                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::"G/L Account", GenSetUp."Excise Duty Account", rec."Posting Date", (PChargeAmount * -1) * (GenSetUp."Excise Duty(%)" / 100), 'BOSA', LoanApps."Loan  No.",
                        ...
                        LineNo := LineNo + 10000;
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", PChargeAmount * (GenSetUp."Excise Duty(%)" / 100), 'BOSA', LoanApps."Loan  No.",
                        PCharges.Description + '-' + LoanApps."Loan Product Type Name" + ' - Excise Duty', LoanApps."Loan  No.", GenJournalLine."Application Source"::" ");
                    END
                //----------------END 10% EXCISE--------------------------------------------
            ...
            IF LoanApps."Loan Offset Amount" > 0 THEN BEGIN
                ...
                IF LoanTopUp.FIND('-') THEN BEGIN
                    REPEAT
                        //------------------------------------PAY-----------------------------------------------------------------------------------------------------------
                        //------------------------------------Principal---------------------
                        if LoanTopUp."Principle Top Up" > 0 then begin
                            LineNo := LineNo + 10000;
                            ...
                            /*                         LineNo := LineNo + 10000;
                                                    IF LoanType.GET(LoanApps."Loan Product Type") THEN BEGIN
                                                        ...
                                                        'Refinancing Charges -' + LoanApps."Client Code" + '-' + LoanApps."Loan  No.", LoanTopUp."Loan Top Up", GenJournalLine."Application Source"::" ");
                                                    END;
                                                    LineNo := LineNo + 10000;
                                                    IF LoanType.GET(LoanApps."Loan Product Type") THEN BEGIN
                                                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                                                        GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", LoanTopUp.Commision, 'BOSA', LoanTopUp."Loan Top Up",
                                                        'Refinancing Charges -' + LoanApps."Client Code" + '-' + LoanApps."Loan  No.", LoanTopUp."Loan Top Up", GenJournalLine."Application Source"::" ");
                                                    END; */
                            ...
                            IF LoanType.GET(LoanApps."Loan Product Type") THEN BEGIN
                                ...
                                GenJournalLine."Account Type"::Customer, LoanApps."Client Code", rec."Posting Date", LoanTopUp."Loan Insurance: Current Year" * -1, 'BOSA', LoanTopUp."Loan Top Up",
                            ...
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", LoanTopUp."Principle Top Up", 'BOSA', LoanTopUp."Loan Top Up",
                            'Loan Offset  - ' + LoanTopUp."Loan Product", LoanTopUp."Loan Top Up", GenJournalLine."Application Source"::" ");
                            //-------------------------------------Outstanding Interest-------------------------------------------------------------------------------------
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", LoanTopUp."Interest Top Up", 'BOSA', LoanTopUp."Loan Top Up",
                            'Interest Paid - ' + LoanTopUp."Loan Product", LoanTopUp."Loan Top Up", GenJournalLine."Application Source"::" ");

                            //-------------------------------------Insurance For the Year-------------------------------------------------------------------------------------
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", LoanTopUp."Loan Insurance: Current Year", 'BOSA', LoanTopUp."Loan Top Up",
                            'Insurance on Loan Clearance - ' + LoanTopUp."Loan Product", LoanTopUp."Loan Top Up", GenJournalLine."Application Source"::" ");
                            //--------------------------------------Levies--------------------------------------------------------------------------------------------------
                            ...
                            IF LoanType.GET(LoanApps."Loan Product Type") THEN BEGIN
                                ...
                                LineNo := LineNo + 10000;
                                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                                GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", LoanTopUp."Interest On TopUp", 'BOSA', LoanTopUp."Loan Top Up",
                                'Interest on Top Up - ' + LoanTopUp."Loan Product", LoanTopUp."Loan Top Up", GenJournalLine."Application Source"::" ");

                                LineNo := LineNo + 10000;
                                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                                GenJournalLine."Account Type"::"G/L Account", LoanType."Loan Interest Account", rec."Posting Date", (LoanTopUp."Interest On TopUp") * -1, 'BOSA', LoanTopUp."Loan Top Up",
                                'Interest on Top Up - ' + LoanTopUp."Loan Product", LoanTopUp."Loan Top Up", GenJournalLine."Application Source"::" ");

                                LineNo := LineNo + 10000;
                                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                                GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", LoanTopUp.Commision, 'BOSA', LoanTopUp."Loan Top Up",
                                'Refinancing Charges - ' + LoanTopUp."Loan Product", LoanTopUp."Loan Top Up", GenJournalLine."Application Source"::" ");

                                LineNo := LineNo + 10000;
                                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                               GenJournalLine."Account Type"::"G/L Account", LoanType."Loan Interest Account", rec."Posting Date", LoanTopUp.Commision * -1, 'BOSA', LoanTopUp."Loan Top Up",
                               'Refinancing Charges - ' + LoanTopUp."Loan Product", LoanTopUp."Loan Top Up", GenJournalLine."Application Source"::" ");
                            END;
                            ...
                            'Excise Duty_Refinancing Charges', BLoan, GenJournalLine."Application Source"::" ");

                            //----------------------Debit FOSA A/C Excise Duty on Bridging Fee-----------------------------------------------------
                            LineNo := LineNo + 10000;
                            SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                            GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", (LoanTopUp.Commision * (GenSetUp."Excise Duty(%)" / 100)), 'BOSA', BLoan,
                            'Excise Duty_Refinancing Charges', BLoan, GenJournalLine."Application Source"::" ");
            ...
            IF LoanType.GET(LoanApps."Loan Product Type") THEN BEGIN
                IF LoanType."Accrue Total Insurance&Interes" = TRUE THEN BEGIN
                    ...
                    GenJournalLine."Account Type"::Customer, LoanApps."Client Code", rec."Posting Date", (LoanApps."Approved Amount" * (LoanType."Interest rate" / 1200)) * LoanType."No of Installment", 'BOSA', BLoan,
            ...
            IF LoanType.GET(LoanApps."Loan Product Type") THEN BEGIN
                IF LoanType."Accrue Total Insurance&Interes" = TRUE THEN BEGIN
                    ...
                    IF PCharges.FIND('-') THEN BEGIN
                        ...
                        GenJournalLine."Account Type"::Customer, LoanApps."Client Code", rec."Posting Date", PChargeAmount * LoanType."No of Installment", 'BOSA', BLoan,
            ...
            IF LoanApps."Share Boosting Comission" > 0 THEN BEGIN
                ...
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", LoanApps."Share Boosting Comission", 'BOSA', BLoan,
                ...
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", (LoanApps."Share Boosting Comission" * (GenSetUp."Excise Duty(%)" / 100)), 'BOSA', BLoan,
            ...
            IF (LoanApps."Manual Special Advance" = true) and (LoanApps."Special Advance" > 0) THEN BEGIN
                Vend.Reset();
                Vend.SetRange(Vend."BOSA Account No", LoanApps."Client Code");
                Vend.SetRange(Vend."Account Type", '102');
                if Vend.FindFirst() then begin
                    //----------------------Debit FOSA a/c-----------------------------------------------------
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                    GenJournalLine."Account Type"::Vendor, LoanApps."Account No", rec."Posting Date", LoanApps."Special Advance", 'BOSA', BLoan,
                    'Deposit Boosting', BLoan, GenJournalLine."Application Source"::" ");

                    //----------------------Debit FOSA A/C Excise Duty on Boosting Fee-----------------------------------------------------
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                    GenJournalLine."Account Type"::Vendor, Vend."No.", rec."Posting Date", -LoanApps."Special Advance", 'BOSA', BLoan,
                    'Deposit Boosting', BLoan, GenJournalLine."Application Source"::" ");
                end;
            END;
            ...
            if LoanTypes.Get(LoanTopUp."Loan Type") then begin
                if Cust.Get(LoanTopUp."Client Code") then begin
                    ...
                    DataSheet.Name := LoanApps."Client Name";
                    DataSheet."ID NO." := LoanApps."ID NO";
                    DataSheet."Amount ON" := LoanApps.Repayment;
    ...
    begin
        ...
        IF LoanApps.GET(VarLoanNo) THEN BEGIN
            ...
            LineNo := LineNo + 10000;
            SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::Loan,
            GenJournalLine."Account Type"::Customer, LoanApps."Client Code", rec."Posting Date", VarAmounttoDisburse, FORMAT(LoanApps.Source),
            ...
            IF LoanTopUp.FIND('-') THEN BEGIN
                REPEAT
                    //Message('%1|%2|%3', LoanTopUp."Principle Top Up", LoanTopUp."Interest Top Up", LoanTopUp.Commision);
                    //------------------------------------PAY-----------------------------------------------------------------------------------------------------------
                    //------------------------------------Principal---------------------
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::Repayment,
                    GenJournalLine."Account Type"::Customer, LoanApps."Client Code", rec."Posting Date", ROUND(LoanTopUp."Principle Top Up", 0.01, '>') * -1, 'BOSA', LoanTopUp."Loan Top Up",
                    'Off Set By - ' + LoanApps."Client Code" + '-' + LoanApps."Loan  No.", LoanTopUp."Loan Top Up", GenJournalLine."Application Source"::" ");
                    //------------------------------------Outstanding Interest----------
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::"Interest Paid",
                    GenJournalLine."Account Type"::Customer, LoanApps."Client Code", rec."Posting Date", ROUND(LoanTopUp."Interest Top Up", 0.01, '>') * -1, 'BOSA', LoanTopUp."Loan Top Up",
                    'Interest Paid- ' + LoanApps."Client Code" + '-' + LoanApps."Loan  No.", LoanTopUp."Loan Top Up", GenJournalLine."Application Source"::" ");
                    //-------------------------------------Levy--------------------------
                    LineNo := LineNo + 10000;
                    IF LoanType.GET(LoanApps."Loan Product Type") THEN BEGIN
                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
                        GenJournalLine."Account Type"::"G/L Account", LoanType."Top Up Commision Account", rec."Posting Date", ROUND(LoanTopUp.Commision, 0.01, '>') * -1, 'BOSA', LoanTopUp."Loan Top Up",
                        'Levy on Bridging -' + LoanApps."Client Code" + '-' + LoanApps."Loan  No.", LoanTopUp."Loan Top Up", GenJournalLine."Application Source"::" ");
                        Message('LoanTopUp.Commision %1', LoanTopUp.Commision);
                        //Error('Stop');
                    END;
            ...
            IF LoanType.GET(LoanApps."Loan Product Type") THEN BEGIN
                IF LoanType."Accrue Total Insurance&Interes" = TRUE THEN BEGIN
                    ...
                    IF PCharges.FIND('-') THEN BEGIN
                        ...
                        Message(' PChargeAmount%1', PChargeAmount);
            ...
            IF LoanApps."Share Capital Boosting" <> 0 THEN BEGIN
                ...
                GenJournalLine."Account Type"::Customer, LoanApps."Client Code", rec."Posting Date", LoanApps."Share Capital Boosting" * -1, 'BOSA', BLoan,
                'Share Capita Boosting Amount', BLoan, GenJournalLine."Application Source"::" ");
                Message('LoanApps."Share Capital Boosting" * -1 %1', LoanApps."Share Capital Boosting" * -1);
                Error('stop');

            end;

            IF LoanApps."Boosted Amount" <> 0 THEN BEGIN
                ...
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::"Deposit Contribution",
                GenJournalLine."Account Type"::Customer, LoanApps."Client Code", rec."Posting Date", LoanApps."Boosted Amount" * -1, 'BOSA', BLoan,
                'Deposits Boosting Amount', BLoan, GenJournalLine."Application Source"::" ");
            ...
            LineNo := LineNo + 10000;
            SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::" ",
            GenJournalLine."Account Type"::"Bank Account", LoanApps."Paying Bank Account No", rec."Posting Date", LoanApps."Loan Disbursed Amount" * -1, 'BOSA', LoanApps."Cheque No.",
